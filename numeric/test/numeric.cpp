...
BOOST_AUTO_TEST_CASE(pixel_numeric_operations_plus) {
  ...
  pixel_plus_t<rgb8_pixel_t, bgr8_pixel_t, rgb8_pixel_t> op;
  ...
  pixel_plus_t<rgb8_pixel_t, bgr8_pixel_t, bgr8_pixel_t> op2;
...
BOOST_AUTO_TEST_CASE(pixel_numeric_operations_multiply) {
  ...
  pixel_multiply_t<rgb32f_pixel_t, bgr32f_pixel_t, rgb32f_pixel_t> op;
...
BOOST_AUTO_TEST_CASE(pixel_numeric_operations_divide) {
  ...
  // float
  {
    ...
    pixel_divide_t<rgb32f_pixel_t, bgr32f_pixel_t, rgb32f_pixel_t> op;
    rgb32f_pixel_t c = op(a, b);

    float epsilon = 1e-6f;
    BOOST_CHECK_CLOSE(static_cast<float>(get_color(c, red_t())), 0.5f, epsilon);
    BOOST_CHECK_CLOSE(static_cast<float>(get_color(c, green_t())), 1.f,
                      epsilon);
    BOOST_CHECK_CLOSE(static_cast<float>(get_color(c, blue_t())), 1.5f,
                      epsilon);
  }
}
