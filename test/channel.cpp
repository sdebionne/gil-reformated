...
// Different core classes depending on the different types of channels - channel
// values, references and subbyte references The cores ensure there are two
// members, _min_v and _max_v initialized with the minimum and maximum channel
// value. The different channel types have different ways to initialize them,
// thus require different cores
...
protected:
  ...
  packed_dynamic_reference_core(int first_bit1 = 1, int first_bit2 = 2)
      : _min_v(&_min_buf, first_bit1), _max_v(&_max_buf, first_bit2) {
...
template <typename ChannelValue> void test_channel_value() {
  do_test<value_core<ChannelValue>>().test_all();
}

template <typename ChannelRef> void test_channel_reference() {
  do_test<reference_core<ChannelRef>>().test_all();
}

template <typename ChannelSubbyteRef> void test_packed_channel_reference() {
  do_test<packed_reference_core<ChannelSubbyteRef, ChannelSubbyteRef>>()
      .test_all();
}
...
void test_packed_dynamic_channel_reference() {
  do_test<packed_dynamic_reference_core<ChannelSubbyteRef, ChannelSubbyteRef>>()
      .test_all();
}

template <typename ChannelSubbyteRef, typename MutableRef>
void test_const_packed_dynamic_channel_reference() {
  do_test<packed_dynamic_reference_core<ChannelSubbyteRef, MutableRef>>()
      .test_all();
}

template <typename ChannelValue> void test_channel_value_impl() {
...
struct channel_archetype {
  ...
  template <typename Scalar> channel_archetype operator+=(Scalar) {
    return *this;
  }
  template <typename Scalar> channel_archetype operator-=(Scalar) {
    return *this;
...
void test_packed_channel_reference() {
  typedef packed_channel_reference<boost::uint16_t, 0, 5, true>
      channel16_0_5_reference_t;
  typedef packed_channel_reference<boost::uint16_t, 5, 6, true>
      channel16_5_6_reference_t;
  typedef packed_channel_reference<boost::uint16_t, 11, 5, true>
...
void test_packed_dynamic_channel_reference() {
  typedef packed_dynamic_channel_reference<boost::uint16_t, 5, true>
      channel16_5_reference_t;
  typedef packed_dynamic_channel_reference<boost::uint16_t, 6, true>
...
void test_channel() {
  ...
  // Do only compile-time tests for the archetype (because asserts like
  // val1<val2 fail)
