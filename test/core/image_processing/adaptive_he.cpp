...
gil::gray8c_view_t gray_view = gil::interleaved_view(
    4, 4, reinterpret_cast<gil::gray8c_pixel_t *>(image_matrix), 4);
...
void check_actual_clip_limit() {
  ...
  BOOST_TEST((std::abs(excess / h.size() + actual_limit) - limit * h.sum()) <
             epsilon);
}
...
void check_non_overlapping_interpolated_clahe() {
  ...
  }
  {
    ...
    BOOST_TEST(gil::equal_pixels(gil::subimage_view(view(img2), 0, 0, 4, 4),
                                 view(img3)));
    BOOST_TEST(gil::equal_pixels(gil::subimage_view(view(img2), 0, 4, 4, 4),
                                 view(img3)));
    BOOST_TEST(gil::equal_pixels(gil::subimage_view(view(img2), 4, 0, 4, 4),
                                 view(img3)));
    BOOST_TEST(gil::equal_pixels(gil::subimage_view(view(img2), 4, 4, 4, 4),
                                 view(img3)));
  }
}
