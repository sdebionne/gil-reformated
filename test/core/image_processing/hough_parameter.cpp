...
void from_step_count_test() {
  ...
  auto param = gil::hough_parameter<double>::from_step_count(
      middle_point, neighborhood, step_count);
...
void from_step_size_test(const double middle_point, const double step_size,
  ...
  auto param = gil::hough_parameter<double>::from_step_size(
      middle_point, neighborhood, step_size);
  BOOST_TEST(param.start_point ==
             middle_point - step_size * std::floor(expected_step_count / 2));
...
void minimum_step_angle_test(const std::ptrdiff_t width,
