...
void exact_fit_test(std::ptrdiff_t radius, gil::point_t offset,
  ...
  // const auto x_parameter = param_t::from_step_count(offset.x, neighborhood,
  // half_step_count); const auto y_parameter =
  // param_t::from_step_count(offset.y, neighborhood, half_step_count);
...
int main() {
  ...
  for (std::ptrdiff_t radius = 5; radius < test_dim_length; ++radius) {
    for (std::ptrdiff_t x_offset = radius; x_offset < radius + test_dim_length;
         ++x_offset) {
      for (std::ptrdiff_t y_offset = radius;
           y_offset < radius + test_dim_length; ++y_offset) {

        exact_fit_test(radius, {x_offset, y_offset},
                       gil::midpoint_circle_rasterizer{});
        exact_fit_test(radius, {x_offset, y_offset},
