...
void test_dx_sobel_kernel() {
  ...
  BOOST_TEST_ALL_EQ(kernel.begin(), kernel.end(), gil::dx_sobel.begin(),
                    gil::dx_sobel.end());
}

void test_dx_scharr_kernel() {
  auto const kernel = gil::generate_dx_scharr(1);
  BOOST_TEST_ALL_EQ(kernel.begin(), kernel.end(), gil::dx_scharr.begin(),
                    gil::dx_scharr.end());
}

void test_dy_sobel_kernel() {
  auto const kernel = gil::generate_dy_sobel(1);
  BOOST_TEST_ALL_EQ(kernel.begin(), kernel.end(), gil::dy_sobel.begin(),
                    gil::dy_sobel.end());
}

void test_dy_scharr_kernel() {
  auto const kernel = gil::generate_dy_scharr(1);
  BOOST_TEST_ALL_EQ(kernel.begin(), kernel.end(), gil::dy_scharr.begin(),
                    gil::dy_scharr.end());
}
