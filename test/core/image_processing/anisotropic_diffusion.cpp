...
void brightness_function_test() {
  ...
  for (const auto &pixel : rgb_pixels) {
    ...
    auto brightness_stencil =
        boost::gil::brightness_function::identity{}(stencil);
    for (const auto &brightness_pixel : brightness_stencil) {
      BOOST_TEST(pixel == brightness_pixel);
    }
  }

  std::vector<float> corresponding_luminance_values{8.878f, 98.5436f, 61.8362f,
                                                    242.0308f, 255};
  std::size_t index = 0;
  for (const auto &pixel : rgb_pixels) {
    boost::gil::laplace_function::stencil_type<gil::rgb32f_pixel_t> stencil;
    std::fill(stencil.begin(), stencil.end(), pixel);
    auto brightness_stencil =
        boost::gil::brightness_function::rgb_luminance{}(stencil);
    for (const auto &brightness_pixel : brightness_stencil) {
      gil::static_for_each(
          brightness_pixel, [&corresponding_luminance_values,
                             index](boost::gil::float32_t value) {
            BOOST_TEST(std::abs(value - corresponding_luminance_values[index]) <
                       1.0f);
          });
    }
    ++index;
  ...
  std::vector<gil::gray32f_pixel_t> gray_pixels{
      gil::gray32f_pixel_t(11),  gil::gray32f_pixel_t(0),
      gil::gray32f_pixel_t(255), gil::gray32f_pixel_t(123),
...
void heat_conservation_test(std::uint32_t seed) {
  ...
  constexpr std::ptrdiff_t num_channels =
      gil::num_channels<decltype(view)>::value;
  double before_diffusion[num_channels] = {0};
  for (auto &pixel : view) {
    for (std::size_t channel_index = 0; channel_index < num_channels;
         ++channel_index) {
  ...
  for (std::ptrdiff_t channel_index = 0; channel_index < num_channels;
       ++channel_index) {
    const auto percentage = before_diffusion[channel_index] != 0
                                ? std::abs(after_diffusion[channel_index] -
                                           before_diffusion[channel_index]) /
...
void laplace_functions_test() {
  ...
  // reduce result for 8 way should be ((-20-23+15+65)*0.5+(-14+33+6-8)) * 0.125
  // = (18.5+17) * 0.125 = 4.4375
