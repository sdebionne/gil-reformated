...
void check_helper_fn_pixel_to_tuple() {
  ...
  bool const same_gray_type =
      std::is_same<std::tuple<gil::uint8_t>, decltype(g2)>::value;
  ...
  bool const same_rgb_type =
      std::is_same<std::tuple<gil::uint8_t, gil::uint8_t, gil::uint8_t>,
                   decltype(r2)>::value;
  BOOST_TEST(same_rgb_type);
  BOOST_TEST(r1[0] == std::get<0>(r2) && r1[1] == std::get<1>(r2) &&
             r1[2] == std::get<2>(r2));

  auto r3 = gil::detail::pixel_to_tuple(r1, mp11::index_sequence<1, 2, 0>{});
  BOOST_TEST(r1[0] == std::get<2>(r3) && r1[1] == std::get<0>(r3) &&
             r1[2] == std::get<1>(r3));
}
...
void check_helper_fn_tuple_to_tuple() {
  ...
  bool const same_gray_type =
      std::is_same<std::tuple<int>, decltype(t2)>::value;
  BOOST_TEST(same_gray_type);
  ...
  bool const same_rgb_type =
      std::is_same<std::tuple<int, int, std::string>, decltype(r2)>::value;
  BOOST_TEST(same_rgb_type);
...
void check_helper_tuple_limit() {
  ...
  type1 t1_min(std::numeric_limits<int>::min(),
               std::numeric_limits<short>::min());
  type1 t1_max(std::numeric_limits<int>::max(),
               std::numeric_limits<short>::max());
  type2 t2_min(std::numeric_limits<unsigned int>::min(),
               std::numeric_limits<unsigned char>::min());
  type2 t2_max(std::numeric_limits<unsigned int>::max(),
