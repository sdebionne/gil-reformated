...
template <typename Locator> void test_locator_from_iterator() {
  // The `memory_based_2d_locator` for X-step is calculated based on adapted
  // iterator Only verify `type` member is available (i.e. specialization
  // defined).
  static_assert(
      std::is_class<typename gil::dynamic_x_step_type<Locator>::type>::value,
      "");

  static_assert(
      std::is_same<Locator,
                   typename gil::dynamic_y_step_type<Locator>::type>::value,
