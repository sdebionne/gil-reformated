...
endpoints
create_endpoints(std::mt19937 &twister,
...
//     BOOST_TEST_ALL_EQ(forward.begin(), forward.end(), backward.begin(),
//     backward.end());
// }
...
void test_bresenham_rasterizer_follows_equation(line_type line_points) {
  ...
  const double intercept = static_cast<double>(start.y) -
                           sign * slope * static_cast<double>(start.x);
  for (const auto &point : line_points) {
    double const expected_y =
        sign * slope * static_cast<double>(point.x) + intercept;
