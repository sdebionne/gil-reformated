...
void test_rasterizer_follows_equation(std::ptrdiff_t radius,
  ...
  for (std::size_t i = 0, octant_index = 0; i < circle_points.size();
       i += 8, ++octant_index) {
  ...
  for (const auto &point : first_octant) {
    ...
    std::ptrdiff_t lower_result =
        static_cast<std::ptrdiff_t>(std::floor(y_exact));
    std::ptrdiff_t upper_result =
        static_cast<std::ptrdiff_t>(std::ceil(y_exact));
...
void test_connectivity(std::ptrdiff_t radius, Rasterizer rasterizer) {
  ...
  for (std::size_t i = 0; i < 8; ++i) {
    ...
    for (std::size_t octant_index = i, index = 0;
         octant_index < circle_points.size(); octant_index += 8, ++index) {
