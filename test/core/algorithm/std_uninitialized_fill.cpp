...
template <typename Pixel> struct pixel_array {
  ...
  pixel_array(std::size_t x_size = default_x_size,
              std::size_t y_size = default_y_size)
      : pixels_(new Pixel[x_size * y_size]), x_size_(x_size), y_size_(y_size) {}
...
struct fill_with_pixel_integer_types {
  template <typename Pixel> void operator()(Pixel const &) {
    using pixel_t = Pixel;
...
struct fill_with_pixel_float_types {
  ...
  static void run() {
    boost::mp11::mp_for_each<fixture::pixel_float_types>(
        fill_with_pixel_float_types{});
  }
};
...
void test_fill_with_packed_pixel_gray3() {
  ...
  for (auto const &fill_pixel : {min_pixel, max_pixel, mid_pixel}) {
    ...
    for (fixture::packed_pixel_gray3 const &p : pixels) {
      BOOST_TEST_EQ(p, fill_pixel);
      BOOST_TEST_EQ((int)get_color(p, gil::gray_color_t()),
...
void test_fill_with_packed_pixel_bgr121() {
  ...
  for (auto const &fill_pixel : {min_pixel, max_pixel, mid_pixel}) {
    ...
    for (fixture::packed_pixel_bgr121 const &p : pixels) {
      BOOST_TEST_EQ(p, fill_pixel);
      BOOST_TEST_EQ((int)get_color(p, gil::red_t()),
                    (int)get_color(fill_pixel, gil::red_t()));
      BOOST_TEST_EQ((int)get_color(p, gil::green_t()),
                    (int)get_color(fill_pixel, gil::green_t()));
      BOOST_TEST_EQ((int)get_color(p, gil::blue_t()),
                    (int)get_color(fill_pixel, gil::blue_t()));
    }
  }
}
