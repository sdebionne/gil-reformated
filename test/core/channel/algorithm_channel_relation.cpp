...
template <typename ChannelFixtureBase> void test_channel_relation() {
  ...
  // b2 toolset=clang variant=release cxxstd=11 define=_GLIBCXX_USE_CXX11_ABI=0
  // libs/gil/test/core/channel//algorithm_channel_relation
...
struct test_channel_reference_const {
  ...
  static void run() {
    boost::mp11::mp_for_each<fixture::channel_byte_types>(
        test_channel_reference_const{});
  }
};

struct test_packed_channel_reference {
  template <typename BitField> void operator()(BitField const &) {
