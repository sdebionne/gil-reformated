...
void test_channel_value_convert_from_integral() {
  ...
  test_convert_to<fixture::channel_reference<channel_t &>>::from(f.min_v_,
                                                                 f.max_v_);
  test_convert_to<fixture::channel_reference<channel_t const &>>::from(
      f.min_v_, f.max_v_);

  // packed_channel_reference
  {
    using channels565_t = fixture::packed_channels565<std::uint16_t>;
    test_convert_to<typename channels565_t::fixture_0_5_t>::from(f.min_v_,
                                                                 f.max_v_);
    test_convert_to<typename channels565_t::fixture_5_6_t>::from(f.min_v_,
                                                                 f.max_v_);
    test_convert_to<typename channels565_t::fixture_11_5_t>::from(f.min_v_,
                                                                  f.max_v_);
  }
  // packed_dynamic_channel_reference
  {
    using channels565_t = fixture::packed_dynamic_channels565<std::uint16_t>;
    test_convert_to<typename channels565_t::fixture_5_t>::from(f.min_v_,
                                                               f.max_v_);
    test_convert_to<typename channels565_t::fixture_6_t>::from(f.min_v_,
                                                               f.max_v_);
  }
}

struct test_channel_value_convert_from_uint8_t {
  template <typename Channel> void operator()(Channel const &) {
...
struct test_channel_value_convert_from_float32_t {
  template <typename Channel> void operator()(Channel const &) {
    ...
    test_convert_to<fixture::channel_value<channel_t>>::from(f.min_v_,
                                                             f.max_v_);
  }
  static void run() {
