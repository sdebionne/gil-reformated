...
#define BOOST_TEST_MODULE                                                      \
  gil / test / core / channel / algorithm_channel_convert
...
template <typename ChannelFixtureBase> struct test_convert_to {
  using channel_t = typename fixture::channel<ChannelFixtureBase>::channel_t;
  using channel_value_t =
      typename fixture::channel<ChannelFixtureBase>::channel_value_t;
...
#define BOOST_GIL_TEST_CHANNEL_CONVERT_FROM(source_channel_type)               \
  ...
  BOOST_AUTO_TEST_CASE_TEMPLATE(channel_value, Channel,                        \
                                fixture::channel_byte_types) {                 \
    test_convert_to<fixture::channel_value<Channel>>::from(min_v_, max_v_);    \
  }                                                                            \
  BOOST_AUTO_TEST_CASE_TEMPLATE(channel_reference, Channel,                    \
                                fixture::channel_byte_types) {                 \
    test_convert_to<fixture::channel_reference<Channel &>>::from(min_v_,       \
                                                                 max_v_);      \
  }                                                                            \
  BOOST_AUTO_TEST_CASE_TEMPLATE(channel_reference_const, Channel,              \
                                fixture::channel_byte_types) {                 \
    test_convert_to<fixture::channel_reference<Channel const &>>::from(        \
        min_v_, max_v_);                                                       \
  }                                                                            \
  BOOST_AUTO_TEST_CASE(packed_channel_reference) {                             \
    using channels565_t = fixture::packed_channels565<std::uint16_t>;          \
    test_convert_to<typename channels565_t::fixture_0_5_t>::from(min_v_,       \
                                                                 max_v_);      \
    test_convert_to<typename channels565_t::fixture_5_6_t>::from(min_v_,       \
                                                                 max_v_);      \
    test_convert_to<typename channels565_t::fixture_11_5_t>::from(min_v_,      \
                                                                  max_v_);     \
  }                                                                            \
  BOOST_AUTO_TEST_CASE(packed_dynamic_channel_reference) {                     \
    using channels565_t = fixture::packed_dynamic_channels565<std::uint16_t>;  \
    test_convert_to<typename channels565_t::fixture_5_t>::from(min_v_,         \
                                                               max_v_);        \
    test_convert_to<typename channels565_t::fixture_6_t>::from(min_v_,         \
                                                               max_v_);        \
  }                                                                            \
...
BOOST_AUTO_TEST_CASE_TEMPLATE(channel_reference_const, Channel,
                              fixture::channel_integer_types) {
  test_convert_to<fixture::channel_reference<Channel const &>>::from(min_v_,
                                                                     max_v_);
}
