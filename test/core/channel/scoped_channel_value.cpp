...
BOOST_AUTO_TEST_CASE(scoped_channel_value_float64_t) {
  ...
  BOOST_TEST(gil::channel_traits<gil::float64_t>::min_value() == 0.0,
             tolerance);
  // max
  BOOST_TEST(gil::float_point_one<double>::apply() == 1.0, tolerance);
  BOOST_TEST(gil::channel_traits<gil::float64_t>::max_value() == 1.0,
             tolerance);
}
...
BOOST_AUTO_TEST_CASE(scoped_channel_value_halfs) {
  ...
  auto const tolerance = btt::tolerance(std::numeric_limits<double>::epsilon());
  BOOST_TEST(gil::channel_traits<halfs>::min_value() == minus_half::apply(),
             tolerance);
  BOOST_TEST(gil::channel_traits<halfs>::max_value() == plus_half::apply(),
             tolerance);
  // scoped channel maximum should map to the maximum
  BOOST_TEST(gil::channel_convert<std::uint16_t>(
                 gil::channel_traits<halfs>::max_value()) == 65535,
             tolerance);
}
