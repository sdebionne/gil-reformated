...
#define BOOST_GIL_TEST_IS_CLOSE(a, b, epsilon)                                 \
  BOOST_TEST(std::abs((a) - (b)) < (epsilon))
...
using fixture =
    gil::scoped_channel_value<std::uint8_t, int_minus_value, int_plus_value>;
...
void test_scoped_channel_value_float64_t() {
  ...
  BOOST_GIL_TEST_IS_CLOSE(gil::channel_traits<gil::float64_t>::min_value(), 0.0,
                          epsilon);
  // max
  BOOST_GIL_TEST_IS_CLOSE(gil::float_point_one<double>::apply(), 1.0, epsilon);
  BOOST_GIL_TEST_IS_CLOSE(gil::channel_traits<gil::float64_t>::max_value(), 1.0,
                          epsilon);
}
...
void test_scoped_channel_value_halfs() {
  ...
  auto const epsilon = std::numeric_limits<double>::epsilon();
  BOOST_GIL_TEST_IS_CLOSE(gil::channel_traits<halfs>::min_value(),
                          minus_half::apply(), epsilon);
  BOOST_GIL_TEST_IS_CLOSE(gil::channel_traits<halfs>::max_value(),
                          plus_half::apply(), epsilon);
