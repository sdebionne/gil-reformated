...
template <typename T> bool check_equal(T &cont1, T &cont2, std::size_t size) {
  bool ch = true;
...
template <typename T> bool check_equal(T &cont1, T &cont2) {
  bool ch = true;
  for (auto &it : cont1) {
...
void check_fill_histogram_vector() {
  ...
  gil::fill_histogram(gil::color_converted_view<gil::gray8_pixel_t>(v2),
                      c2_expected);
