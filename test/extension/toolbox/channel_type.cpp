...
void test_channel_type() {
  static_assert(std::is_same<unsigned char,
                             gil::channel_type<gil::rgb8_pixel_t>::type>::value,
                "");

  // float32_t is a scoped_channel_value object
  static_assert(
      std::is_same<gil::float32_t,
                   gil::channel_type<gil::rgba32f_pixel_t>::type>::value,
      "");

  // channel_type for bit_aligned images doesn't work with standard gil.
  using image_t =
      gil::bit_aligned_image4_type<4, 4, 4, 4, gil::rgb_layout_t>::type;
