...
using image_types =
    std::tuple<gil::gray8_image_t, gil::gray16_image_t, gil::gray32_image_t,
               gil::bgr8_image_t, gil::bgr16_image_t, gil::bgr32_image_t,
               gil::rgb8_image_t, gil::rgb16_image_t, gil::rgb32_image_t,
...
template <typename T> struct random_value {
  ...
  T operator()() {
    auto random_value = uid_(rng_);
    BOOST_ASSERT(range_min <= random_value && random_value <= range_max);
    return static_cast<T>(random_value);
  }
...
auto generate_image(std::ptrdiff_t size_x, std::ptrdiff_t size_y,
...
auto create_image(std::ptrdiff_t size_x, std::ptrdiff_t size_y,
                  int channel_value) -> Image {
  using pixel_t = typename Image::value_type;
  using channel_t = typename gil::channel_type<pixel_t>::type;
  static_assert(std::is_integral<channel_t>::value,
                "channel must be integral type");
...
auto create_kernel(std::initializer_list<T> const &values)
  ...
  BOOST_ASSERT_MSG((values.size() - 1) % 2 == 0,
                   "expected odd number of kernel values");
  gil::kernel_1d<T> kernel(values.begin(), values.size(),
                           (values.size() - 1) / 2);
  return kernel;
}
