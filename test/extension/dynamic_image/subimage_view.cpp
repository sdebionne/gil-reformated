...
struct test_subimage_equals_image_quadrants {
  template <typename Image> void operator()(Image const &) {
    ...
    gil::apply_operation(v0, fixture::fill_any_view<image_t>(
                                 {2, 3, 6, 7}, gil::const_view(i1)[0]));
    //  quadrant 2
    auto const i2 = fixture::create_image<image_t>(2, 2, 128);
    gil::apply_operation(v0, fixture::fill_any_view<image_t>(
                                 {0, 1, 4, 5}, gil::const_view(i2)[0]));
    //  quadrant 3
  ...
  static void run() {
    boost::mp11::mp_for_each<fixture::image_types>(
        test_subimage_equals_image_quadrants{});
  }
};
