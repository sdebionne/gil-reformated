...
void test_read_image_info_using_string() {
  ...
  }
  {
    ...
    using backend_t =
        gil::get_reader_backend<std::ifstream, gil::targa_tag>::type;
...
void test_read_and_convert_view() {
  {
    gil::rgb8_image_t img(124, 124);
    gil::read_and_convert_view(targa_filename, gil::view(img),
                               gil::targa_tag());
  }
  {
    std::ifstream in(targa_filename.c_str(), std::ios::binary);
...
void test_write_view() {
  {
    ...
    gil::write_view(filename,
                    create_mandel_view(124, 124, gil::rgb8_pixel_t(0, 0, 255),
...
void test_dynamic_image() {
  ...
  gil::write_view(targa_out + "dynamic_image_test.tga", gil::view(image),
                  gil::targa_tag());
}
