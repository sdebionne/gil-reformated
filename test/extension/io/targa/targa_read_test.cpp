...
void test_targa_scanline_reader(std::string filename) {
  test_scanline_reader<Image, gil::targa_tag>(
      std::string(targa_in + filename).c_str());
}
...
void test_read_header() {
  using backend_t =
      gil::get_reader_backend<std::string const, gil::targa_tag>::type;
...
void test_read_reference_images() {
  // 24BPP_compressed.tga
  {
    ...
    BOOST_TEST_EQ(gil::view(img)(width - 1, height - 1),
                  gil::rgb8_pixel_t(248, 0, 248));
  ...
  // 32BPP_compressed.tga
  {
    ...
    BOOST_TEST_EQ(gil::view(img)(width - 1, 0),
                  gil::rgba8_pixel_t(0, 0, 248, 255));
    BOOST_TEST_EQ(gil::view(img)(0, height - 1),
                  gil::rgba8_pixel_t(0, 0, 0, 0));
    BOOST_TEST_EQ(gil::view(img)(width - 1, height - 1),
                  gil::rgba8_pixel_t(248, 0, 248, 255));
  ...
  {
    gil::rgb8_image_t img;
    gil::read_image(targa_in + "24BPP_compressed_ul_origin.tga", img,
                    gil::targa_tag());
  ...
  {
    gil::rgb8_image_t img;
    gil::read_image(targa_in + "24BPP_uncompressed_ul_origin.tga", img,
                    gil::targa_tag());
  ...
  {
    gil::rgba8_image_t img;
    gil::read_image(targa_in + "32BPP_compressed_ul_origin.tga", img,
                    gil::targa_tag());
  ...
  {
    gil::rgba8_image_t img;
    gil::read_image(targa_in + "32BPP_uncompressed_ul_origin.tga", img,
                    gil::targa_tag());
...
void test_partial_image() {
  ...
  gil::read_image(filename, img,
                  gil::image_read_settings<gil::targa_tag>(
                      gil::point_t(0, 0), gil::point_t(50, 50)));

  gil::write_view(targa_out + "targa_partial.tga", gil::view(img),
                  gil::targa_tag());
}
