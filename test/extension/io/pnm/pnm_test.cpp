...
void test_read_image_info_using_string() {
  {
    using backend_t =
        gil::get_reader_backend<std::string const, gil::pnm_tag>::type;
  ...
  }
  {
    ...
    using backend_t =
        gil::get_reader_backend<std::ifstream, gil::pnm_tag>::type;
...
#ifdef BOOST_GIL_IO_TEST_ALLOW_WRITING_IMAGES
  gil::write_view(pnm_out + "dynamic_image_test.pnm", gil::view(image),
                  gil::pnm_tag());
#endif // BOOST_GIL_IO_TEST_ALLOW_WRITING_IMAGES
}
