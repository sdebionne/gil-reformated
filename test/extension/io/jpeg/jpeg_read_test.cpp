...
void test_read_header() {
  using backend_t =
      gil::get_reader_backend<std::string const, gil::jpeg_tag>::type;
...
void test_read_pixel_density() {
  using backend_t =
      gil::get_reader_backend<std::string const, gil::jpeg_tag>::type;
  backend_t backend =
      gil::read_image_info(jpeg_in + "EddDawson/36dpi.jpg", gil::jpeg_tag());
  ...
  std::stringstream in_memory(std::ios_base::in | std::ios_base::out |
                              std::ios_base::binary);
  gil::write_view(in_memory, gil::view(img), write_settings);

  using backend2_t =
      gil::get_reader_backend<std::stringstream, gil::jpeg_tag>::type;
...
#ifdef BOOST_GIL_IO_TEST_ALLOW_WRITING_IMAGES
  gil::write_view(jpeg_out + "fast_dct_read_test.jpg", gil::view(img),
                  gil::jpeg_tag());
#endif // BOOST_GIL_IO_TEST_ALLOW_WRITING_IMAGES
}
