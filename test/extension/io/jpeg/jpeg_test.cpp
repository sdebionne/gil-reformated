...
void test_read_image_info() {
  ...
  }
  {
    ...
    using backend_t =
        gil::get_reader_backend<std::ifstream, gil::jpeg_tag>::type;
  ...
  }
  {
    ...
    backend_t backend =
        gil::read_image_info(fs::path(jpeg_filename), gil::jpeg_tag());
...
void test_write_view() {
  ...
  auto const g = gil::rgb8_pixel_t(0, 255, 0);
  {
    ...
    gil::write_view(filename, create_mandel_view(320, 240, b, g),
                    gil::jpeg_tag());
  }
  {
...
void test_stream() {
  ...
  std::stringstream in_buffer(std::ios_base::in | std::ios_base::out |
                              std::ios_base::binary);
...
void test_dynamic_image() {
  gil::any_image<gil::gray8_image_t, gil::gray16_image_t, gil::rgb8_image_t,
                 gil::rgba8_image_t>
      image;
  gil::read_image(jpeg_filename.c_str(), image, gil::jpeg_tag());

#ifdef BOOST_GIL_IO_TEST_ALLOW_WRITING_IMAGES
  gil::write_view(jpeg_out + "old_dynamic_image_test.jpg", gil::view(image),
                  gil::jpeg_tag());
#endif // BOOST_GIL_IO_TEST_ALLOW_WRITING_IMAGES
}
