...
using gray_alpha8c_pixel_t =
    gil::pixel<std::uint8_t, gil::gray_alpha_layout_t> const;
using gray_alpha8_image_t = gil::image<gray_alpha8_pixel_t, false>;

using gray_alpha16_pixel_t =
    gil::pixel<std::uint16_t, gil::gray_alpha_layout_t>;
using gray_alpha16c_pixel_t =
    gil::pixel<std::uint16_t, gil::gray_alpha_layout_t> const;
using gray_alpha16_image_t = gil::image<gray_alpha16_pixel_t, false>;

template <typename Image> void test_file(std::string filename) {
  Image src, dst;
  ...
  using backend_t =
      gil::get_reader_backend<std::string const, gil::png_tag>::type;
...
template <typename Image> void test_png_scanline_reader(std::string filename) {
  test_scanline_reader<Image, gil::png_tag>(
      std::string(png_in + filename).c_str());
}
...
void test_read_pixel_per_meter() {
  ...
  using backend_t =
      gil::get_reader_backend<std::string const, gil::png_tag>::type;
  backend_t backend =
      gil::read_image_info(png_base_in + "EddDawson/36dpi.png", settings);

  BOOST_TEST_EQ(backend._info._pixels_per_meter, png_uint_32(1417));
}

void test_read_with_trns_chunk_color_type_0() {
  // PNG 1.2: For color type 0 (grayscale), the tRNS chunk contains a single
...
void test_read_with_trns_chunk_color_type_2() {
  // PNG 1.2: For color type 2 (truecolor), the tRNS chunk contains a single RGB
  ...
  // Blue:  2 bytes, range 0 .. (2^bitdepth)-1
  {
    ...
    BOOST_TEST_EQ(backend._info._width, 32u);
    BOOST_TEST_EQ(backend._info._height, 32u);
    BOOST_TEST_EQ(const_view(img).front(),
                  gil::rgba16c_pixel_t(65535, 65535, 65535, 0));
    BOOST_TEST_EQ(const_view(img)[78],
                  gil::rgba16c_pixel_t(58339, 58339, 58339, 65535));
    BOOST_TEST_EQ(const_view(img)[79],
                  gil::rgba16c_pixel_t(51657, 51657, 51657, 65535));
    BOOST_TEST_EQ(const_view(img)[975],
                  gil::rgba16c_pixel_t(62965, 62965, 62965, 65535));
    BOOST_TEST_EQ(const_view(img)[976],
                  gil::rgba16c_pixel_t(58339, 58339, 58339, 65535));
    BOOST_TEST_EQ(const_view(img).back(),
                  gil::rgba16c_pixel_t(65535, 65535, 65535, 0));
  }
  {
  ...
  }
  {
    ...
    BOOST_TEST_EQ(backend._info._height, 32u);
    BOOST_TEST_EQ(const_view(img).front(),
                  gil::rgba8c_pixel_t(255, 255, 255, 0));
    ...
    BOOST_TEST_EQ(const_view(img)[975],
                  gil::rgba8c_pixel_t(245, 245, 245, 255));
    BOOST_TEST_EQ(const_view(img)[976],
                  gil::rgba8c_pixel_t(227, 227, 227, 255));
    BOOST_TEST_EQ(const_view(img).back(),
                  gil::rgba8c_pixel_t(255, 255, 255, 0));
  }
}
...
void test_read_with_trns_chunk_color_type_3() {
  // PNG 1.2: For color type 3 (indexed color), the tRNS chunk contains a series
  ...
  // ...etc...
  {
    ...
    BOOST_TEST_EQ(const_view(img)[79], gil::rgba8c_pixel_t(201, 201, 201, 255));
    BOOST_TEST_EQ(const_view(img)[975],
                  gil::rgba8c_pixel_t(246, 246, 246, 255));
    BOOST_TEST_EQ(const_view(img)[976],
                  gil::rgba8c_pixel_t(227, 227, 227, 255));
    BOOST_TEST_EQ(const_view(img).back(),
                  gil::rgba8c_pixel_t(255, 255, 255, 0));
