...
template <typename Image> void test_file(string filename) {
  Image src, dst;
  ...
  backend_t backend = read_image_info(png_in + filename, settings);

  read_image(png_in + filename, src, settings);

#ifdef BOOST_GIL_IO_TEST_ALLOW_WRITING_IMAGES
  image_write_info<png_tag> write_info;
  write_info._file_gamma = backend._info._file_gamma;

  write_view(png_out + filename, view(src), write_info);

  read_image(png_out + filename, dst, settings);

  BOOST_CHECK(equal_pixels(const_view(src), const_view(dst)));
#endif // BOOST_GIL_IO_TEST_ALLOW_WRITING_IMAGES
}

template <typename Image> void test_png_scanline_reader(string filename) {
...
BOOST_AUTO_TEST_CASE(read_header_test) {
  ...
  backend_t backend = read_image_info(png_filename, tag_t());
  ...
  BOOST_CHECK_EQUAL(backend._info._compression_method,
...
BOOST_AUTO_TEST_CASE(read_with_trns_chunk_color_type_2) {
  ...
  // Blue:  2 bytes, range 0 .. (2^bitdepth)-1
  {
    ...
    BOOST_TEST(const_view(img).front() ==
               rgba16c_pixel_t(65535, 65535, 65535, 0));
    BOOST_TEST(const_view(img)[78] ==
               rgba16c_pixel_t(58339, 58339, 58339, 65535));
    BOOST_TEST(const_view(img)[79] ==
               rgba16c_pixel_t(51657, 51657, 51657, 65535));
    BOOST_TEST(const_view(img)[975] ==
               rgba16c_pixel_t(62965, 62965, 62965, 65535));
