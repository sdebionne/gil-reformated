...
#define BOOST_GIL_TEST_NAME_SUBIMAGE_TEST(n, data)                             \
  BOOST_PP_CAT(BOOST_PP_CAT(BOOST_PP_CAT(test_, BOOST_PP_CAT(data, _)), n),    \
               bit_bit_aligned)
...
#define BOOST_GIL_TEST_GENERATE_SUBIMAGE_TEST(z, n, data)                      \
  void BOOST_GIL_TEST_NAME_SUBIMAGE_TEST(n, data)() {                          \
    ...
    gil::bit_aligned_image1_type<n, gil::gray_layout_t>::type img1, img2,      \
        img3;                                                                  \
    ...
    BOOST_TEST(                                                                \
        gil::equal_pixels(gil::const_view(img1), gil::const_view(img2)));      \
    BOOST_TEST(gil::equal_pixels(                                              \
        gil::const_view(img1),                                                 \
        gil::subimage_view(gil::view(img3), top_left, dim)));                  \
  }
...
BOOST_PP_REPEAT_FROM_TO(9, 16, BOOST_GIL_TEST_GENERATE_SUBIMAGE_TEST,
                        minisblack)
BOOST_PP_REPEAT_FROM_TO(17, 27, BOOST_GIL_TEST_GENERATE_SUBIMAGE_TEST,
                        minisblack)
...
void test_subimage_test_8() {
  ...
  gil::read_image(tiff_in_GM + "tiger-minisblack-strip-08.tif", img3,
                  gil::tiff_tag());

  BOOST_TEST(gil::equal_pixels(gil::const_view(img1), gil::const_view(img2)));
  BOOST_TEST(
      gil::equal_pixels(gil::const_view(img1),
                        gil::subimage_view(gil::view(img3), top_left, dim)));
}
...
void test_subimage_test_16() {
  ...
  gil::read_image(tiff_in_GM + "tiger-minisblack-strip-16.tif", img3,
                  gil::tiff_tag());
...
void test_subimage_test_32() {
  ...
  gil::read_image(tiff_in_GM + "tiger-minisblack-strip-32.tif", img3,
                  gil::tiff_tag());
