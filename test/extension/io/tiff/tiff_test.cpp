...
void test_read_image_info() {
  {
    using backend_t =
        gil::get_reader_backend<std::string const, gil::tiff_tag>::type;
  ...
  }
  {
    ...
    using backend_t =
        gil::get_reader_backend<std::ifstream, gil::tiff_tag>::type;
...
void test_dynamic_image() {
  ...
  using my_img_types =
      mp11::mp_list<gil::gray8_image_t, gil::gray16_image_t, gil::rgb8_image_t,
                    gil::rgba8_image_t, gil::gray1_image_t>;
  gil::any_image<my_img_types> image;

  gil::read_image(tiff_filename.c_str(), image, gil::tiff_tag());

#ifdef BOOST_GIL_IO_TEST_ALLOW_WRITING_IMAGES
  gil::write_view(tiff_out + "dynamic_image_test.tif", gil::view(image),
                  gil::tiff_tag());
#endif // BOOST_GIL_IO_TEST_ALLOW_WRITING_IMAGES
}
