...
// TODO: Rename macros to use BOOST_GIL_ prefix. See
// https://github.com/boostorg/gil/issues/410 ~mloskot
...
#define BOOST_GIL_TEST_NAME_TILE_STRIP_COMPARISON_BIT_ALIGNED_RGB(n, data)     \
  BOOST_PP_CAT(                                                                \
      BOOST_PP_CAT(                                                            \
          BOOST_PP_CAT(                                                        \
              BOOST_PP_CAT(BOOST_PP_CAT(read_tile_and_compare_with_,           \
...
#define BOOST_GIL_TEST_GENERATE_TILE_STRIP_COMPARISON_BIT_ALIGNED_RGB(z, n,    \
                                                                      data)    \
  void BOOST_GIL_TEST_NAME_TILE_STRIP_COMPARISON_BIT_ALIGNED_RGB(n, data)() {  \
    ...
    if (BOOST_PP_LESS(n, 10) == 1)                                             \
      padding = "0";                                                           \
    filename_strip =                                                           \
        filename_strip + padding + BOOST_PP_STRINGIZE(n) + ".tif";             \
    filename_tile = filename_tile + padding + BOOST_PP_STRINGIZE(n) + ".tif";  \
    gil::bit_aligned_image3_type<n, n, n, gil::rgb_layout_t>::type img_strip,  \
        img_tile;                                                              \
    gil::read_image(filename_strip, img_strip, gil::tiff_tag());               \
    gil::read_image(filename_tile, img_tile, gil::tiff_tag());                 \
    BOOST_TEST_EQ(gil::equal_pixels(gil::const_view(img_strip),                \
                                    gil::const_view(img_tile)),                \
                  true);                                                       \
  }

// Special case for minisblack images
#define BOOST_GIL_TEST_NAME_TILE_TILE_STRIP_COMPARISON_BIT_ALIGNED_MINISBLACK( \
    n, data)                                                                   \
  BOOST_PP_CAT(                                                                \
      BOOST_PP_CAT(                                                            \
          BOOST_PP_CAT(BOOST_PP_CAT(read_tile_and_compare_with_, data),        \
                       _strip_),                                               \
          n),                                                                  \
      bit_bit_aligned)

#define BOOST_GIL_TEST_GENERATE_TILE_STRIP_COMPARISON_BIT_ALIGNED_MINISBLACK(  \
    z, n, data)                                                                \
  void BOOST_GIL_TEST_NAME_TILE_TILE_STRIP_COMPARISON_BIT_ALIGNED_MINISBLACK(  \
      n, data)() {                                                             \
    namespace gil = boost::gil;                                                \
    std::string filename_strip(tiff_in_GM + "tiger-" +                         \
                               BOOST_PP_STRINGIZE(data) + "-strip-");          \
    std::string filename_tile(tiff_in_GM + "tiger-" +                          \
                              BOOST_PP_STRINGIZE(data) + "-tile-");            \
    std::string padding("");                                                   \
    if (BOOST_PP_LESS(n, 10) == 1)                                             \
      padding = "0";                                                           \
    filename_strip =                                                           \
        filename_strip + padding + BOOST_PP_STRINGIZE(n) + ".tif";             \
    filename_tile = filename_tile + padding + BOOST_PP_STRINGIZE(n) + ".tif";  \
    gil::bit_aligned_image1_type<n, gil::gray_layout_t>::type img_strip,       \
        img_tile;                                                              \
    gil::read_image(filename_strip, img_strip, gil::tiff_tag());               \
    gil::read_image(filename_tile, img_tile, gil::tiff_tag());                 \
    BOOST_TEST_EQ(gil::equal_pixels(gil::const_view(img_strip),                \
