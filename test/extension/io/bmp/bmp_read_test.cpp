...
template <typename Image> void write(Image &img, std::string const &file_name) {
  write_view(bmp_out + file_name, gil::view(img), gil::bmp_tag());
}

void test_read_header() {
  using backend_t =
      gil::get_reader_backend<std::string const, gil::bmp_tag>::type;
...
void test_read_reference_images_image_iterator() {
  ...
  // g04.bmp - basic 4bpp (16 color) image
  test_scanline_reader<gil::rgba8_image_t, gil::bmp_tag>(
      std::string(bmp_in + "g04.bmp").c_str());
  ...
  // g08.bmp - basic 8bpp (256 color) image
  test_scanline_reader<gil::rgba8_image_t, gil::bmp_tag>(
      std::string(bmp_in + "g08.bmp").c_str());
