...
template <typename Op> double measure_time(Op op, std::size_t num_loops) {
  clock_t begin = clock();
...
#define RGB_VIEW(T)                                                            \
  image_view<memory_based_2d_locator<                                          \
      memory_based_step_iterator<pixel<T, rgb_layout_t> *>>>
#define BGR_VIEW(T)                                                            \
  image_view<memory_based_2d_locator<                                          \
      memory_based_step_iterator<pixel<T, bgr_layout_t> *>>>
#define RGB_PLANAR_VIEW(T)                                                     \
  image_view<memory_based_2d_locator<                                          \
      memory_based_step_iterator<planar_pixel_iterator<T *, rgb_t>>>>

template <typename View, typename P> struct fill_gil_t {
  View _v;
  P _p;
...
template <typename View, typename P> void test_fill(std::size_t trials) {
  image<typename View::value_type, is_planar<View>::value> im(width, height);
  std::cout << "GIL: "
            << measure_time(fill_gil_t<View, P>(view(im), P()), trials)
            << std::endl;
  std::cout << "Non-GIL: "
            << measure_time(fill_nongil_t<View, P>(view(im), P()), trials)
            << std::endl;
};

template <typename T> struct rgb_fr_t {
...
template <typename View, typename F> struct for_each_gil_t {
  View _v;
  F _f;
...
template <typename View, typename F> void test_for_each(std::size_t trials) {
  image<typename View::value_type, is_planar<View>::value> im(width, height);
  std::cout << "GIL: "
            << measure_time(for_each_gil_t<View, F>(view(im), F()), trials)
            << std::endl;
  std::cout << "Non-GIL: "
            << measure_time(for_each_nongil_t<View, F>(view(im), F()), trials)
            << std::endl;
}

// copy
template <typename View1, typename View2> struct copy_gil_t {
  View1 _v1;
  View2 _v2;
...
template <typename View1, typename View2> void test_copy(std::size_t trials) {
  ...
  std::cout << "GIL: "
            << measure_time(copy_gil_t<View1, View2>(view(im1), view(im2)),
                            trials)
            << std::endl;
  std::cout << "Non-GIL: "
            << measure_time(copy_nongil_t<View1, View2>(view(im1), view(im2)),
                            trials)
            << std::endl;
}

// transform()
template <typename T, typename Pixel> struct bgr_to_rgb_t {
...
template <typename View1, typename View2, typename F> struct transform_gil_t {
  View1 _v1;
  View2 _v2;
  F _f;
...
void test_transform(std::size_t trials) {
  ...
  image<typename View2::value_type, is_planar<View2>::value> im2(width, height);
  // std::cout << "GIL: "
  // <<measure_time(transform_gil_t<View1,View2,F>(view(im1),view(im2),F()),trials)
  // << std::endl; std::cout << "Non-GIL:
  // "<<measure_time(transform_nongil_t<View1,View2,F>(view(im1),view(im2),F()),trials)
  // << std::endl;
}
...
BOOST_AUTO_TEST_CASE(performance_test) {
  ...
  // fill()
  std::cout << "test fill_pixels() on rgb8_image_t with rgb8_pixel_t"
            << std::endl;
  test_fill<rgb8_view_t, rgb8_pixel_t>(num_trials);
  std::cout << std::endl;

  std::cout << "test fill_pixels() on rgb8_planar_image_t with rgb8_pixel_t"
            << std::endl;
  test_fill<rgb8_planar_view_t, rgb8_pixel_t>(num_trials);
  std::cout << std::endl;

  std::cout << "test fill_pixels() on rgb8_image_t with bgr8_pixel_t"
            << std::endl;
  test_fill<rgb8_view_t, bgr8_pixel_t>(num_trials);
  std::cout << std::endl;

  std::cout << "test fill_pixels() on rgb8_planar_image_t with bgr8_pixel_t"
            << std::endl;
  ...
  // copy()
  std::cout << "test copy_pixels() between rgb8_image_t and rgb8_image_t"
            << std::endl;
  test_copy<rgb8_view_t, rgb8_view_t>(num_trials);
  std::cout << std::endl;

  std::cout << "test copy_pixels() between rgb8_image_t and bgr8_image_t"
            << std::endl;
  test_copy<rgb8_view_t, bgr8_view_t>(num_trials);
  std::cout << std::endl;

  std::cout << "test copy_pixels() between rgb8_planar_image_t and "
               "rgb8_planar_image_t"
            << std::endl;
  test_copy<rgb8_planar_view_t, rgb8_planar_view_t>(num_trials);
  std::cout << std::endl;

  std::cout << "test copy_pixels() between rgb8_image_t and rgb8_planar_image_t"
            << std::endl;
  test_copy<rgb8_view_t, rgb8_planar_view_t>(num_trials);
  std::cout << std::endl;

  std::cout << "test copy_pixels() between rgb8_planar_image_t and rgb8_image_t"
            << std::endl;
  test_copy<rgb8_planar_view_t, rgb8_view_t>(num_trials);
  std::cout << std::endl;

  // transform()
  std::cout << "test transform_pixels() between rgb8_image_t and rgb8_image_t"
            << std::endl;
  test_transform<rgb8_view_t, rgb8_view_t,
                 bgr_to_rgb_t<uint8_t, pixel<uint8_t, rgb_layout_t>>>(
      num_trials);
  std::cout << std::endl;

  std::cout << "test transform_pixels() between rgb8_planar_image_t and "
               "rgb8_planar_image_t"
            << std::endl;
  test_transform<rgb8_planar_view_t, rgb8_planar_view_t,
                 bgr_to_rgb_t<uint8_t, planar_pixel_reference<uint8_t, rgb_t>>>(
      num_trials);
  std::cout << std::endl;

  std::cout
      << "test transform_pixels() between rgb8_image_t and rgb8_planar_image_t"
      << std::endl;
  test_transform<rgb8_view_t, rgb8_planar_view_t,
                 bgr_to_rgb_t<uint8_t, pixel<uint8_t, rgb_layout_t>>>(
      num_trials);
  std::cout << std::endl;

  std::cout
      << "test transform_pixels() between rgb8_planar_image_t and rgb8_image_t"
      << std::endl;
  test_transform<rgb8_planar_view_t, rgb8_view_t,
                 bgr_to_rgb_t<uint8_t, planar_pixel_reference<uint8_t, rgb_t>>>(
      num_trials);
  std::cout << std::endl;
}
