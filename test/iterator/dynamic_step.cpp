...
template <typename Iterator> void test_non_step() {
  static_assert(
      std::is_same<gil::memory_based_step_iterator<Iterator>,
...
template <typename Iterator> void test_step() {
  static_assert(
      std::is_same<Iterator,
                   typename gil::dynamic_x_step_type<Iterator>::type>::value,
      "iterator does not model HasDynamicXStepTypeConcept");

  static_assert(!std::is_same<typename Iterator::x_iterator,
                              typename gil::dynamic_x_step_type<
                                  gil::gray8_step_ptr_t>::type>::value,
                "dynamic_x_step_type yields X step iterator, not X iterator");
}
