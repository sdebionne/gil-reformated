...
int main() {
  ...
  static_assert(
      ...
      "2-bit packed_dynamic_channel_reference should be recognized as "
      "integral");

  static_assert(boost::is_integral<gil::packed_dynamic_channel_reference<
                    std::uint16_t, 15, true>>::value,
                "15-bit packed_dynamic_channel_reference should be recognized "
                "as integral");
  ...
  static_assert(
      boost::is_integral<gil::scoped_channel_value<
          std::uint8_t, int_minus_value, int_plus_value>>::value,
      "integer-based scoped_channel_value should be recognized as integral");

  static_assert(
      !boost::is_integral<gil::float32_t>::value,
      "float-based packed_channel_value should not be recognized as integral");

  static_assert(
      !boost::is_integral<gil::float64_t>::value,
      "float-based packed_channel_value should not be recognized as integral");
}
