...
template <typename T> struct printable_numeric {
...
struct print_color_base {
  ...
  template <typename Element> void operator()(Element const &c) {
  ...
  void operator()(gil::packed_channel_reference<BitField, FirstBit, NumBits,
                                                IsMutable> const &c) {
  ...
  void operator()(
      gil::scoped_channel_value<BaseChannelValue, MinVal, MaxVal> const &c) {
...
std::ostream &operator<<(std::ostream &os,
                         packed_pixel<BitField, ChannelRefs, Layout> const &p) {
  os << "packed_pixel<"
...
operator<<(std::ostream &os,
  ...
  os << "planar_pixel_reference<"
     << "\nChannelReference="
     << boost::core::demangled_name(typeid(ChannelReference))
