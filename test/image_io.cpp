...
typedef any_image<
    boost::mpl::vector<gray8_image_t, bgr8_image_t, argb8_image_t, rgb8_image_t,
                       rgb8_planar_image_t, cmyk8_image_t, cmyk8_planar_image_t,
                       rgba8_image_t, rgba8_planar_image_t>>
    any_image_t;
...
void test_image_io() {
  ...
  jpeg_write_view(out_dir + "grayFromRGB.jpg",
                  color_converted_view<rgb8_pixel_t>(view(imgGray)));
  ...
  png_write_view(out_dir + "grayFromRGB.png",
                 color_converted_view<rgb8_pixel_t>(view(imgGray)));
  ...
  // JPEG
  tiff_read_and_convert_image(in_dir + "gray.tif",
                              imgGray32); // again TIF (jpeg load not supported)
  // save RGB image to gray jpeg
  tiff_write_view(out_dir + "gray32FromGray.jpg", view(imgGray32));

  // load RGB jpeg into RGB image
  tiff_read_and_convert_image(in_dir + "RGB.tif",
                              imgGray32); // again TIF (jpeg load not supported)
  // save RGB image to RGB jpeg
  tiff_write_view(out_dir + "gray32FromRGB.jpg",
                  color_converted_view<rgb32f_pixel_t>(view(imgGray32)));
