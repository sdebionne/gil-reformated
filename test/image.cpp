...
// compiler doesn't realize that the two types are the same)
#pragma warning(                                                               \
...
void image_test::packed_image_test() {
  ...
  // TODO: Commented out because there is an error with packed images under
  // GCC 4.1.2, but not under VC8
...
private:
  const char *_filename;
  virtual void check_view_impl(const rgb8c_view_t &img_view,
                               const string &name);
  virtual void finalize();
};
...
protected:
  void check_view_impl(const boost::gil::rgb8c_view_t &img_view,
                       const string &name) {
