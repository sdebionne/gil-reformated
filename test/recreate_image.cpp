...
std::size_t get_row_size_in_memunits(
    typename View::x_coord_t width) { // number of units per row
  std::size_t size_in_memunits =
      width * memunit_step(typename View::x_iterator());
...
std::size_t
total_allocated_size_in_bytes(const typename View::point_t &dimensions) {
...
BOOST_AUTO_TEST_CASE(recreate_image_test) {
  auto tasib_1 = total_allocated_size_in_bytes<rgb8_view_t, false>(
      point2<rgb8_view_t::coord_t>(640, 480));
  auto tasib_2 = total_allocated_size_in_bytes<rgb8_view_t, false>(
