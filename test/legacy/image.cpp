...
// When BOOST_GIL_GENERATE_REFERENCE_DATA is defined, the reference data is
// generated and saved. When it is undefined, regression tests are checked
// against it
...
template <typename V, typename R> void get_hist(const V &img_view, R &hist) {
...
template <typename C1> struct my_color_converter_impl<C1, gray_t> {
  template <typename P1, typename P2>
  void operator()(const P1 &src, P2 &dst) const {
    default_color_converter_impl<C1, gray_t>()(src, dst);
    get_color(dst, gray_color_t()) =
        channel_invert(get_color(dst, gray_color_t()));
  }
};
...
template <typename P> struct mandelbrot_fn {
  ...
  result_type operator()(const point_t &p) const {
    ...
    for (std::size_t k = 0; k < num_channels<P>::value; ++k)
      ret[k] =
          (typename channel_type<value_type>::type)(_in_color[k] * t +
                                                    _out_color[k] * (1 - t));
    return ret;
  }
...
template <typename T> void x_gradient(const T &src, const gray8s_view_t &dst) {
...
protected:
  virtual void check_view_impl(const rgb8c_view_t &view,
                               const string &name) = 0;
  template <typename View>
...
template <typename Img> void image_test::basic_test(const string &prefix) {
  ...
  // fill it with red
  rgb8_pixel_t red8(255, 0, 0), green8(0, 255, 0), blue8(0, 0, 255),
      white8(255, 255, 255);
  typename View::value_type red, green, blue, white;
  color_convert(red8, red);
  default_color_converter()(red8, red);
  red = color_convert_deref_fn<rgb8_ref_t, typename Img::view_t::value_type>()(
      red8);
  ...
  Img imgWhite(img);
  for (typename View::iterator it = view(imgWhite).end();
       (it - 1) != view(imgWhite).begin(); --it) {
    if (*(it - 1) == red)
      *(it - 1) = white;
  }
...
template <typename View>
void image_test::view_transformations_test(const View &img_view,
                                           const string &prefix) {
  check_view(img_view, prefix + "original");

  check_view(subimage_view(img_view, iround(img_view.dimensions() / 4),
                           iround(img_view.dimensions() / 2)),
             prefix + "cropped");
...
void image_test::virtual_view_test() {
  ...
  fill_pixels(view(img), 0); // our x_gradient algorithm doesn't change the
                             // first & last column, so make sure they are 0
  x_gradient(color_converted_view<gray8_pixel_t>(mandel), view(img));
  check_view(color_converted_view<gray8_pixel_t>(const_view(img)),
...
void image_test::dynamic_image_test() {
  using any_image_t = any_image<gray8_image_t, bgr8_image_t, argb8_image_t,
  ...
  check_view(subsampled_view(rotated180_view(view(any_img)), 2, 1),
             "dynamic_subimage_subsampled180rot");
}

template <typename Img> void image_test::image_all_test(const string &prefix) {
...
// Create a checksum for the given view and compare it with the reference
// checksum. Throw exception if different
void checksum_image_test::check_view_impl(const rgb8c_view_t &img_view,
                                          const string &name) {
  ...
  if (crc == crc_expect) {
    cerr << "Checking checksum for " << name << " (crc=" << std::hex << crc
         << ")" << endl;
  } else {
    cerr << "Checksum error in " << name << " (crc=" << std::hex << crc
         << " != " << std::hex << crc_expect << ")" << endl;
    error_if(true);
  }
}
...
private:
  const char *_filename;
  void check_view_impl(const rgb8c_view_t &img_view,
                       const string &name) override;
  void finalize() override;
};

// Add the checksum of the given view to the map of checksums
void checksum_image_generate::check_view_impl(const rgb8c_view_t &img_view,
                                              const string &name) {
...
void static_checks() {
  ...
  static_assert(std::is_same<derived_view_type<cmyk8c_planar_step_view_t>::type,
                             cmyk8c_planar_step_view_t>::value,
                "");
  static_assert(
      std::is_same<derived_view_type<cmyk8c_planar_step_view_t, std::uint16_t,
                                     rgb_layout_t>::type,
