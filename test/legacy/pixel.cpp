...
struct do_basic_test : public C1, public C2 {
  ...
  void test_heterogeneous() {
    ...
    using is_swappable = std::integral_constant<
        bool, pixel_reference_is_mutable<p1_ref>::value &&
...
using representative_pixels_t =
    mp11::mp_list<value_core<gray8_pixel_t>, reference_core<gray16_pixel_t &>,
...
template <typename Pixel1> struct ccv2 {
  ...
  template <typename Pixel2> void operator()(Pixel2) {
    ...
    using pixel_model_t =
        typename std::remove_reference<typename Pixel2::type>::type;
    using p2_value_t = typename pixel_model_t::value_type;
    using pixel2_mutable =
        mp11::mp_if<p2_is_mutable, Pixel2, value_core<p2_value_t>>;
...
void test_packed_pixel() {
  using rgb565_pixel_t =
      packed_pixel_type<uint16_t, mp11::mp_list_c<unsigned, 5, 6, 5>,
                        rgb_layout_t>::type;
  ...
  using bgr121_ref_t = const bit_aligned_pixel_reference<
      std::uint8_t, mp11::mp_list_c<int, 1, 2, 1>, bgr_layout_t, true>;
  using rgb121_ref_t = const bit_aligned_pixel_reference<
      std::uint8_t, mp11::mp_list_c<int, 1, 2, 1>, rgb_layout_t, true>;
