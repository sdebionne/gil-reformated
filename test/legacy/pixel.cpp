...
struct increment {
  template <typename Incrementable> void operator()(Incrementable &x) const {
    ++x;
  }
};
...
// Construct with two pixel types. They must be compatible and the second must
// be mutable
...
struct do_basic_test : public C1, public C2 {
  ...
  void test_heterogeneous() {
    // Both must be pixel types (not necessarily pixel values). The second must
    // be mutable. They must be compatible
...
template <typename PixelValue, int Tag = 0> class value_core {
public:
...
public:
  ...
  reference_core(const typename pixel_t::value_type &val)
      : parent_t(val), _pixel(parent_t::_pixel) {
    boost::function_requires<PixelConcept<pixel_t>>();
  }
};

// Use a subset of pixel models that covers all color spaces, channel depths,
// reference/value, planar/interleaved, const/mutable color conversion will be
// invoked on pairs of them. Having an exhaustive binary check would be too
// big/expensive.
using representative_pixels_t =
    mpl::vector<value_core<gray8_pixel_t>, reference_core<gray16_pixel_t &>,
                value_core<bgr8_pixel_t>, reference_core<rgb8_planar_ref_t>,
...
template <typename Vector, typename Fun, int K> struct for_each_impl {
...
template <typename Pixel1> struct ccv2 {
  ...
  void color_convert_impl(const P1 &p1, P2 &p2) {
    color_convert_compatible(
        p1, p2, mpl::bool_<pixels_are_compatible<P1, P2>::value>());
  }

  template <typename Pixel2> void operator()(Pixel2) {
    ...
    using pixel2_mutable =
        typename mpl::if_c<p2_is_mutable, Pixel2, value_core<p2_value_t>>::type;
...
void test_color_convert() { for_each<representative_pixels_t>(ccv1()); }

void test_packed_pixel() {
  using rgb565_pixel_t =
      packed_pixel_type<uint16_t, mpl::vector3_c<unsigned, 5, 6, 5>,
                        rgb_layout_t>::type;
  ...
  // Convert all channels of the unpacked pixel to the packed one & ensure the
  // packed one is full
  get_color(r565, red_t()) =
      channel_convert<kth_element_type<rgb565_pixel_t, 0>::type>(
          get_color(rgb_full, red_t()));
  get_color(r565, green_t()) =
      channel_convert<kth_element_type<rgb565_pixel_t, 1>::type>(
          get_color(rgb_full, green_t()));
  get_color(r565, blue_t()) =
      channel_convert<kth_element_type<rgb565_pixel_t, 2>::type>(
          get_color(rgb_full, blue_t()));
  error_if(r565 != rgb565_pixel_t((uint16_t)65535));

  // rgb565 is compatible with bgr556. Test interoperability
  boost::function_requires<
      PixelsCompatibleConcept<rgb565_pixel_t, bgr556_pixel_t>>();

  do_basic_test<value_core<rgb565_pixel_t, 0>, value_core<bgr556_pixel_t, 1>>(
      r565)
      .test_heterogeneous();
  ...
  using rgb121_ref_t = const bit_aligned_pixel_reference<
      std::uint8_t, boost::mpl::vector3_c<int, 1, 2, 1>, rgb_layout_t, true>;
  using rgb121_pixel_t = rgb121_ref_t::value_type;
  rgb121_pixel_t p121;
  do_basic_test<reference_core<bgr121_ref_t, 0>,
                reference_core<rgb121_ref_t, 1>>(p121)
      .test_heterogeneous();
  do_basic_test<value_core<rgb121_pixel_t, 0>, reference_core<rgb121_ref_t, 1>>(
      p121)
      .test_heterogeneous();
...
void test_pixel() {
  ...
  do_basic_test<value_core<rgb8_pixel_t, 0>, reference_core<rgb8_pixel_t &, 1>>(
      rgb8)
      .test_all();
  do_basic_test<value_core<bgr8_pixel_t, 0>,
                reference_core<rgb8_planar_ref_t, 1>>(rgb8)
      .test_all();
  do_basic_test<reference_core<rgb8_planar_ref_t, 0>,
                reference_core<bgr8_pixel_t &, 1>>(rgb8)
      .test_all();
  do_basic_test<reference_core<const rgb8_pixel_t &, 0>,
                reference_core<rgb8_pixel_t &, 1>>(rgb8)
      .test_all();
