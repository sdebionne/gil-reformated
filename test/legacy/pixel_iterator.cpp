...
//
#ifdef _MSC_VER
#pragma warning(disable : 4244) // 'argument': conversion from 'int' to
...
void test_pixel_iterator() {
  ...
  boost::function_requires<
      MutableStepIteratorConcept<cmyk8_planar_step_ptr_t>>();
  ...
  using bgr121_ref_t =
      bit_aligned_pixel_reference<std::uint8_t,
  ...
  gil_function_requires<PixelDereferenceAdaptorConcept<
      deref_compose<ccv_rgb_g_fn, ccv_g_rgb_fn>>>();
  ...
  // BGR232 pixel value. It is a packed_pixel of size 1 byte. (The last bit is
  // unused)
...
void test_pixel_iterator() {
    ...
    const rgb8_pixel_t& ref=memunit_advanced_ref(ptr1,10);
...
//  bool v=std::is_pod<iterator_traits<memory_based_step_iterator<rgb8_ptr_t>
>::value_type>::value;
...
//    invert_pixel1(*r8cpp);
    ...
    *(pix_img_it+3)=rgb8_pixel_t(1,2,3);
    ...
    iterator_from_2d<rgb8c_loc_t> pix_img_it_c(rgb8c_loc_t(rgb8c_ptr_t(ptr),20),
5);
    ++pix_img_it_c;
    pix_img_it_c+=10;
    //  *pix_img_it_c=rgb8_pixel_t(1,2,3);        // error: assigning though
const iterator using dif_t = iterator_from_2d<rgb8_loc_t>::difference_type;
    dif_t dt=0;
...
//  using RGB8ConstPlanarPtr = iterator_traits<rgb8_planar_ptr_t>::pointer
const;
...
bit depth
    ...
    //  iterator_traits<CMYK8>::pointer cmyk8_ptr_t=&rgb8;    // error:
incompatible pointer type

    RGB8ConstPtr rgb8_const_ptr_err=rgb8ptr;        // const pointer from
non-regular pointer ignore_unused_variable_warning(rgb8_const_ptr_err);
// dereferencing pointers to obtain references
    ...
    //  RGB8Ref rgb8ref_2_err=*rgb8_const_planar_ptr;   // error: non-const
reference from const pointer

    rgb8_planar_ref_t rgb8planarref_3=*rgb8_pptr; // planar reference from
