...
public:
  ...
  interleaved_ptr(const interleaved_ptr<CP, Layout> &ptr)
      : _channels(ptr._channels) {}
  ...
  /// custom class that is convertible to reference We require our own reference
...
// through the channel traits, which take a model of channel So we can get a
// model of channel from the channel pointer via iterator_traits. Notice that we
// take the iterator_traits::reference and not iterator_traits::value_type. This
// is because sometimes multiple reference and pointer types share the same
// value type. An example of this is GIL's planar reference and iterator
// ("planar_pixel_reference" and "planar_pixel_iterator") which share the class
// "pixel" as the value_type. The class "pixel" is also the value type for
// interleaved pixel references. Here we are dealing with channels, not pixels,
...
memunit_step(const interleaved_ptr<ChannelPtr, Layout> &) {
  return sizeof(typename std::iterator_traits<
                ChannelPtr>::value_type) * // size of each channel in bytes
         interleaved_ptr<ChannelPtr,
                         Layout>::num_channels; // times the number of channels
}

template <typename ChannelPtr, typename Layout>
inline std::ptrdiff_t
memunit_distance(const interleaved_ptr<ChannelPtr, Layout> &p1,
                 const interleaved_ptr<ChannelPtr, Layout> &p2) {
  return memunit_distance(p1.channels(), p2.channels());
}

template <typename ChannelPtr, typename Layout>
inline void memunit_advance(interleaved_ptr<ChannelPtr, Layout> &p,
                            std::ptrdiff_t diff) {
  memunit_advance(p.channels(), diff);
}

template <typename ChannelPtr, typename Layout>
inline interleaved_ptr<ChannelPtr, Layout>
memunit_advanced(const interleaved_ptr<ChannelPtr, Layout> &p,
                 std::ptrdiff_t diff) {
...
memunit_advanced_ref(const interleaved_ptr<ChannelPtr, Layout> &p,
                     std::ptrdiff_t diff) {
  interleaved_ptr<ChannelPtr, Layout> ret = p;
  memunit_advance(ret, diff);
  return *ret;
