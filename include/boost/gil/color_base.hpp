...
/// HomogeneousColorBaseConcept or HomogeneousColorBaseValueConcept If the
...
struct homogeneous_color_base_impl {
  ...
  template <int K>
  struct kth_element_reference_type : public add_reference<Element> {};
  template <int K>
...
struct homogeneous_color_base<Element, Layout, 1>
    : public homogeneous_color_base_impl<Element, Layout> {
private:
  Element _v0;
...
public:
  ...
  homogeneous_color_base(P *p, bool)
      : _v0(&semantic_at_c<0>(*p)), _v1(&semantic_at_c<1>(*p)) {}
...
/// \brief A homogeneous color base holding three color elements. Models
/// HomogeneousColorBaseConcept or HomogeneousColorBaseValueConcept \ingroup
/// ColorBaseModelHomogeneous
template <typename Element, typename Layout>
...
/// \brief A homogeneous color base holding four color elements. Models
/// HomogeneousColorBaseConcept or HomogeneousColorBaseValueConcept \ingroup
/// ColorBaseModelHomogeneous
template <typename Element, typename Layout>
...
public:
  ...
  homogeneous_color_base(P *p, bool)
      : _v0(&semantic_at_c<0>(*p)), _v1(&semantic_at_c<1>(*p)),
        _v2(&semantic_at_c<2>(*p)), _v3(&semantic_at_c<3>(*p)) {}
...
/// \brief A homogeneous color base holding five color elements. Models
/// HomogeneousColorBaseConcept or HomogeneousColorBaseValueConcept \ingroup
/// ColorBaseModelHomogeneous
template <typename Element, typename Layout>
...
// -- there is no guarantee that the compiler won't add any padding between
// adjacent channels. Note, however, that GIL _must_ be compiled with compiler
// settings ensuring there is no padding in the color base structs. This is
// because the color base structs must model the interleaved organization in
// memory. In other words, the client may have existing RGB image in the form
// "RGBRGBRGB..." and we must be able to represent it with an array of RGB color
// bases (i.e. RGB pixels) with no padding. We have tested with
// char/int/float/double channels on gcc and VC and have so far discovered no
// problem. We have even tried using strange channels consisting of short + char
// (3 bytes). With the default 4-byte alignment on VC, the size of this channel
// is padded to 4 bytes, so an RGB pixel of it will be 4x3=12 bytes. The code
// below will still work properly. However, the client must nevertheless ensure
// that proper compiler settings are used for their compiler and their channel
// types.
