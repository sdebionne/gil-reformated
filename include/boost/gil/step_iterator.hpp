...
/// \brief Iterators that allow for specifying the step between two adjacent
/// values
...
/// operators ++, --, +=, -=, etc. the adaptor is using the passed policy object
/// SFn for advancing and for computing the distance between iterators.

template <typename Derived,  // type of the derived class
          typename Iterator, // Models Iterator
          typename SFn> // A policy object that can compute the distance between
                        // two iterators of type Iterator and can advance an
                        // iterator of type Iterator a given number of
                        // Iterator's units
...
// although iterator_adaptor defines these, the default implementation computes
// distance and compares for zero. it is often faster to just apply the relation
// operator to the base
template <typename D, typename Iterator, typename SFn>
inline bool operator>(const step_iterator_adaptor<D, Iterator, SFn> &p1,
...
/// \brief Iterator with dynamically specified step in bytes. Models
/// StepIteratorConcept, IteratorAdaptorConcept, ByteAdvanceableIteratorConcept,
/// PixelIteratorConcept, HasDynamicXStepTypeConcept
///
/// A refinement of step_iterator_adaptor that uses a dynamic parameter for the
/// step which is specified in bytes
///
...
///
/// Another application is as a sub-channel view. For example, a red intensity
/// image over interleaved RGB data would use a step iterator adaptor with step
/// sizeof(channel_t)*3 In the latter example the step size could be fixed at
/// compile time for efficiency. Compile-time fixed step can be implemented by
/// providing a step function object that takes the step as a template
////////////////////////////////////////////////////////////////////////////////////////

/// \ingroup PixelIteratorModelStepPtr
/// \brief function object that returns the byte distance between two iterators
/// and advances a given iterator a given number of bytes
template <typename Iterator> struct byte_step_fn {
  ...
  difference_type difference(const Iterator &it1, const Iterator &it2) const {
    return byte_distance(it1, it2) / _step;
  }
  void advance(Iterator &it, difference_type d) const {
    byte_advance(it, d * _step);
  }
...
class byte_addressable_step_iterator
    : public detail::step_iterator_adaptor<
          byte_addressable_step_iterator<Iterator>, Iterator,
...
public:
  ...
  byte_addressable_step_iterator(Iterator it, std::ptrdiff_t byte_step)
      : parent_t(it, byte_step_fn<Iterator>(byte_step)) {}
  ...
  /// For some reason operator[] provided by iterator_adaptor returns a custom
  ...
  void set_step(std::ptrdiff_t byte_step) {
    this->_step_fn.set_step(byte_step);
  }
...
template <typename Iterator>
struct iterator_is_mutable<byte_addressable_step_iterator<Iterator>>
...
template <typename Iterator>
struct is_iterator_adaptor<byte_addressable_step_iterator<Iterator>>
    : public mpl::true_ {};
...
struct iterator_adaptor_rebind<byte_addressable_step_iterator<Iterator>,
                               NewBaseIterator> {
...
template <typename Iterator>
struct color_space_type<byte_addressable_step_iterator<Iterator>>
    : public color_space_type<Iterator> {};

template <typename Iterator>
struct channel_mapping_type<byte_addressable_step_iterator<Iterator>>
    : public channel_mapping_type<Iterator> {};

template <typename Iterator>
struct is_planar<byte_addressable_step_iterator<Iterator>>
    : public is_planar<Iterator> {};

template <typename Iterator>
struct channel_type<byte_addressable_step_iterator<Iterator>>
...
byte_step(const byte_addressable_step_iterator<Iterator> &p) {
  return p.step();
}

template <typename Iterator>
inline std::ptrdiff_t
byte_distance(const byte_addressable_step_iterator<Iterator> &p1,
...
byte_advanced(const byte_addressable_step_iterator<Iterator> &p,
              std::ptrdiff_t byteDiff) {
  return byte_addressable_step_iterator<Iterator>(
      byte_advanced(p.base(), byteDiff), p.step());
}
...
struct iterator_add_deref<byte_addressable_step_iterator<Iterator>, Deref> {
  ...
  static type make(const byte_addressable_step_iterator<Iterator> &it,
                   const Deref &d) {
    return type(iterator_add_deref<Iterator, Deref>::make(it.base(), d),
                it.step());
  }
};
...
template <typename I>
typename dynamic_x_step_type<I>::type make_step_iterator(const I &it,
                                                         std::ptrdiff_t step);

namespace detail {

// if the iterator is a plain base iterator (non-adaptor), wraps it in
// byte_addressable_step_iterator
template <typename I>
...
byte_addressable_step_iterator<BaseIt>
make_step_iterator_impl(const byte_addressable_step_iterator<BaseIt> &it,
...
///
/// To construct a step iterator from a given iterator Iterator and a given
/// step, if Iterator does not already have a dynamic step, we wrap it in a
/// byte_addressable_step_iterator. Otherwise we do a compile-time traversal of
/// the chain of iterator adaptors to locate the step iterator and then set it
/// step to the new one.
///
/// The step iterator of Iterator is not always
/// byte_addressable_step_iterator<Iterator>. For example, Iterator may already
/// be a byte_addressable_step_iterator, in which case it will be inefficient to
/// stack them; we can obtain the same result by multiplying their steps. Note
/// that for Iterator to be a step iterator it does not necessarily have to have
/// the form byte_addressable_step_iterator<J>. The step iterator can be wrapped
/// inside another iterator. Also, it may not have the type
/// byte_addressable_step_iterator, but it could be a user-provided type.
template <typename I> // Models ByteAdvanceableIteratorConcept,
                      // HasDynamicXStepTypeConcept
typename dynamic_x_step_type<I>::type make_step_iterator(const I &it,
                                                         std::ptrdiff_t step) {
