...
/// \brief Iterator with dynamically specified step in memory units (bytes or
/// bits). Models StepIteratorConcept, IteratorAdaptorConcept,
/// MemoryBasedIteratorConcept, PixelIteratorConcept, HasDynamicXStepTypeConcept
///
/// A refinement of step_iterator_adaptor that uses a dynamic parameter for the
/// step which is specified in memory units, such as bytes or bits
///
...
/// \ingroup PixelIteratorModelStepPtr
/// \brief function object that returns the memory unit distance between two
...
memunit_step(const memory_based_step_iterator<Iterator> &p) {
  return p.step();
}

template <typename Iterator>
inline std::ptrdiff_t
memunit_distance(const memory_based_step_iterator<Iterator> &p1,
...
memunit_advanced(const memory_based_step_iterator<Iterator> &p,
                 std::ptrdiff_t diff) {
  return memory_based_step_iterator<Iterator>(memunit_advanced(p.base(), diff),
                                              p.step());
}
...
/// memory_based_step_iterator. Otherwise we do a compile-time traversal of the
/// chain of iterator adaptors to locate the step iterator and then set it step
/// to the new one.
///
/// The step iterator of Iterator is not always
/// memory_based_step_iterator<Iterator>. For example, Iterator may already be a
/// memory_based_step_iterator, in which case it will be inefficient to stack
/// them; we can obtain the same result by multiplying their steps. Note that
/// for Iterator to be a step iterator it does not necessarily have to have the
/// form memory_based_step_iterator<J>. The step iterator can be wrapped inside
/// another iterator. Also, it may not have the type memory_based_step_iterator,
/// but it could be a user-provided type.
template <typename I> // Models MemoryBasedIteratorConcept,
                      // HasDynamicXStepTypeConcept
                      typename dynamic_x_step_type<I>::type
