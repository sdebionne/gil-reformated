...
typename heterogeneous_packed_pixel<
    P, C, L>::template kth_element_reference_type<K>::type
at_c(heterogeneous_packed_pixel<P, C, L> &p);

template <int K, typename P, typename C, typename L>
typename heterogeneous_packed_pixel<
    P, C, L>::template kth_element_const_reference_type<K>::type
...
struct remove_const_and_reference
    : public remove_reference<typename remove_const<T>::type> {};
...
template <typename T> struct Assignable {
  void constraints() { function_requires<boost::AssignableConcept<T>>(); }
};
/// \ingroup BasicConcepts
/**
\code
...
/**
\code
auto concept Regular<typename T> : DefaultConstructible<T>,
CopyConstructible<T>, EqualityComparable<T>, Assignable<T>, Swappable<T> {};
\endcode
*/
...
template <typename P> struct PointNDConcept {
  void constraints() {
...
//
// Taken from boost's concept_check.hpp. Isolating mutability to result in
// faster compile time
//
////////////////////////////////////////////////////////////////////////////////////////

namespace detail {
template <class TT> // Preconditions: TT Models
...
\endcode
*/
template <typename Cs> struct ColorSpaceConcept {
  void constraints() {
    // An MPL Random Access Sequence, whose elements are color tags
  }
};

template <typename ColorSpace1,
          typename ColorSpace2> // Models ColorSpaceConcept
...
/**
\code
concept ColorSpacesCompatibleConcept<ColorSpaceConcept Cs1, ColorSpaceConcept
Cs2> { where SameType<Cs1,Cs2>;
};
\endcode
*/
...
concept ChannelMappingConcept<MPLRandomAccessSequence CM> {
   // An MPL Random Access Sequence, whose elements model MPLIntegralConstant
representing a permutation
};
\endcode
*/
template <typename CM> struct ChannelMappingConcept {
  void constraints() {
    // An MPL Random Access Sequence, whose elements model MPLIntegralConstant
    // representing a permutation
  }
...
/// \brief A channel is the building block of a color. Color is defined as a
/// mixture of primary colors and a channel defines the degree to which each
/// primary color is used in the mixture.
/**
For example, in the RGB color space, using 8-bit unsigned channels, the color
red is defined as [255 0 0], which means maximum of Red, and no Green and Blue.

Built-in scalar types, such as \p int and \p float, are valid GIL channels. In
more complex scenarios, channels may be represented as bit ranges or even
individual bits. In such cases special classes are needed to represent the value
and reference to a channel.

Channels have a traits class, \p channel_traits, which defines their associated
types as well as their operating ranges.

\code
concept ChannelConcept<typename T> : EqualityComparable<T> {
    typename value_type      = T;        // use channel_traits<T>::value_type to
access it typename reference       = T&;       // use
channel_traits<T>::reference to access it typename pointer         = T*; // use
channel_traits<T>::pointer to access it typename const_reference = const T&; //
use channel_traits<T>::const_reference to access it typename const_pointer   =
const T*; // use channel_traits<T>::const_pointer to access it static const bool
is_mutable;        // use channel_traits<T>::is_mutable to access it

    static T min_value();                // use channel_traits<T>::min_value to
access it static T max_value();                // use
...
{}; \endcode
*/
template <typename T> struct MutableChannelConcept {
  void constraints() {
...
\endcode
*/
template <typename T> struct ChannelValueConcept {
  void constraints() {
    gil_function_requires<ChannelConcept<T>>();
...
///
/// Channels are considered compatible if their value types (ignoring constness
/// and references) are the same \ingroup ChannelModel
...
/// constness and references) are the same \ingroup ChannelConcept
/**
\code
...
\endcode
*/
template <typename T1, typename T2> struct ChannelsCompatibleConcept {
  void constraints() {
    BOOST_STATIC_ASSERT((channels_are_compatible<T1, T2>::value));
  }
};

/// \brief A channel is convertible to another one if the \p channel_convert
...
/**
\code
concept ChannelConvertibleConcept<ChannelConcept SrcChannel, ChannelValueConcept
DstChannel> { DstChannel channel_convert(const SrcChannel&);
};
\endcode
*/
...
/// \brief A color base is a container of color elements (such as channels,
/// channel references or channel pointers)
/**
The most common use of color base is in the implementation of a pixel, in which
case the color elements are channel values. The color base concept, however, can
be used in other scenarios. For example, a planar pixel has channels that are
not contiguous in memory. Its reference is a proxy class that uses a color base
whose elements are channel references. Its iterator uses a color base whose
elements are channel iterators.

A color base must have an associated layout (which consists of a color space, as
well as an ordering of the channels). There are two ways to index the elements
of a color base: A physical index corresponds to the way they are ordered in
memory, and a semantic index corresponds to the way the elements are ordered in
their color space. For example, in the RGB color space the elements are ordered
as {red_t, green_t, blue_t}. For a color base with a BGR layout, the first
element in physical ordering is the blue element, whereas the first semantic
element is the red one. Models of \p ColorBaseConcept are required to provide
the \p at_c<K>(ColorBase) function, which allows for accessing the elements
based on their physical order. GIL provides a \p semantic_at_c<K>(ColorBase)
function (described later) which can operate on any model of ColorBaseConcept
and returns the corresponding semantic element.

\code
concept ColorBaseConcept<typename T> : CopyConstructible<T>,
EqualityComparable<T> {
    // a GIL layout (the color space and element permutation)
    typename layout_t;

    // The type of K-th element
    template <int K> struct kth_element_type;                 where
Metafunction<kth_element_type>;

    // The result of at_c
    template <int K> struct kth_element_const_reference_type; where
Metafunction<kth_element_const_reference_type>;

    template <int K> kth_element_const_reference_type<T,K>::type at_c(T);

    // Copy-constructible and equality comparable with other compatible color
bases template <ColorBaseConcept T2> where { ColorBasesCompatibleConcept<T,T2> }
        T::T(T2);
...
template <typename ColorBase> struct ColorBaseConcept {
  void constraints() {
    ...
    typedef
        typename ColorBase::template kth_element_type<num_elements - 1>::type
            TN;
    typedef typename ColorBase::template kth_element_const_reference_type<
    ...
    // also static_max(cb), static_min(cb), static_fill(cb,value), and all
    // variations of static_for_each(), static_generate(), static_transform()
  }
...
\code
concept MutableColorBaseConcept<ColorBaseConcept T> : Assignable<T>,
Swappable<T> { template <int K> struct kth_element_reference_type;       where
Metafunction<kth_element_reference_type>;

    template <int K> kth_element_reference_type<kth_element_type<K>::type>::type
at_c(T);
...
\endcode
*/
template <typename ColorBase> struct MutableColorBaseConcept {
  void constraints() {
...
/**
\code
concept ColorBaseValueConcept<typename T> : MutableColorBaseConcept<T>,
Regular<T> {
};
\endcode
*/
template <typename ColorBase> struct ColorBaseValueConcept {
  void constraints() {
...
concept HomogeneousColorBaseConcept<ColorBaseConcept CB> {
    ...
    //     where SameType<kth_element_type<K>::type,
kth_element_type<K+1>::type>; kth_element_const_reference_type<0>::type
dynamic_at_c(const CB&, std::size_t n) const;
};
\endcode
*/

template <typename ColorBase> struct HomogeneousColorBaseConcept {
  void constraints() {
    gil_function_requires<ColorBaseConcept<ColorBase>>();
    ...
    typedef
        typename ColorBase::template kth_element_const_reference_type<0>::type
            CR0;
...
HomogeneousColorBaseConcept<CB> { kth_element_reference_type<0>::type
dynamic_at_c(CB&, std::size_t n);
};
\endcode
*/

template <typename ColorBase> struct MutableHomogeneousColorBaseConcept {
  void constraints() {
    gil_function_requires<ColorBaseConcept<ColorBase>>();
...
/// \ingroup ColorBaseConcept
/// \brief Homogeneous color base that also has a default constructor. Refines
/// Regular.
/**
...
template <typename ColorBase> struct HomogeneousColorBaseValueConcept {
  void constraints() {
...
/// \ingroup ColorBaseConcept
/// \brief Two color bases are compatible if they have the same color space and
/// their elements are compatible, semantic-pairwise.
/**
...
/// iterators, locators, views and images whose value type is a pixel \ingroup
/// PixelBasedConcept
/**
\code
...
template <typename P> struct PixelConcept {
  void constraints() {
    ...
    gil_function_requires<
        PixelConcept<typename remove_const_and_reference<reference>::type>>();
...
HomogeneousColorBaseConcept<P>, HomogeneousPixelBasedConcept<P> { P::template
element_const_reference_type<P>::type operator[](P p, std::size_t i) const {
return dynamic_at_c(p,i); }
};
\endcode
*/
template <typename P> struct HomogeneousPixelConcept {
  void constraints() {
    gil_function_requires<PixelConcept<P>>();
...
MutableHomogeneousColorBaseConcept<P> { P::template
element_reference_type<P>::type operator[](P p, std::size_t i) { return
dynamic_at_c(p,i); }
};
\endcode
*/
template <typename P> struct MutableHomogeneousPixelConcept {
  void constraints() {
...
\endcode
*/
template <typename P> struct PixelValueConcept {
  void constraints() {
    gil_function_requires<PixelConcept<P>>();
    gil_function_requires<Regular<P>>();
  }
...
\endcode
*/
template <typename P> struct HomogeneousPixelValueConcept {
  void constraints() {
    gil_function_requires<HomogeneousPixelConcept<P>>();
    gil_function_requires<Regular<P>>();
...
///
/// Pixels are compatible if their channels and color space types are
/// compatible. Compatible pixels can be assigned and copy constructed from one
/// another. \ingroup PixelModel
...
///    Pixels are compatible if their channels and color space types are
///    compatible. Compatible pixels can be assigned and copy constructed from
///    one another.
/// \ingroup PixelConcept
/**
\code
...
ColorBasesCompatibleConcept<P1,P2> {
    ...
    //    ChannelsCompatibleConcept<kth_semantic_element_type<P1,K>::type,
kth_semantic_element_type<P2,K>::type>;
};
\endcode
*/
template <typename P1,
          typename P2> // precondition: P1 and P2 model PixelConcept
...
///
/// Convertibility is non-symmetric and implies that one pixel can be converted
/// to another, approximating the color. Conversion is explicit and sometimes
/// lossy. \ingroup PixelConcept
/**
\code
...
\endcode
*/
template <typename SrcP, typename DstP> struct PixelConvertibleConcept {
  void constraints() {
...
/// dereferencing a pixel iterator.
///
/// This can perform an arbitrary computation, such as color conversion or table
/// lookup
/**
\code
...
AssignableConcept<D>  { typename const_t;         where
PixelDereferenceAdaptorConcept<const_t>; typename value_type;      where
PixelValueConcept<value_type>; typename reference;         // may be mutable
...
template <typename D> struct PixelDereferenceAdaptorConcept {
  void constraints() {
    gil_function_requires<boost::UnaryFunctionConcept<
        D, typename remove_const_and_reference<typename D::result_type>::type,
    ...
    typedef typename D::reference
        reference; // == PixelConcept (if you remove const and reference)
    typedef typename D::const_reference
        const_reference; // == PixelConcept (if you remove const and reference)
...
/// \brief Concept for iterators, locators and views that can define a type just
/// like the given iterator/locator/view, except it supports runtime specified
/// step along the X navigation \ingroup PixelIteratorConcept
/**
\code
...
/// \brief Concept for locators and views that can define a type just like the
/// given locator or view, except it supports runtime specified step along the Y
/// navigation \ingroup PixelLocatorConcept
/**
\code
...
/// \brief Concept for locators and views that can define a type just like the
/// given locator or view, except X and Y is swapped \ingroup
/// PixelLocatorConcept
/**
\code
...
GIL's iterators must also provide the following metafunctions:
 - \p const_iterator_type<Iterator>:   Returns a read-only equivalent of \p
Iterator
 - \p iterator_is_mutable<Iterator>:   Returns whether the given iterator is
read-only or mutable
 - \p is_iterator_adaptor<Iterator>:   Returns whether the given iterator is an
adaptor over another iterator. See IteratorAdaptorConcept for additional
...
PixelBasedConcept<Iterator> { where PixelValueConcept<value_type>; typename
const_iterator_type<It>::type; where
PixelIteratorConcept<const_iterator_type<It>::type>; static const bool
iterator_is_mutable<It>::type::value; static const bool
is_iterator_adaptor<It>::type::value;   // is it an iterator adaptor
};
\endcode
*/
template <typename Iterator> struct PixelIteratorConcept {
  void constraints() {
    gil_function_requires<
        boost_concepts::RandomAccessTraversalConcept<Iterator>>();
    ...
    ignore_unused_variable_warning(
        const_it); // immutable iterator must be constructible from (possibly
                   // mutable) iterator
...
namespace detail {
template <typename Iterator> // Preconditions: Iterator Models
...
\endcode
*/
template <typename Iterator> struct MutablePixelIteratorConcept {
  void constraints() {
...
namespace detail {
// Iterators that can be used as the base of byte_addressable_step_iterator
// require some additional functions
template <
    typename Iterator> // Preconditions: Iterator Models
                       // boost_concepts::RandomAccessTraversalConcept
                       struct RandomAccessIteratorIsByteAdvanceableConcept {
  void constraints() {
    std::ptrdiff_t bs = byte_step(it);
    ignore_unused_variable_warning(bs);
    it = byte_advanced(it, 3);
    std::ptrdiff_t bd = byte_distance(it, it);
    ignore_unused_variable_warning(bd);
    byte_advance(it, 3);
    // for performace you may also provide a customized implementation of
    // byte_advanced_ref
  }
  Iterator it;
};
...
\code
concept
ByteAdvanceableIteratorConcept<boost_concepts::RandomAccessTraversalConcept
Iterator> { std::ptrdiff_t      byte_step(const Iterator&); std::ptrdiff_t
byte_distance(const Iterator& , const Iterator&); void byte_advance(Iterator&,
std::ptrdiff_t byteDiff); Iterator            byte_advanced(const Iterator& p,
std::ptrdiff_t byteDiff) { Iterator tmp; byte_advance(tmp,pyteDiff); return tmp;
} Iterator::reference byte_advanced_ref(const Iterator& p, std::ptrdiff_t
byteDiff) { return *byte_advanced(p,byteDiff); }
};
\endcode
*/
template <typename Iterator> struct ByteAdvanceableIteratorConcept {
  void constraints() {
    gil_function_requires<
        boost_concepts::RandomAccessTraversalConcept<Iterator>>();
    gil_function_requires<
        detail::RandomAccessIteratorIsByteAdvanceableConcept<Iterator>>();
  }
};
...
\endcode
*/
template <typename Iterator> struct StepIteratorConcept {
  void constraints() {
...
StepIteratorConcept<Iterator> {}; \endcode
*/
template <typename Iterator> struct MutableStepIteratorConcept {
  void constraints() {
...
/// \brief Iterator adaptor is a forward iterator adapting another forward
/// iterator.
/**
In addition to GIL iterator requirements, GIL iterator adaptors must provide the
following metafunctions:
 ...
 - \p iterator_adaptor_rebind<Iterator,NewBase>: Replaces the base iterator with
the new one
...
{ where SameType<is_iterator_adaptor<Iterator>::type, mpl::true_>;
    ...
    typename iterator_adaptor_get_base<Iterator>;
        ...
        where
boost_concepts::ForwardTraversalConcept<iterator_adaptor_get_base<Iterator>::type>;

    typename another_iterator;
    typename iterator_adaptor_rebind<Iterator,another_iterator>::type;
        where boost_concepts::ForwardTraversalConcept<another_iterator>;
        where
IteratorAdaptorConcept<iterator_adaptor_rebind<Iterator,another_iterator>::type>;

    const iterator_adaptor_get_base<Iterator>::type& Iterator::base() const;
};
\endcode
*/
template <typename Iterator> struct IteratorAdaptorConcept {
  void constraints() {
    gil_function_requires<boost_concepts::ForwardTraversalConcept<Iterator>>();
...
IteratorAdaptorConcept<Iterator> {}; \endcode
*/
template <typename Iterator> struct MutableIteratorAdaptorConcept {
  void constraints() {
...
concept RandomAccessNDLocatorConcept<Regular Loc> {
    ...
    typename difference_type; where PointNDConcept<difference_type>; // return
value of operator-. typename const_t;           // same as Loc, but operating
over immutable values typename cached_location_t; // type to store relative
location (for efficient repeated access) typename point_t  = difference_type;

    static const size_t num_dimensions; // dimensionality of the locator
    where num_dimensions = point_t::num_dimensions;

    // The difference_type and iterator type along each dimension. The iterators
may only differ in
...
iterator along D-th axis. where iterator::value_type == value_type;
    ...
    // Defines the type of a locator similar to this type, except it invokes
Deref upon dereferencing template <PixelDereferenceAdaptorConcept Deref> struct
add_deref { typename type;        where RandomAccessNDLocatorConcept<type>;
    ...
    // Accessing iterators along a given dimension at the current location or at
a given offset template <size_t D> axis<D>::iterator& Loc::axis_iterator();
    template <size_t D> axis<D>::iterator const& Loc::axis_iterator() const;
    template <size_t D> axis<D>::iterator        Loc::axis_iterator(const
difference_type&) const;
};
\endcode
*/
template <typename Loc> struct RandomAccessNDLocatorConcept {
  void constraints() {
...
\endcode
*/
template <typename Loc> struct RandomAccess2DLocatorConcept {
  void constraints() {
...
\endcode
*/
template <typename Loc> struct PixelLocatorConcept {
  void constraints() {
...
\endcode
*/
template <typename Loc> struct MutableRandomAccessNDLocatorConcept {
  void constraints() {
    gil_function_requires<RandomAccessNDLocatorConcept<Loc>>();
    gil_function_requires<detail::RandomAccessNDLocatorIsMutableConcept<Loc>>();
  }
...
MutableRandomAccessNDLocatorConcept<Loc> {}; \endcode
*/
template <typename Loc> struct MutableRandomAccess2DLocatorConcept {
  void constraints() {
    gil_function_requires<RandomAccess2DLocatorConcept<Loc>>();
...
MutableRandomAccess2DLocatorConcept<Loc> {}; \endcode
*/
template <typename Loc> struct MutablePixelLocatorConcept {
  void constraints() {
...
concept RandomAccessNDImageViewConcept<Regular View> {
    typename value_type;
    typename reference;       // result of dereferencing
    typename difference_type; // result of operator-(iterator,iterator)
(1-dimensional!) typename const_t;  where RandomAccessNDImageViewConcept<View>;
// same as View, but over immutable values typename point_t;  where
PointNDConcept<point_t>; // N-dimensional point typename locator;  where
RandomAccessNDLocatorConcept<locator>; // N-dimensional locator. typename
iterator; where RandomAccessTraversalConcept<iterator>; // 1-dimensional
iterator over all values typename reverse_iterator; where
RandomAccessTraversalConcept<reverse_iterator>; typename size_type;       // the
return value of size()
    ...
    template <size_t D> struct axis {
        typename coord_t = point_t::axis<D>::coord_t;
        typename iterator; where RandomAccessTraversalConcept<iterator>;   //
iterator along D-th axis. where SameType<coord_t, iterator::difference_type>;
        where SameType<iterator::value_type,value_type>;
    };

    // Defines the type of a view similar to this type, except it invokes Deref
upon dereferencing template <PixelDereferenceAdaptorConcept Deref> struct
add_deref { typename type;        where RandomAccessNDImageViewConcept<type>;
...
1-dimensional reference iterator         View::begin()      const; iterator
View::end()        const; reverse_iterator View::rbegin()     const;
    reverse_iterator View::rend()       const;
    iterator         View::at(const point_t&);
    point_t          View::dimensions() const; // number of elements along each
dimension bool             View::is_1d_traversable() const;   // can an iterator
over the first dimension visit each value? I.e. are there gaps between values?

    // iterator along a given dimension starting at a given point
    template <size_t D> View::axis<D>::iterator View::axis_iterator(const
point_t&) const;
...
\endcode
*/
template <typename View> struct RandomAccessNDImageViewConcept {
  void constraints() {
    ...
    //        BOOST_STATIC_ASSERT((typename std::iterator_traits<
    //        last_it_type>::difference_type, typename point_t::template
    //        axis<N-1>::coord_t>::value));
...
\endcode
*/
template <typename View> struct ImageViewConcept {
  void constraints() {
    gil_function_requires<RandomAccess2DImageViewConcept<View>>();

    // TODO: This executes the requirements for RandomAccess2DLocatorConcept
    // again. Fix it to improve compile time
...
namespace detail {
template <typename View> // Preconditions: View Models
                         // RandomAccessNDImageViewConcept
                         struct RandomAccessNDImageViewIsMutableConcept {
  void constraints() {
    gil_function_requires<detail::RandomAccessNDLocatorIsMutableConcept<
...
struct PixelImageViewIsMutableConcept {
  void constraints() {
    gil_function_requires<
        detail::RandomAccess2DImageViewIsMutableConcept<View>>();
  }
};
} // namespace detail
...
/**
\code
concept MutableRandomAccessNDImageViewConcept<RandomAccessNDImageViewConcept
View> { where Mutable<reference>;
};
\endcode
*/
template <typename View> struct MutableRandomAccessNDImageViewConcept {
  void constraints() {
    gil_function_requires<RandomAccessNDImageViewConcept<View>>();
    gil_function_requires<
        detail::RandomAccessNDImageViewIsMutableConcept<View>>();
...
/**
\code
concept MutableRandomAccess2DImageViewConcept<RandomAccess2DImageViewConcept
View> : MutableRandomAccessNDImageViewConcept<View> {}; \endcode
*/
template <typename View> struct MutableRandomAccess2DImageViewConcept {
  void constraints() {
    gil_function_requires<RandomAccess2DImageViewConcept<View>>();
    gil_function_requires<
        detail::RandomAccess2DImageViewIsMutableConcept<View>>();
...
MutableRandomAccess2DImageViewConcept<View> {}; \endcode
*/
template <typename View> struct MutableImageViewConcept {
  void constraints() {
...
///
/// Views are compatible if their pixels are compatible. Compatible views can be
/// assigned and copy constructed from one another.
...
/// compatibility \ingroup ImageViewConcept
/**
\code
...
concept RandomAccessNDImageConcept<typename Img> : Regular<Img> {
    ...
    void Img::recreate(point_t new_dims, value_type fill_value, std::size_t
alignment);
...
concept RandomAccess2DImageConcept<RandomAccessNDImageConcept Img> {
    ...
    Img::Img(x_coord_t width, y_coord_t height, value_type fill_value,
std::size_t alignment);

    x_coord_t Img::width() const;
    y_coord_t Img::height() const;

    void Img::recreate(x_coord_t width, y_coord_t height, std::size_t
alignment=1); void Img::recreate(x_coord_t width, y_coord_t height, value_type
fill_value, std::size_t alignment);
};
\endcode
*/
template <typename Img> struct RandomAccess2DImageConcept {
  void constraints() {
    ...
    gil_function_requires<
        MutableRandomAccess2DImageViewConcept<typename Img::view_t>>();
...
\endcode
*/
template <typename Img> struct ImageConcept {
  void constraints() {
    ...
    BOOST_STATIC_ASSERT(num_channels<Img>::value ==
                        mpl::size<typename color_space_type<Img>::type>::value);
