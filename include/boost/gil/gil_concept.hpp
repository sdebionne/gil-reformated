...
inline typename kth_element_reference_type<bit_aligned_pixel_reference<C, L, M>,
                                           K>::type
...
kth_element_type<CB,K+1>::type>; kth_element_const_reference_type<CB,0>::type
...
HomogeneousColorBaseConcept<CB> { kth_element_reference_type<CB,0>::type
...
/// another. \ingroup PixelAlgorithm
...
namespace detail {
// Iterators that can be used as the base of memory_based_step_iterator require
...
/// \brief Concept of a random-access iterator that can be advanced in memory
/// units (bytes or bits) \ingroup PixelIteratorConceptStepIterator
/**
\code
concept MemoryBasedIteratorConcept<boost_concepts::RandomAccessTraversalConcept
Iterator> { typename byte_to_memunit<Iterator>; where
metafunction<byte_to_memunit<Iterator> >; std::ptrdiff_t      memunit_step(const
Iterator&); std::ptrdiff_t      memunit_distance(const Iterator& , const
Iterator&); void                memunit_advance(Iterator&, std::ptrdiff_t diff);
    Iterator            memunit_advanced(const Iterator& p, std::ptrdiff_t diff)
{ Iterator tmp; memunit_advance(tmp,diff); return tmp; } Iterator::reference
memunit_advanced_ref(const Iterator& p, std::ptrdiff_t diff) { return
*memunit_advanced(p,diff); }
};
\endcode
*/
template <typename Iterator> struct MemoryBasedIteratorConcept {
  void constraints() {
    gil_function_requires<
        boost_concepts::RandomAccessTraversalConcept<Iterator>>();
    gil_function_requires<
