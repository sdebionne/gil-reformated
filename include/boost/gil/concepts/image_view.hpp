...
///     (1-dimensional!) typename const_t;  where
///     RandomAccessNDImageViewConcept<View>; // same as View, but over
///     immutable values typename point_t;  where PointNDConcept<point_t>; //
///     N-dimensional point typename locator;  where
///     RandomAccessNDLocatorConcept<locator>; // N-dimensional locator.
///     typename iterator; where RandomAccessTraversalConcept<iterator>; //
///     1-dimensional iterator over all values typename reverse_iterator; where
///     RandomAccessTraversalConcept<reverse_iterator>; typename size_type; //
///     the return value of size()
///
///     // Equivalent to RandomAccessNDLocatorConcept::axis
///     template <size_t D> struct axis {
///         typename coord_t = point_t::axis<D>::coord_t;
///         typename iterator; where RandomAccessTraversalConcept<iterator>; //
///         iterator along D-th axis. where SameType<coord_t,
///         iterator::difference_type>; where
///         SameType<iterator::value_type,value_type>;
///     };
///
///     // Defines the type of a view similar to this type, except it invokes
///     Deref upon dereferencing template <PixelDereferenceAdaptorConcept Deref>
///     struct add_deref {
...
///     1-dimensional reference iterator         View::begin()      const;
///     iterator         View::end()        const;
///     reverse_iterator View::rbegin()     const;
///     reverse_iterator View::rend()       const;
///     iterator         View::at(const point_t&);
///     point_t          View::dimensions() const; // number of elements along
///     each dimension bool             View::is_1d_traversable() const;   //
///     can an iterator over the first dimension visit each value? I.e. are
///     there gaps between values?
///
...
template <typename View> struct RandomAccessNDImageViewIsMutableConcept {
  void constraints() {
...
/// \tparam View Models RandomAccessNDImageViewConcept
template <typename View> struct RandomAccess2DImageViewIsMutableConcept {
  void constraints() {
    gil_function_requires<
...
template <typename View> struct PixelImageViewIsMutableConcept {
  void constraints() {
    gil_function_requires<
