...
// some additional functions \tparam Iterator Models
// boost_concepts::RandomAccessTraversalConcept
template <typename Iterator> struct RandomAccessIteratorIsMemoryBasedConcept {
  void constraints() {
...
template <typename Iterator> struct PixelIteratorIsMutableConcept {
  void constraints() {
...
/// navigation \code concept HasDynamicYStepTypeConcept<typename T>
/// {
...
/// \ingroup PixelLocatorConcept
/// \brief Concept for locators and views that can define a type just like the
/// given locator or view, except X and Y is swapped \code concept
/// HasTransposedTypeConcept<typename T>
/// {
...
///  - \p is_iterator_adaptor<Iterator>:   Returns whether the given iterator is
///  an adaptor over another iterator. See IteratorAdaptorConcept for additional
...
/// units (bytes or bits) \code concept
/// MemoryBasedIteratorConcept<boost_concepts::RandomAccessTraversalConcept
/// Iterator>
/// {
///     typename byte_to_memunit<Iterator>; where
///     metafunction<byte_to_memunit<Iterator> >; std::ptrdiff_t
///     memunit_step(const Iterator&); std::ptrdiff_t memunit_distance(const
///     Iterator& , const Iterator&); void memunit_advance(Iterator&,
///     std::ptrdiff_t diff); Iterator            memunit_advanced(const
///     Iterator& p, std::ptrdiff_t diff) { Iterator tmp;
///     memunit_advance(tmp,diff); return tmp; } Iterator::reference
...
/// \ingroup PixelIteratorConceptIteratorAdaptor
/// \code
/// concept MutableIteratorAdaptorConcept<Mutable_ForwardIteratorConcept
/// Iterator>
