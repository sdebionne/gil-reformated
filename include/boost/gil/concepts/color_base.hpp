...
at_c(detail::homogeneous_color_base<E, L, N> const &p);
...
at_c(packed_pixel<P, C, L> const &p);
...
/// space, as well as an ordering of the channels). There are two ways to index
/// the elements of a color base: A physical index corresponds to the way they
/// are ordered in memory, and a semantic index corresponds to the way the
/// elements are ordered in their color space. For example, in the RGB color
/// space the elements are ordered as {red_t, green_t, blue_t}. For a color base
/// with a BGR layout, the first element in physical ordering is the blue
/// element, whereas the first semantic element is the red one. Models of \p
/// ColorBaseConcept are required to provide the \p at_c<K>(ColorBase) function,
/// which allows for accessing the elements based on their physical order. GIL
/// provides a \p semantic_at_c<K>(ColorBase) function (described later) which
/// can operate on any model of ColorBaseConcept and returns the corresponding
/// semantic element.
///
/// \code
...
///
///     // Copy-constructible and equality comparable with other compatible
///     color bases template <ColorBaseConcept T2> where {
///     ColorBasesCompatibleConcept<T,T2> }
///         T::T(T2);
...
///     kth_element_const_reference_type<CB,0>::type dynamic_at_c(const CB&,
///     std::size_t n) const;
/// };
/// \endcode
...
/// {
///     kth_element_reference_type<CB, 0>::type dynamic_at_c(CB&, std::size_t
///     n);
/// };
/// \endcode
...
/// their elements are compatible, semantic-pairwise. \code concept
/// ColorBasesCompatibleConcept<ColorBaseConcept C1, ColorBaseConcept C2>
/// {
///     where SameType<C1::layout_t::color_space_t,
///     C2::layout_t::color_space_t>;
