...
///     return value of operator-. typename const_t;           // same as Loc,
///     but operating over immutable values typename cached_location_t; // type
///     to store relative location (for efficient repeated access) typename
///     point_t  = difference_type;
///
///     static const size_t num_dimensions; // dimensionality of the locator
///     where num_dimensions = point_t::num_dimensions;
///
///     // The difference_type and iterator type along each dimension. The
///     iterators may only differ in
...
///
///     // Accessing iterators along a given dimension at the current location
///     or at a given offset template <size_t D> axis<D>::iterator&
///     Loc::axis_iterator(); template <size_t D> axis<D>::iterator const&
///     Loc::axis_iterator() const; template <size_t D> axis<D>::iterator
///     Loc::axis_iterator(const difference_type&) const;
/// };
/// \endcode
template <typename Loc> struct RandomAccessNDLocatorConcept {
  void constraints() {
    ...
    // type used to store relative location (to allow for more efficient
    // repeated access)
...
template <typename Loc> struct PixelLocatorConcept {
  void constraints() {
    ...
    static_assert(
        std::is_same<typename Loc::x_coord_t, typename Loc::y_coord_t>::value,
        "");
  }
  Loc loc;
};
...
template <typename Loc> struct RandomAccessNDLocatorIsMutableConcept {
  void constraints() {
...
template <typename Loc> struct RandomAccess2DLocatorIsMutableConcept {
  void constraints() {
...
///
/// \code
/// concept MutableRandomAccessNDLocatorConcept<RandomAccessNDLocatorConcept
/// Loc>
/// {
...
///
/// \code
/// concept MutableRandomAccess2DLocatorConcept<RandomAccess2DLocatorConcept
/// Loc>
