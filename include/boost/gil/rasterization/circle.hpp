...
///
/// The main problems are connectivity and equation following. Circle can be
/// easily moved to new offset, and rotation has no effect on it (not
/// recommended to do rotation).

/// \ingroup CircleRasterization
/// \brief Rasterize trigonometric circle according to radius by sine and radius
/// by cosine
///
/// This rasterizer is the one used that is used in standard Hough circle
/// transform in the books. It is also quite expensive to compute. WARNING: the
/// product of this rasterizer does not follow circle equation, even though it
/// produces quite round like shapes.
struct trigonometric_circle_rasterizer {
  /// \brief Calculates minimum angle step that is distinguishable when walking
  /// on circle
  ///
  /// It is important to not have disconnected circle and to not compute
  /// unnecessarily, thus the result of this function is used when rendering.
  ...
  void operator()(std::ptrdiff_t radius, point_t offset,
                  RandomAccessIterator d_first) const {
    ...
    double angle = 0;
    // do note that + 1 was done inside count estimation, thus <= is not needed,
    // only <
    for (std::ptrdiff_t i = 0; i < iteration_count;
         ++i, angle += minimum_angle_step) {
      std::ptrdiff_t x =
          static_cast<std::ptrdiff_t>(std::round(radius * std::cos(angle)));
      std::ptrdiff_t y =
          static_cast<std::ptrdiff_t>(std::round(radius * std::sin(angle)));
...
struct midpoint_circle_rasterizer {
  /// \brief Calculate the amount of points that rasterizer will output
  std::ptrdiff_t point_count(std::ptrdiff_t radius) const noexcept {
    // the reason for pulling 8 out is so that when the expression radius *
    // cos(45 degrees) is used, it would yield the same result as here
  ...
  void operator()(std::ptrdiff_t radius, point_t offset,
    ...
    for (std::ptrdiff_t x = 1; x < iteration_distance; ++x) {
      std::ptrdiff_t midpoint =
          x * x + y_current * y_current - y_current - r_squared;
      if (midpoint > 0) {
        --y_current;
      }
