...
/// \param cc        Color converter function object.
/// \throw std::ios_base::failure
template <typename Device, typename Image, typename ColorConverter,
          typename FormatTag>
inline void read_and_convert_image(
    Device &device, Image &img, image_read_settings<FormatTag> const &settings,
...
/// metafunction. \param img       The image in which the data is read into.
/// \param settings  Specifies read settings depending on the image format.
/// \param cc        Color converter function object.
/// \throw std::ios_base::failure
template <typename String, typename Image, typename ColorConverter,
          typename FormatTag>
inline void read_and_convert_image(
...
/// metafunction. \param img       The image in which the data is read into.
/// \param cc        Color converter function object.
/// \param tag       Defines the image format. Must satisfy is_format_tag
/// metafunction. \throw std::ios_base::failure
template <typename String, typename Image, typename ColorConverter,
          typename FormatTag>
inline void read_and_convert_image(
    String const &file_name, Image &img, ColorConverter const &cc,
    FormatTag const &tag,
    typename std::enable_if<
        mpl::and_<is_format_tag<FormatTag>,
                  detail::is_supported_path_spec<String>>::type::value>::type
...
/// \brief Reads and color-converts an image. Image memory is allocated.
/// \param device Must satisfy is_input_device metafunction or
/// is_adaptable_input_device. \param img    The image in which the data is read
...
template <typename Device, typename Image, typename ColorConverter,
          typename FormatTag>
inline void read_and_convert_image(
    Device &device, Image &img, ColorConverter const &cc, FormatTag const &tag,
    typename std::enable_if<
        mpl::and_<detail::is_read_device<FormatTag, Device>,
                  is_format_tag<FormatTag>>::type::value>::type * /*dummy*/
    = nullptr) {
...
/// \brief Reads and color-converts an image. Image memory is allocated. Default
/// color converter is used. \param file_name File name. Must satisfy
/// is_supported_path_spec metafunction. \param img       The image in which the
/// data is read into. \param settings  Specifies read settings depending on the
...
/// \brief Reads and color-converts an image. Image memory is allocated. Default
/// color converter is used. \param device    It's a device. Must satisfy
/// is_input_device metafunction or is_adaptable_input_device. \param img The
/// image in which the data is read into. \param settings  Specifies read
...
/// color converter is used. \param file_name File name. Must satisfy
/// is_supported_path_spec metafunction. \param img       The image in which the
/// data is read into. \param tag       Defines the image format. Must satisfy
/// is_format_tag metafunction. \throw std::ios_base::failure
template <typename String, typename Image, typename FormatTag>
inline void read_and_convert_image(
...
template <typename Device, typename Image, typename FormatTag>
inline void read_and_convert_image(
    Device &device, Image &img, FormatTag const &tag,
    typename std::enable_if<
        mpl::and_<detail::is_read_device<FormatTag, Device>,
                  is_format_tag<FormatTag>>::type::value>::type * /*dummy*/
    = nullptr) {
