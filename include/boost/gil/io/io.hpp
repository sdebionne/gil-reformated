 ...
 *  - a format tag, to identify the image format, derived from
 * boost::gil::format_tag
 *  - boolean meta function is_supported<PixelType,FormatTag> must be
 * implemented for the new format tag
 *  - explicit specialisation of image_read_info<FormatTag> must be provided,
 * containing runtime information available before/at reading the image
 *  - explicit specialisation of image_write_info<FormatTag> must be provided,
 * containing runtime encoding parameters for writing an image
 ...
 *        reader( IODevice & device, typename
 * ConversionPolicy::color_converter_type const& cc ) image_read_info<FormatTag>
 * get_info(); template<typename Image> void read_image( Image &, point_t const&
 * top_left ); template<typename View> void read_view( View &, point_t const&
 * top_left );
 *      };
 *    \endcode
 ...
 *        template<typename View>
 *        void apply( View const&, point_t const& top_left,
 * image_write_info<FormatTag> const& );
 *      };
 *    \endcode
 *
 * Or instead of the items above implement overloads of read_view,
 * read_and_convert_view, read_image, read_and_convert_image, write_view and
 * read_image_info.
 *
 ...
 * A Device is simply an object used to read and write data to and from a
 * stream. The IODevice was added as a template paramter to be able to replace
 * the file_name access functionality. This is only an interim solution, as soon
 * as boost provides a good IO library, interfaces/constraints provided by that
