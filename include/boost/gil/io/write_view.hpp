...
inline void write_view(
    const String &file_name, const View &view, const FormatTag &tag,
    typename enable_if<typename mpl::and_<
...
inline void write_view(
    const String &file_name, const View &view,
    const image_write_info<FormatTag, Log> &info,
    typename enable_if<typename mpl::and_<
        typename detail::is_supported_path_spec<String>::type,
        typename is_format_tag<FormatTag>::type,
...
inline void write_view(
    Writer &writer, const any_image_view<Views> &view,
    typename enable_if<typename mpl::and_<
        typename detail::is_dynamic_image_writer<Writer>::type,
        typename is_format_tag<typename Writer::format_tag_t>::type>::type>::
        type * /* ptr */
    = 0) {
  writer.apply(view);
}
...
inline void
write_view(Device &device, const any_image_view<Views> &views,
           const FormatTag &tag,
           typename enable_if<typename mpl::and_<
               typename detail::is_write_device<FormatTag, Device>::type,
...
inline void
write_view(const String &file_name, const any_image_view<Views> &views,
           const FormatTag &tag,
           typename enable_if<typename mpl::and_<
               typename detail::is_supported_path_spec<String>::type,
               typename is_format_tag<FormatTag>::type>::type>::type * /* ptr */
