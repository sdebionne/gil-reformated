...
/// \brief A 2D locator over a virtual image. Upon dereferencing, invokes a
/// given function object passing it its coordinates. Models:
/// PixelLocatorConcept, HasDynamicXStepTypeConcept, HasDynamicYStepTypeConcept,
/// HasTransposedTypeConcept \ingroup PixelLocatorModel PixelBasedModel
///
template <typename Deref,
          bool IsTransposed> // A function object that given a point returns a
                             // reference. Models PixelDereferenceAdaptorConcept
...
public:
  ...
  template <typename NewDeref> struct add_deref {
    ...
    static type make(const virtual_2d_locator<Deref, IsTransposed> &loc,
      ...
      return type(loc.pos(), loc.step(),
                  deref_compose<NewDeref, Deref>(nderef, loc.deref_fn()));
    }
  };

  virtual_2d_locator(const point_t &p = point_t(0, 0),
  ...
  template <typename D, bool TR>
  virtual_2d_locator(const virtual_2d_locator<D, TR> &pl) : _p(pl._p) {}
  ...
  bool is_1d_traversable(x_coord_t) const {
    return false;
  } // is there no gap at the end of each row? I.e. can we use x_iterator to
    // visit every pixel instead of nested loops?
...
private:
  ...
  y_iterator
      _p; // contains the current position, the step and the dereference object
};
...
struct channel_mapping_type<virtual_2d_locator<D, TR>>
    : public channel_mapping_type<
          typename virtual_2d_locator<D, TR>::parent_t> {};
