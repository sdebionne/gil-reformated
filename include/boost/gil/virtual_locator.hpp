...
/// Upon dereferencing, invokes a given function object passing it its
/// coordinates. Models:
///   PixelLocatorConcept,
...
/// \tparam IsTransposed Indicates if locator should navigate in transposed
/// mode.
template <typename DerefFn, bool IsTransposed>
class virtual_2d_locator : public pixel_2d_locator_base<
...
public:
  using parent_t =
      pixel_2d_locator_base<virtual_2d_locator<DerefFn, IsTransposed>,
                            position_iterator<DerefFn, IsTransposed>,
                            position_iterator<DerefFn, 1 - IsTransposed>>;
  ...
  template <typename NewDerefFn> struct add_deref {
    using type =
        virtual_2d_locator<deref_compose<NewDerefFn, DerefFn>, IsTransposed>;
  ...
  virtual_2d_locator(point_t const &p = {0, 0}, point_t const &step = {1, 1},
  ...
  virtual_2d_locator(virtual_2d_locator<D, TR> const &loc, coord_t y_step)
      : y_pos_(loc.pos(), point_t(loc.step().x, loc.step().y * y_step),
               loc.deref_fn()) {}
  ...
  auto x() -> x_iterator & { return *gil_reinterpret_cast<x_iterator *>(this); }
  ...
  ///       i.e. can we use x_iterator to visit every pixel instead of nested
  ///       loops?
