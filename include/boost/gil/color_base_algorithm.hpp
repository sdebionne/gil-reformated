...
/// kth_semantic_element_const_reference_type, semantic_at_c \ingroup
/// ColorBaseAlgorithm \brief Support for accessing the elements of a color base
/// by semantic index The semantic index of an element is the index of its color
/// in the color space. Semantic indexing allows for proper pairing of elements
/// of color bases independent on their layout. For example, red is the first
/// semantic element of a color base regardless of whether it has an RGB layout
/// or a BGR layout. All GIL color base algorithms taking multiple color bases
/// use semantic indexing to access their elements.
  ...
  BOOST_STATIC_CONSTANT(
      int, semantic_index =
               (mpl::at_c<typename ColorBase::layout_t::channel_mapping_t,
                          K>::type::value));
  typedef
      typename ColorBase::template kth_element_type<semantic_index>::type type;
};
...
struct kth_semantic_element_reference_type {
  BOOST_STATIC_CONSTANT(
      int, semantic_index =
               (mpl::at_c<typename ColorBase::layout_t::channel_mapping_t,
                          K>::type::value));
  typedef typename ColorBase::template kth_element_reference_type<
      semantic_index>::type type;
...
typename color_element_reference_type<Color, CB>::type
get_color(CB &cb, Color = Color()) {
...
/// \ingroup ColorBaseAlgorithmColor
template <typename CB, typename Color>
typename color_element_const_reference_type<Color, CB>::type
get_color(const CB &cb, Color = Color()) {
...
/// \defgroup ColorBaseAlgorithmHomogeneous element_type,
/// element_reference_type, element_const_reference_type \ingroup
/// ColorBaseAlgorithm \brief Types for homogeneous color bases
...
struct element_reference_type
    : public ColorBase::template kth_element_reference_type<0> {};

/// \brief Specifies the return type of the constant element accessor at_c of a
/// homogeneous color base \ingroup ColorBaseAlgorithmHomogeneous
template <typename ColorBase>
struct element_const_reference_type
    : public ColorBase::template kth_element_const_reference_type<0> {};
...
template <typename Q> inline const Q &min(const Q &x, const Q &y) {
  return x < y ? x : y;
}
template <typename Q> inline Q &min(Q &x, Q &y) { return x < y ? x : y; }
template <typename Q> inline const Q &max(const Q &x, const Q &y) {
  return x < y ? y : x;
}
