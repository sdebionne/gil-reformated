...
///
/// Image view consists of a pixel 2D locator (defining the mechanism for
/// navigating in 2D) and the image dimensions.
///
/// Image views to images are what ranges are to STL containers. They are
/// lightweight objects, that don't own the pixels. It is the user's
/// responsibility that the underlying data remains valid for the lifetime of
/// the image view.
///
/// Similar to iterators and ranges, constness of views does not extend to
/// constness of pixels. A const \p image_view does not allow changing its
/// location in memory (resizing, moving) but does not prevent one from changing
/// the pixels. The latter requires an image view whose value_type is const.
///
/// Images have interfaces consistent with STL 1D random access containers, so
/// they can be used directly in STL algorithms like: \code
///  std::fill(img.begin(), img.end(), red_pixel);
/// \endcode
///
/// In addition, horizontal, vertical and 2D random access iterators are
/// provided.
///
/// Note also that \p image_view does not require that its element type be a
/// pixel. It could be instantiated with a locator whose \p value_type models
/// only \p Regular. In this case the image view models the weaker
/// RandomAccess2DImageViewConcept, and does not model PixelBasedConcept. Many
/// generic algorithms don't require the elements to be pixels.
///
////////////////////////////////////////////////////////////////////////////////////////
...
public:
  ...
  using coord_t =
      typename Loc::coord_t; // 1D difference type (same for all dimensions)
  using difference_type =
      coord_t; // result of operator-(1d_iterator,1d_iterator)
  using point_t = typename Loc::point_t;
  using locator = Loc;
  using const_t = image_view<typename Loc::const_t>; // same as this type, but
                                                     // over const values
  template <std::size_t D> struct axis {
    using coord_t =
        typename Loc::template axis<D>::coord_t; // difference_type along each
                                                 // dimension
    using iterator =
        typename Loc::template axis<D>::iterator; // 1D iterator type along each
                                                  // dimension
  };
  using iterator = iterator_from_2d<Loc>; // 1D iterator type for each pixel
                                          // left-to-right inside top-to-bottom
  using const_iterator = typename const_t::iterator; // may be used to examine,
                                                     // but not to modify values
  using const_reference =
      typename const_t::reference; // behaves as a const reference
  using pointer =
      typename std::iterator_traits<iterator>::pointer; // behaves as a pointer
                                                        // to the value type
  ...
  using x_iterator =
      typename xy_locator::x_iterator; // pixel iterator along a row
  using y_iterator =
      typename xy_locator::y_iterator; // pixel iterator along a column
  ...
  auto at(difference_type i) const -> iterator {
    // UB if the specified increment advances non-incrementable iterator (i.e.
    // past-the-end)
    BOOST_ASSERT(i < static_cast<difference_type>(size()));
    return begin() + i;
  }
  ...
  auto at(point_t const &p) const -> iterator {
    // UB if the specified coordinates advance non-incrementable iterator (i.e.
    // past-the-end)
  ...
  auto x_at(x_coord_t x, y_coord_t y) const -> x_iterator {
    ...
    BOOST_ASSERT(0 <= y &&
                 y < height()); // TODO: For empty image/view, shouldn't we
                                // accept: row_begin(0) == view.row_end(0) ?
  ...
  auto x_at(point_t const &p) const -> x_iterator {
    ...
    BOOST_ASSERT(0 <= p.y &&
                 p.y < height()); // TODO: For empty image/view, shouldn't we
                                  // accept: row_begin(0) == view.row_end(0) ?
  ...
  auto y_at(x_coord_t x, y_coord_t y) const -> y_iterator {
    BOOST_ASSERT(0 <= x &&
                 x < width()); // TODO: For empty image/view, shouldn't we
                               // accept: view.col_begin(0) == view.col_end(0) ?
    BOOST_ASSERT(0 <= y &&
  ...
  auto y_at(point_t const &p) const -> y_iterator {
    BOOST_ASSERT(0 <= p.x &&
                 p.x <
                     width()); // TODO: For empty image/view, shouldn't we
                               // accept: view.col_begin(0) == view.col_end(0) ?
    BOOST_ASSERT(0 <= p.y &&
...
template <typename L2> inline void swap(image_view<L2> &x, image_view<L2> &y) {
  using std::swap;
...
template <typename L> struct is_planar<image_view<L>> : public is_planar<L> {};

/////////////////////////////
//  HasDynamicXStepTypeConcept
/////////////////////////////

template <typename L> struct dynamic_x_step_type<image_view<L>> {
...
template <typename L> struct dynamic_y_step_type<image_view<L>> {
...
template <typename L> struct transposed_type<image_view<L>> {
