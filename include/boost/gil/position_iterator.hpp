...
/// PixelBasedConcept, HasDynamicXStepTypeConcept. Used to create virtual image
/// views.
...
struct position_iterator
    : public iterator_facade<
          ...
          random_access_traversal_tag, typename Deref::reference,
  ...
  position_iterator(const point_t &p, const point_t &step, const Deref &d)
      : _p(p), _step(step), _d(d) {}

  position_iterator(const position_iterator &p)
      : _p(p._p), _step(p._step), _d(p._d) {}
  template <typename D>
...
private:
  ...
  ptrdiff_t distance_to(const position_iterator &it) const {
