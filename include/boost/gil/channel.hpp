...
/// \brief channel traits implementation for built-in scalar or floating point
/// channel type \ingroup ChannelModel
...
/// floating point type \ingroup ChannelModel
template <typename T>
...
/// bit range. Models: ChannelValueConcept
...
protected:
  typedef typename mpl::if_c<Mutable, BitField &, const BitField &>::type
      data_ref_t;
  data_ref_t _data;
...
/// whose offset is fixed at compile time. Models ChannelConcept

template <typename BitField, // An integral type (the data type containing the
                             // bits of interest). Models unsigned integer
...
class packed_channel_reference<BitField, FirstBit, NumBits, false>
  ...
  typedef packed_channel_reference<BitField, FirstBit, NumBits, true>
      mutable_reference;
...
public:
  ...
  integer_t get() const {
    return (this->_data & (parent_t::max_val << FirstBit)) >> FirstBit;
  }
};
...
public:
  typedef packed_channel_reference<BitField, FirstBit, NumBits, false>
      const_reference;
  ...
  operator=(const packed_channel_reference &ref) const {
    set_from_reference(ref._data);
    return *this;
  }
  ...
  void set_unsafe(integer_t value) const {
    this->_data = (this->_data & ~channel_mask) | (value << FirstBit);
  }

private:
  void set_from_reference(const BitField &data) const {
    this->_data = (this->_data & ~channel_mask) | (data & channel_mask);
  }
};
...
class packed_dynamic_channel_reference<BitField, NumBits, false>
  ...
  typedef packed_dynamic_channel_reference<BitField, NumBits, true>
      mutable_reference;
  typedef typename parent_t::integer_t integer_t;

public:
  typedef packed_dynamic_channel_reference<BitField, NumBits, false>
      const_reference;

  packed_dynamic_channel_reference(typename parent_t::data_ref_t data,
                                   int first_bit)
      : parent_t(data), _first_bit(first_bit) {}
  packed_dynamic_channel_reference(const packed_dynamic_channel_reference &ref)
      : parent_t(ref._data), _first_bit(ref._first_bit) {}
...
public:
  ...
  operator=(const packed_dynamic_channel_reference &ref) const {
    set_unsafe(ref.get());
    return *this;
  }
  const packed_dynamic_channel_reference &
