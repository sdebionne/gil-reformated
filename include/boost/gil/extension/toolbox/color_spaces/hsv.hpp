...
typedef mpl::vector3<hsv_color_space::hue_t, hsv_color_space::saturation_t,
...
/// \brief RGB to HSV
template <> struct default_color_converter_impl<rgb_t, hsv_t> {
  ...
  void operator()(const P1 &src, P2 &dst) const {
    ...
    float32_t min_color =
        (std::min)(temp_red, (std::min)(temp_green, temp_blue));
    float32_t max_color =
...
/// \ingroup ColorConvert
/// \brief HSV to RGB
template <> struct default_color_converter_impl<hsv_t, rgb_t> {
  template <typename P1, typename P2>
  void operator()(const P1 &src, P2 &dst) const {
    using namespace hsv_color_space;
