...
///// \brief Converting CMYKA to any pixel type. Note: Supports homogeneous
///pixels only.
...
//                                    channel_multiply(get_color(src,yellow_t()),
//                                    get_color(src,alpha_t())),
//                                    channel_multiply(get_color(src,black_t()),
//                                    get_color(src,alpha_t())))
//            ,dst);
//    }
//};
  ...
  void operator()(const P1 &src, P2 &dst) const {
    ...
    default_color_converter_impl<cmyk_t, rgba_t>()(
        pixel<T1, cmyk_layout_t>(
            ...
            get_color(src, yellow_t()), get_color(src, black_t())),
        dst);
  }
};

/// \ingroup ColorConvert
/// \brief Unfortunately CMYKA to CMYKA must be explicitly provided - otherwise
/// we get ambiguous specialization error.
template <> struct default_color_converter_impl<cmyka_t, cmyka_t> {
  template <typename P1, typename P2>
  void operator()(const P1 &src, P2 &dst) const {
