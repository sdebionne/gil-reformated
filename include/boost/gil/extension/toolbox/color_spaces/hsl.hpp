...
typedef mpl::vector3<hsl_color_space::hue_t, hsl_color_space::saturation_t,
...
/// \brief RGB to HSL
template <> struct default_color_converter_impl<rgb_t, hsl_t> {
...
/// \ingroup ColorConvert
/// \brief HSL to RGB
template <> struct default_color_converter_impl<hsl_t, rgb_t> {
  template <typename P1, typename P2>
  void operator()(const P1 &src, P2 &dst) const {
    using namespace hsl_color_space;
