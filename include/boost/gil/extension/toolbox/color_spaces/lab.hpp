...
/// \brief LAB to XYZ
template <> struct default_color_converter_impl<lab_t, xyz_t> {
  ...
  void operator()(const P1 &src, P2 &dst) const {
    ...
    get_color(dst, x_t()) =
        0.95047f *
        powf((p + (get_color(src, a_color_opponent_t()) / 500.f)), 3.f);
    get_color(dst, z_t()) =
        1.08883f *
        powf((p - (get_color(src, b_color_opponent_t()) / 200.f)), 3.f);
  }
};
...
public:
  ...
  void operator()(const P1 &src, P2 &dst) const {
    ...
    bits32f f_y = forward_companding(
        channel_convert<bits32f>(get_color(src, xyz_color_space::y_t()))
        // / 1.f
    );

    bits32f f_x = forward_companding(
        channel_convert<bits32f>(get_color(src, xyz_color_space::x_t())) *
        (1.f / 0.95047f) // if the compiler is smart, it should
                         // precalculate this, no?
    );

    bits32f f_z = forward_companding(
        channel_convert<bits32f>(get_color(src, xyz_color_space::z_t())) *
...
/// \ingroup ColorConvert
/// \brief RGB to LAB
template <> struct default_color_converter_impl<rgb_t, lab_t> {
  template <typename P1, typename P2>
  void operator()(const P1 &src, P2 &dst) const {
    using namespace lab_color_space;
...
/// \ingroup ColorConvert
/// \brief LAB to RGB
template <> struct default_color_converter_impl<lab_t, rgb_t> {
  template <typename P1, typename P2>
  void operator()(const P1 &src, P2 &dst) const {
    using namespace lab_color_space;
