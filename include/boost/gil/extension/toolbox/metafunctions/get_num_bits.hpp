...
template <typename T> struct get_num_bits;
...
template <int N> struct get_num_bits<packed_channel_value<N>> {
  BOOST_STATIC_CONSTANT(int, value = N);
};

template <int N> struct get_num_bits<const packed_channel_value<N>> {
  BOOST_STATIC_CONSTANT(int, value = N);
};
