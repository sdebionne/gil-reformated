...
template <typename Locator> struct get_pixel_type_locator {
  using type =
      mp11::mp_if<typename is_bit_aligned<typename Locator::value_type>::type,
...
template <typename IndicesLoc, typename PaletteLoc>
struct indexed_image_deref_fn<
    IndicesLoc, PaletteLoc,
    typename std::enable_if<detail::is_channel_integral<
        typename IndicesLoc::value_type>::value>::type>
    : indexed_image_deref_fn_base<IndicesLoc, PaletteLoc> {
  using base_t = indexed_image_deref_fn_base<IndicesLoc, PaletteLoc>;

  indexed_image_deref_fn() : base_t() {}

  indexed_image_deref_fn(typename base_t::indices_locator_t const &indices_loc,
...
struct indexed_image_locator_type {
  using type =
      virtual_2d_locator<indexed_image_deref_fn<IndicesLoc, PaletteLoc>, false>;
};
...
view(Index_View iv, Palette_View pv) {
  ...
  using defer_fn_t = indexed_image_deref_fn<typename Index_View::locator,
...
private:
  void init(const point_t &dimensions, const std::size_t num_colors) {
    using defer_fn_t =
        indexed_image_deref_fn<indices_locator_t, palette_locator_t>;
