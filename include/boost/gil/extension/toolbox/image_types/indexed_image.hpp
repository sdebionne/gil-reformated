...
struct get_pixel_type_locator
    : mpl::if_<typename is_bit_aligned<typename Locator::value_type>::type,
               typename Locator::reference, typename Locator::value_type> {};
...
struct indexed_image_deref_fn_base {
  ...
  indexed_image_deref_fn_base(const indices_locator_t &indices_loc,
      ...
      : _indices_loc(indices_loc), _palette_loc(palette_loc) {}
...
struct indexed_image_deref_fn
  ...
  typedef indexed_image_deref_fn_base<IndicesLoc, PaletteLoc> base_t;
...
template <typename IndicesLoc, typename PaletteLoc>
struct indexed_image_deref_fn<IndicesLoc, PaletteLoc,
                              typename boost::enable_if<boost::is_integral<
                                  typename IndicesLoc::value_type>>::type>
    : indexed_image_deref_fn_base<IndicesLoc, PaletteLoc> {
  typedef indexed_image_deref_fn_base<IndicesLoc, PaletteLoc> base_t;
...
struct indexed_image_locator_type {
  typedef virtual_2d_locator<indexed_image_deref_fn<IndicesLoc, PaletteLoc>,
                             false>
      type;
};
...
public:
  ...
  indexed_image_view(const point_t &dimensions, std::size_t num_colors,
                     const Locator &locator)
      : image_view<Locator>(dimensions, locator), _num_colors(num_colors) {}
...
indexed_image_view<typename indexed_image_locator_type<
    typename Index_View::locator, typename Palette_View::locator>::type>
view(Index_View iv, Palette_View pv) {
  typedef indexed_image_view<typename indexed_image_locator_type<
      typename Index_View::locator, typename Palette_View::locator>::type>
      view_t;

  typedef indexed_image_deref_fn<typename Index_View::locator,
...
public:
  ...
  typedef
      typename indexed_image_locator_type<indices_locator_t,
                                          palette_locator_t>::type locator_t;
  ...
  indexed_image(const x_coord_t width = 0, const y_coord_t height = 0,
  ...
  indexed_image(const point_t &dimensions, const std::size_t num_colors = 1,
                const std::size_t indices_alignment = 0,
                const std::size_t palette_alignment = 0)
  ...
  indexed_image(const indexed_image &img)
      : _indices(img._indices), _palette(img._palette) {}
...
private:
  void init(const point_t &dimensions, const std::size_t num_colors) {
    typedef indexed_image_deref_fn<indices_locator_t, palette_locator_t>
        defer_fn_t;
...
template <typename Index, typename Pixel>
inline const typename indexed_image<Index, Pixel>::view_t &
view(indexed_image<Index, Pixel> &img) {
  return img._view;
}

template <typename Index, typename Pixel>
inline const typename indexed_image<Index, Pixel>::const_view_t
const_view(indexed_image<Index, Pixel> &img) {
  return static_cast<const typename indexed_image<Index, Pixel>::const_view_t>(
      img._view);
}
...
void fill_pixels(const indexed_image_view<Locator> &view, const Value &value) {
  ...
  fill_pixels(view.get_indices_view(),
              typename view_t::indices_view_t::value_type(0));
