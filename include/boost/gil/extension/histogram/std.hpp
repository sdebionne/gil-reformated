...
/// \brief Collection of functions to provide histogram support in GIL using
/// Standard
///        Template Library Containers
/// The conversion from Boost.GIL images to compatible histograms are provided.
/// The supported container types would be std::vector, std::array, std::map.
///
...
void fill_histogram(SrcView const &srcview, std::vector<T> &histogram,
...
void fill_histogram(SrcView const &srcview, std::array<T, N> &histogram,
                    bool accumulate = false) {
  gil_function_requires<ImageViewConcept<SrcView>>();
...
void fill_histogram(SrcView const &srcview, std::map<T1, T2> &histogram,
                    bool accumulate = false) {
  gil_function_requires<ImageViewConcept<SrcView>>();
