...
template <typename T> using get_view_t = typename T::view_t;

template <typename Images>
using images_get_views_t = mp11::mp_transform<get_view_t, Images>;

template <typename T> using get_const_view_t = typename T::const_view_t;
...
struct any_image_get_const_view {
  ...
  template <typename Image> result_type operator()(Image const &img) const {
...
////////////////////////////////////////////////////////////////////////////////////////
/// \ingroup ImageModel
/// \brief Represents a run-time specified image. Note it does NOT model
/// ImageConcept
///
/// Represents an image whose type (color space, layout, planar/interleaved
/// organization, etc) can be specified at run time. It is the runtime
/// equivalent of \p image. Some of the requirements of ImageConcept, such as
/// the \p value_type alias cannot be fulfilled, since the language does not
/// allow runtime type specification. Other requirements, such as access to the
/// pixels, would be inefficient to provide. Thus \p any_image does not fully
/// model ImageConcept. In particular, its \p view and \p const_view methods
...
public:
  using view_t =
      mp11::mp_rename<detail::images_get_views_t<any_image>, any_image_view>;
  using const_view_t =
      mp11::mp_rename<detail::images_get_const_views_t<any_image>,
                      any_image_view>;
  ...
  template <typename Image> any_image &operator=(Image const &img) {
  ...
  any_image &operator=(any_image<OtherImages...> const &img) {
    parent_t::operator=(
        (typename variant2::variant<OtherImages...> const &)img);
    return *this;
  }
...
/// is any view. \tparam Images Models ImageVectorConcept
template <typename... Images>
BOOST_FORCEINLINE auto view(any_image<Images...> &img) ->
...
/// any view. \tparam Types Models ImageVectorConcept
template <typename... Images>
