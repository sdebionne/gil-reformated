...
// "Efficient Run-Time Dispatching in Generic Programming with Minimal Code
...
template <typename Src> struct copy_to_vector {
  using type = typename detail::copy_to_vector_impl<typename begin<Src>::type,
                                                    size<Src>::value>::type;
};

template <> struct copy_to_vector<set<>> { using type = vector0<>; };
...
template <typename Types, typename Op> struct unary_reduce_impl {
  ...
  using unique_t = typename mpl::copy<
      reduced_t,
      mpl::inserter<mpl::set<>, mpl::insert<mpl::_1, mpl::_2>>>::type;
...
struct pair_generator {
  template <typename Vec2> struct apply {
    using type = std::pair<const typename mpl::at_c<Vec2, 0>::type *,
...
struct binary_reduce_impl {
  ...
  using BIN_TYPES =
      mpl::cross_vector<mpl::vector2<vec1_types, vec2_types>, pair_generator>;
...
////
////////////////////////////////////////////////////////
template <typename Op, typename T> struct reduce { using type = T; };
...
template <typename CS> struct reduce_color_space { using type = CS; };
...
{ using first_t = rgba_t; using second_t = bgra_t;
};
...
struct reduce_color_spaces {
    ...
    using mapping = typename mpl::transform<src_order_t,
type_to_index<dst_order_t,mpl::_1>>::type; static const int mapping_val =
type_vec_to_integer<mapping>::value;

    using _first_t = typename
reduce_color_spaces_impl<SrcColorSpace,DstColorSpace,mapping_val>::first_t;
    using _second_t = typename
reduce_color_spaces_impl<SrcColorSpace,DstColorSpace,mapping_val>::second_t;
    using swap_t = typename mpl::and_<color_space_is_base<DstColorSpace>,
mpl::not_< color_space_is_base<_second_t>>>; public: using first_t = typename
mpl::if_<swap_t, _second_t, _first_t>::type; using second_t = typename
mpl::if_<swap_t, _first_t, _second_t>::type;
};
*/
...
reduce_view_basic<copy_pixels_fn,View,true> { private: using color_space_t =
typename reduce_color_space<typename View::color_space_t>::type color_space_t;
// reduce the color space using layout_t = layout<color_space_t, typename
View::channel_mapping_t>; public: using type = typename derived_view_type<View,
use_default, layout_t, use_default, use_default, mpl::true_>::type;
};
*/
...
struct reduce_copy_pixop_compat<V1, V2, true> {
  using layout1 =
      layout<typename V1::color_space_t, typename V1::channel_mapping_t>;
  using layout2 =
      layout<typename V2::color_space_t, typename V2::channel_mapping_t>;
...
struct reduce_views_basic<copy_and_convert_pixels_fn<CC>, V1, V2, AreBasic> {
  ...
  using Cs1 = typename mpl::if_<Same, typename CsR::type,
                                typename V1::color_space_t>::type;
  using Cs2 = typename mpl::if_<Same, typename CsR::type,
                                typename V2::color_space_t>::type;

  using DV1 =
      typename derived_view_type<V1, use_default,
                                 layout<Cs1, typename V1::channel_mapping_t>,
                                 use_default, use_default, mpl::false_>::type;
  using DV2 =
      typename derived_view_type<V2, use_default,
                                 layout<Cs2, typename V2::channel_mapping_t>,
                                 use_default, use_default, mpl::true_>::type;
