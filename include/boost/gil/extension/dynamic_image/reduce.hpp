...
namespace detail {
template <typename SFirst, std::size_t NLeft> struct copy_to_vector_impl {
private:
...
template <typename SFirst> struct copy_to_vector_impl<SFirst, 1> {
  typedef vector<typename deref<SFirst>::type> type;
};
} // namespace detail

template <typename Src> struct copy_to_vector {
  typedef typename detail::copy_to_vector_impl<typename begin<Src>::type,
                                               size<Src>::value>::type type;
};

template <> struct copy_to_vector<set<>> { typedef vector0<> type; };
...
/// dynamically specified index to that container, the bits of an instance of
/// the corresponding type and a generic operation, invokes the operation on the
/// given type
///
///////////////////////////////////////////////////////
...
/// \brief Unary reduce.
///
/// Given a set of types and an operation, reduces each type in the set (to
/// reduced_t), then removes duplicates (to unique_t) To apply the operation,
/// first constructs a lookup table that maps each element from Types to its
/// place in unique_t and uses it to map the index to anther index (in
/// map_index). Then invokes apply_operation_base on the unique types with the
/// new index.
///
///////////////////////////////////////////////////////

template <typename Types, typename Op> struct unary_reduce_impl {
  typedef typename mpl::transform<Types, detail::reduce<Op, mpl::_1>>::type
      reduced_t;
  typedef typename mpl::copy<
      reduced_t, mpl::inserter<mpl::set<>, mpl::insert<mpl::_1, mpl::_2>>>::type
      unique_t;
...
struct unary_reduce : public unary_reduce_impl<Types, Op> {
  ...
  GIL_FORCEINLINE static typename Op::result_type
  applyc(const Bits &bits, std::size_t index, Op op) {
    return apply_operation_basec<unique_t>(bits, map_index(index), op);
  }

  template <typename Bits>
  GIL_FORCEINLINE static typename Op::result_type
  apply(Bits &bits, std::size_t index, Op op) {
...
struct unary_reduce<Types, Op, true> : public unary_reduce_impl<Types, Op> {
  ...
  applyc(const Bits &bits, std::size_t index, Op op) {
    return op(
        *gil_reinterpret_cast_c<const typename mpl::front<unique_t>::type *>(
            &bits));
  }
  ...
  apply(Bits &bits, std::size_t index, Op op) {
    return op(
        *gil_reinterpret_cast<typename mpl::front<unique_t>::type *>(&bits));
  }
};
...
/// types and performs unary reduction on the result (bin_reduced_t). To apply
/// the binary operation, it simply invokes a unary apply_operation_base on the
/// reduced cross-product types
///
///////////////////////////////////////////////////////
...
struct pair_generator {
  template <typename Vec2> struct apply {
    typedef std::pair<const typename mpl::at_c<Vec2, 0>::type *,
...
struct binary_reduce_impl {
  // private:
  typedef
      typename mpl::copy_to_vector<typename Unary1::unique_t>::type vec1_types;
  typedef
      typename mpl::copy_to_vector<typename Unary2::unique_t>::type vec2_types;

  typedef mpl::cross_vector<mpl::vector2<vec1_types, vec2_types>,
                            pair_generator>
      BIN_TYPES;
...
public:
  ...
  static typename Op::result_type inline apply(const Bits1 &bits1,
                                               std::size_t index1,
                                               const Bits2 &bits2,
...
// When the types are large performs a double-dispatch. Binary reduction is not
// done.
...
template <typename Types1, typename Types2, typename Op> struct binary_reduce {
  // private:
  typedef unary_reduce<Types1, Op> unary1_t;
  typedef unary_reduce<Types2, Op> unary2_t;

  static const std::size_t CROSS_SIZE =
      mpl::size<typename unary1_t::unique_t>::value *
      mpl::size<typename unary2_t::unique_t>::value;

  typedef detail::binary_reduce_impl<unary1_t, unary2_t, Op,
...
apply_operation(variant<Types> &arg, UnaryOp op) {
  return unary_reduce<Types, UnaryOp>::template apply(arg._bits, arg._index,
                                                      op);
}
...
apply_operation(const variant<Types> &arg, UnaryOp op) {
  return unary_reduce<Types, UnaryOp>::template applyc(arg._bits, arg._index,
                                                       op);
}
...
namespace mpl {
///////////////////////////////////////////////////////
/// \brief Represents the virtual cross-product of the types generated from
/// VecOfVecs. \ingroup CrossVector INPUT:
///   VecOfVecs - a vector of vector types. For example [ [A1,A2,A3], [B1,B2],
///   [C1,C2,C3,C4] ]
///       Each element must be a non-empty mpl vector
///   TypeGen - a metafunction that generates a type from a vector of types,
///   each of which can be
///       selected from the corresponding vector in VecOfVecs. For example, [A1,
///       B2, C4]
///
...
template <typename VecOfVecs, typename TypeGen> struct cross_vector {};
...
/// \ingroup CrossVectorIterator
/// Creates a vector of the sizes of each type vector in VecOfVecs, then uses it
/// as a basis to represent the iterator's position K as a vector of indices.
/// Extracts the corresponding type of each input vector and passes the element
/// types to the type generation function, which returns the dereferenced type
template <typename VecOfVecs, typename TypeGen, std::size_t K>
...
/// \ingroup CrossVectorIterator
// (shortened the names of the template arguments - otherwise doxygen cannot
// parse this...)
...
///////////////////////////////////////////////////////
/// \brief Computes the size of a cross vector as the product of the sizes of
/// all vectors in VecOfVecs \ingroup CrossVector
...
struct transform<cross_vector<VecOfVecs, TypeGen>, OPP> {
  ...
  struct adapter {
    template <typename Elements> struct apply {
...
template <typename Op, typename View, bool IsBasic> struct reduce_view_basic {
  typedef View type;
};

template <typename Op, typename Loc>
struct reduce<Op, image_view<Loc>>
    : public reduce_view_basic<Op, image_view<Loc>,
                               view_is_basic<image_view<Loc>>::value> {};
...
template <typename Op, typename Img, bool IsBasic> struct reduce_image_basic {
  typedef Img type;
};

template <typename Op, typename V, typename Alloc>
struct reduce<Op, image<V, Alloc>>
    : public reduce_image_basic<Op, image<V, Alloc>,
                                image_is_basic<image<V, Alloc>>::value> {};
...
////////////////////////////////////////////////////////
////
////   Color space unary reduce operation. Reduce a color space to a base with
...
////////////////////////////////////////////////////////
////
////   Color space binary reduce operation. Given a source and destination color
spaces,
////   returns a reduced source and destination color spaces that have the same
mapping of channels
////
////   Precondition: The two color spaces must be compatible (i.e. must have the
...
struct type_vec_to_integer_impl {
    ...
    static const int value = type_vec_to_integer_impl<rest, Basis,
VecSize-1>::value * Basis + last::value;
};
...
struct type_vec_to_integer {
    static const int value = type_vec_to_integer_impl<Vec,Basis,
mpl::size<Vec>::value>::value;
};

// Given two color spaces and the mapping of the channels between them, returns
the reduced pair of color spaces
...
// 2103: RGBA-bgra, bgra-RGBA (uses subclass to ensure that base color space is
not reduced to derived) template <typename SrcColorSpace, typename
DstColorSpace> struct reduce_color_spaces_impl<SrcColorSpace,DstColorSpace,2103>
{ typedef rgba_t first_t; typedef bgra_t second_t;
};
...
struct reduce_color_spaces {
    ...
    typedef typename mpl::transform<src_order_t,
type_to_index<dst_order_t,mpl::_1> >::type mapping; static const int mapping_val
= type_vec_to_integer<mapping>::value;

    typedef typename
reduce_color_spaces_impl<SrcColorSpace,DstColorSpace,mapping_val>::first_t
_first_t; typedef typename
reduce_color_spaces_impl<SrcColorSpace,DstColorSpace,mapping_val>::second_t
_second_t; typedef typename mpl::and_<color_space_is_base<DstColorSpace>,
mpl::not_< color_space_is_base<_second_t> > > swap_t; public: typedef typename
mpl::if_<swap_t, _second_t, _first_t>::type first_t; typedef typename
mpl::if_<swap_t, _first_t, _second_t>::type second_t;
};
*/
// TODO: Use the old code for reduce_color_spaces above to do color layout
// reduction
template <typename SrcLayout, typename DstLayout> struct reduce_color_layouts {
...
/*
// 1D reduce for copy_pixels reduces the channel to mutable and the color space
to its base with same dimensions template <typename View> struct
reduce_view_basic<copy_pixels_fn,View,true> { private: typedef typename
reduce_color_space<typename View::color_space_t>::type Cs;    // reduce the
color space typedef layout<Cs, typename View::channel_mapping_t> layout_t;
public:
    typedef typename derived_view_type<View, use_default, layout_t, use_default,
use_default, mpl::true_>::type type;
};
*/
...
// For compatible basic views, reduce their color spaces based on their channel
// mapping. Make the source immutable and the destination mutable (they should
// already be that way)
template <typename V1, typename V2>
struct reduce_copy_pixop_compat<V1, V2, true> {
  typedef layout<typename V1::color_space_t, typename V1::channel_mapping_t>
      layout1;
  typedef layout<typename V2::color_space_t, typename V2::channel_mapping_t>
      layout2;
  ...
  typedef
      typename derived_view_type<V1, use_default, L1, use_default, use_default,
                                 use_default, mpl::false_>::type DV1;
  typedef
      typename derived_view_type<V2, use_default, L2, use_default, use_default,
                                 use_default, mpl::true_>::type DV2;
...
struct reduce_views_basic<copy_pixels_fn, V1, V2, true>
    : public reduce_copy_pixop_compat<
          V1, V2,
          mpl::and_<views_are_compatible<V1, V2>, view_is_mutable<V2>>::value> {
};
...
template <typename View> struct reduce_view_basic<destructor_op, View, true> {
  typedef gray8_view_t type;
};

////////////////////////////////////////////////////////
////
////   Reduce for get_dimensions (basic views and images have the same structure
///and the dimensions are contained at the beginning)
////
////////////////////////////////////////////////////////
...
struct reduce_view_basic<any_type_get_num_channels, View, true> {
  ...
  typedef typename view_type<bits8, typename reduce_color_space<Cs>::type>::type
      type;
};
template <typename Img>
struct reduce_image_basic<any_type_get_num_channels, Img, true> {
  ...
  typedef
      typename image_type<bits8, typename reduce_color_space<Cs>::type>::type
          type;
};
...
struct reduce_view_basic<resample_pixels_fn<S, M>, V, IsBasic>
    : public reduce_view_basic<copy_pixels_fn, V, IsBasic> {};

template <typename S, typename M, typename V1, typename V2, bool IsBasic>
struct reduce_views_basic<resample_pixels_fn<S, M>, V1, V2, IsBasic>
    : public reduce_views_basic<copy_pixels_fn, V1, V2, IsBasic> {};

////////////////////////////////////////////////////////
////
////   Reduce for copy_and_convert_pixels
////   (the only reduction could be made when views are compatible and have the
///same mapping, planarity and stepness)
////
////////////////////////////////////////////////////////
...
struct reduce_view_basic<copy_and_convert_pixels_fn<CC>, View, IsBasic>
    : public derived_view_type<View, use_default, use_default, use_default,
                               use_default, mpl::true_> {};

// For 2D reduce, if they have the same channels and color spaces (i.e. the same
// pixels) then copy_and_convert is just copy. In this case, reduce their common
...
struct reduce_views_basic<copy_and_convert_pixels_fn<CC>, V1, V2, AreBasic> {
  ...
  typedef typename mpl::if_<Same, typename CsR::type,
                            typename V1::color_space_t>::type Cs1;
  typedef typename mpl::if_<Same, typename CsR::type,
                            typename V2::color_space_t>::type Cs2;

  typedef typename derived_view_type<
      V1, use_default, layout<Cs1, typename V1::channel_mapping_t>, use_default,
      use_default, mpl::false_>::type DV1;
  typedef typename derived_view_type<
      V2, use_default, layout<Cs2, typename V2::channel_mapping_t>, use_default,
      use_default, mpl::true_>::type DV2;
...
// detail::flipped_left_right_view_fn<typename
// any_image_view<ViewTypes>::dynamic_step_t> detail::tranposed_view_fn<typename
// any_image_view<ViewTypes>::dynamic_step_t>
// detail::rotated90cw_view_fn<typename
// any_image_view<ViewTypes>::dynamic_step_t>
// detail::rotated90ccw_view_fn<typename
// any_image_view<ViewTypes>::dynamic_step_t> detail::rotated180_view_fn<typename
// any_image_view<ViewTypes>::dynamic_step_t>
// detail::subimage_view_fn<any_image_view<ViewTypes> >
// detail::subsampled_view_fn<typename
// any_image_view<ViewTypes>::dynamic_step_t>
// detail::nth_channel_view_fn<typename
// nth_channel_view_type<any_image_view<ViewTypes> >
// detail::color_converted_view_fn<DstP,typename
// color_convert_view_type<any_image_view<ViewTypes>, DstP>::type >
} // namespace detail
