...
template <typename Types1, // Model MPL Random Access Container of models of
                           // ImageViewConcept
...
template <typename View1,  // Model ImageViewConcept
          typename Types2> // Model MPL Random Access Container of models of
                           // MutableImageViewConcept
                           bool
                           equal_pixels(const View1 &src,
...
template <typename Types1, // Model MPL Random Access Container of models of
          ...
          typename Types2> // Model MPL Random Access Container of models of
                           // MutableImageViewConcept
                           bool
                           equal_pixels(const any_image_view<Types1> &src,
                                        const any_image_view<Types2> &dst) {
...
template <typename View1,  // Model ImageViewConcept
          typename Types2> // Model MPL Random Access Container of models of
                           // MutableImageViewConcept
                           void copy_pixels(const View1 &src,
                                            const any_image_view<Types2> &dst) {
...
void copy_and_convert_pixels(const any_image_view<Types1> &src,
                             const View2 &dst, CC cc) {
  apply_operation(
      src, boost::bind(detail::copy_and_convert_pixels_fn<CC>(cc), _1, dst));
}
...
void copy_and_convert_pixels(const any_image_view<Types1> &src,
  ...
  apply_operation(
      src,
      boost::bind(detail::copy_and_convert_pixels_fn<default_color_converter>(),
                  _1, dst));
}
...
template <typename View1,  // Model ImageViewConcept
          typename Types2, // Model MPL Random Access Container of models of
                           // MutableImageViewConcept
          typename CC>     // Model ColorConverterConcept
void copy_and_convert_pixels(const View1 &src,
                             const any_image_view<Types2> &dst, CC cc) {
  apply_operation(
      dst, boost::bind(detail::copy_and_convert_pixels_fn<CC>(cc), src, _1));
}
...
template <typename Value> struct fill_pixels_fn {
  ...
  template <typename V> result_type operator()(const V &img_view) const {
    fill_pixels_fn1<pixels_are_compatible<typename V::value_type,
                                          Value>::value>::apply(img_view, _val);
  }
  Value _val;
};
} // namespace detail
...
/// compatible with the current view
template <typename Types, // Model MPL Random Access Container of models of
                          // MutableImageViewConcept
          typename Value>
