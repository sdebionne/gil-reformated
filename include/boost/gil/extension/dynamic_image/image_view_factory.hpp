...
template <typename ResultView> struct flipped_up_down_view_fn {
...
template <typename ResultView> struct flipped_left_right_view_fn {
  using result_type = ResultView;
...
template <typename ResultView> struct rotated90cw_view_fn {
  using result_type = ResultView;
...
template <typename ResultView> struct rotated90ccw_view_fn {
  using result_type = ResultView;
...
template <typename ResultView> struct tranposed_view_fn {
  using result_type = ResultView;
...
template <typename ResultView> struct rotated180_view_fn {
  using result_type = ResultView;
...
template <typename ResultView> struct subimage_view_fn {
  using result_type = ResultView;
...
template <typename ResultView> struct subsampled_view_fn {
  using result_type = ResultView;
...
template <typename ResultView> struct nth_channel_view_fn {
  using result_type = ResultView;
...
template <typename DstP, typename ResultView,
...
/// \tparam Views Models Boost.MP11-compatible list of models of
/// ImageViewConcept
...
inline auto flipped_left_right_view(any_image_view<Views> const &src) ->
  ...
  using result_view_t =
      typename dynamic_x_step_type<any_image_view<Views>>::type;
  return apply_operation(src,
                         detail::flipped_left_right_view_fn<result_view_t>());
}
...
inline auto rotated90ccw_view(const any_image_view<Views> &src) ->
  ...
  return apply_operation(
      src,
      detail::rotated90ccw_view_fn<typename dynamic_xy_step_transposed_type<
...
/// ImageViewConcept
template <typename Views>
inline auto subimage_view(any_image_view<Views> const &src,
...
/// ImageViewConcept
template <typename Views>
inline auto subimage_view(any_image_view<Views> const &src, int xMin, int yMin,
                          int width, int height) -> any_image_view<Views> {
  using subimage_view_fn = detail::subimage_view_fn<any_image_view<Views>>;
  return apply_operation(
      src, subimage_view_fn(point_t(xMin, yMin), point_t(width, height)));
}
...
/// ImageViewConcept
template <typename Views>
inline auto subsampled_view(any_image_view<Views> const &src,
                            point_t const &step) ->
    typename dynamic_xy_step_type<any_image_view<Views>>::type {
  using step_type = typename dynamic_xy_step_type<any_image_view<Views>>::type;
...
/// ImageViewConcept
template <typename Views>
inline auto subsampled_view(any_image_view<Views> const &src, int xStep,
                            int yStep) ->
    typename dynamic_xy_step_type<any_image_view<Views>>::type {
  using step_type = typename dynamic_xy_step_type<any_image_view<Views>>::type;
...
template <typename View> struct get_nthchannel_type {
  using type = typename nth_channel_view_type<View>::type;
};

template <typename Views>
struct views_get_nthchannel_type
    : mp11::mp_transform<get_nthchannel_type, Views> {};

} // namespace detail

/// \ingroup ImageViewTransformationsNthChannel
/// \brief Given a runtime source image view, returns the type of a runtime
/// image view over a single channel of the source view
template <typename Views> struct nth_channel_view_type<any_image_view<Views>> {
  using type =
      any_image_view<typename detail::views_get_nthchannel_type<Views>::type>;
};
...
private:
  ...
  // in the class definition of the same name as the specialization (Peter
  // Dimov):
  //    invalid template argument for template parameter 'F', expected a class
  //    template
  template <typename T> using ccvt = detail::get_ccv_type<T, DstP, CC>;
...
/// \brief Returns the type of a runtime-specified view, color-converted to a
/// given pixel type with user specified color converter
template <typename Views, typename DstP, typename CC>
struct color_converted_view_type<any_image_view<Views>, DstP, CC> {
  using type = any_image_view<
      typename detail::views_get_ccv_type<Views, DstP, CC>::type>;
};

/// \ingroup ImageViewTransformationsColorConvert
/// \brief overload of generic color_converted_view with user defined
/// color-converter \tparam Views Models Boost.MP11-compatible list of models of
/// ImageViewConcept
...
inline auto color_converted_view(const any_image_view<Views> &src, CC) ->
  ...
  using cc_view_t =
      typename color_converted_view_type<any_image_view<Views>, DstP, CC>::type;
  return apply_operation(src,
                         detail::color_converted_view_fn<DstP, cc_view_t>());
}

/// \ingroup ImageViewTransformationsColorConvert
/// \brief Returns the type of a runtime-specified view, color-converted to a
/// given pixel type with the default coor converter
template <typename Views, typename DstP>
struct color_converted_view_type<any_image_view<Views>, DstP> {
  using type = any_image_view<typename detail::views_get_ccv_type<
      Views, DstP, default_color_converter>::type>;
};
...
/// \brief overload of generic color_converted_view with user defined
/// color-converter
///        These are workarounds for GCC 3.4, which thinks color_converted_view
///        is ambiguous with the same method for templated views (in
///        gil/image_view_factory.hpp)
/// \tparam Views Models Boost.MP11-compatible list of models of
/// ImageViewConcept
template <typename DstP, typename Views, typename CC>
