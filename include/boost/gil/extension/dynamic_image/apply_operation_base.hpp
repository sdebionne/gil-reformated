...
////////////////////////////////////////////////////////////////////////////////////////
/// \file
/// \brief Given an object with run-time specified type (denoted as an array of
/// Bits, dynamic index, and a static set of Types) and a generic operation,
...
/*
GENERATE_APPLY_FWD_OPS generates for every N functions that look like this (for
N==2):

    template <> struct apply_operation_fwd_fn<3> {
        template <typename Types, typename Bits, typename UnaryOp>
        typename UnaryOp::result_type apply(Bits& bits, std::size_t index,
UnaryOp op) const { typedef typename mpl::begin<Types>::type T0; typedef
typename mpl::next<T0>::type T1; typedef typename mpl::next<T1>::type T2; switch
(index) { case 0: return op(reinterpret_cast<typename
mpl::deref<T0>::type&>(bits)); case 1: return op(reinterpret_cast<typename
mpl::deref<T1>::type&>(bits)); case 2: return op(reinterpret_cast<typename
mpl::deref<T2>::type&>(bits));
            }
            throw;
        }

        template <typename Types, typename Bits, typename UnaryOp>
        typename UnaryOp::result_type applyc(const Bits& bits, std::size_t
index, UnaryOp op) const { typedef typename mpl::begin<Types>::type T0; typedef
typename mpl::next<T0>::type T1; typedef typename mpl::next<T1>::type T2; switch
(index) { case 0: return op(reinterpret_cast<const typename
mpl::deref<T0>::type&>(bits)); case 1: return op(reinterpret_cast<const typename
mpl::deref<T1>::type&>(bits)); case 2: return op(reinterpret_cast<const typename
mpl::deref<T2>::type&>(bits));
            }
            throw;
        }
...
#define GIL_FWD_TYPEDEFS(z, N, text)                                           \
  T##N;                                                                        \
  typedef typename mpl::next<T##N>::type
#define GIL_FWD_CASE(z, N, SUM)                                                \
  case N:                                                                      \
    return op(*gil_reinterpret_cast<typename mpl::deref<T##N>::type *>(&bits));
#define GIL_FWD_CONST_CASE(z, N, SUM)                                          \
  case N:                                                                      \
    return op(                                                                 \
        *gil_reinterpret_cast_c<const typename mpl::deref<T##N>::type *>(      \
            &bits));

#define GIL_FWD_CASE_WITH_INFO(z, N, SUM)                                      \
  case N:                                                                      \
    return op(*gil_reinterpret_cast<typename mpl::deref<T##N>::type *>(&bits), \
              info);
#define GIL_FWD_CONST_CASE_WITH_INFO(z, N, SUM)                                \
  case N:                                                                      \
    return op(                                                                 \
        *gil_reinterpret_cast_c<const typename mpl::deref<T##N>::type *>(      \
            &bits),                                                            \
...
#define GIL_APPLY_FWD_OP(z, N, text)                                           \
  template <> struct apply_operation_fwd_fn<BOOST_PP_ADD(N, 1)> {              \
    ...
    typename UnaryOp::result_type apply(Bits &bits, std::size_t index,         \
                                        UnaryOp op) const {                    \
      typedef typename mpl::begin<Types>::type BOOST_PP_REPEAT(                \
          N, GIL_FWD_TYPEDEFS, BOOST_PP_EMPTY) T##N;                           \
      switch (index) {                                                         \
    ...
    template <typename Types, typename Bits, typename UnaryOp>                 \
    typename UnaryOp::result_type applyc(const Bits &bits, std::size_t index,  \
                                         UnaryOp op) const {                   \
      typedef typename mpl::begin<Types>::type BOOST_PP_REPEAT(                \
          N, GIL_FWD_TYPEDEFS, BOOST_PP_EMPTY) T##N;                           \
      switch (index) {                                                         \
        BOOST_PP_REPEAT(BOOST_PP_ADD(N, 1), GIL_FWD_CONST_CASE,                \
                        BOOST_PP_EMPTY)                                        \
      }                                                                        \
      throw;                                                                   \
    }                                                                          \
    ...
    typename UnaryOp::result_type apply(Bits &bits, std::size_t index,         \
      ...
      typedef typename mpl::begin<Types>::type BOOST_PP_REPEAT(                \
          N, GIL_FWD_TYPEDEFS, BOOST_PP_EMPTY) T##N;                           \
      switch (index) {                                                         \
        BOOST_PP_REPEAT(BOOST_PP_ADD(N, 1), GIL_FWD_CASE_WITH_INFO,            \
                        BOOST_PP_EMPTY)                                        \
      }                                                                        \
      throw;                                                                   \
    }                                                                          \
    ...
    typename UnaryOp::result_type applyc(const Bits &bits, std::size_t index,  \
      ...
      typedef typename mpl::begin<Types>::type BOOST_PP_REPEAT(                \
          N, GIL_FWD_TYPEDEFS, BOOST_PP_EMPTY) T##N;                           \
      switch (index) {                                                         \
        BOOST_PP_REPEAT(BOOST_PP_ADD(N, 1), GIL_FWD_CONST_CASE_WITH_INFO,      \
                        BOOST_PP_EMPTY)                                        \
      }                                                                        \
      throw;                                                                   \
    }                                                                          \
...
#define GIL_GENERATE_APPLY_FWD_OPS(N)                                          \
  BOOST_PP_REPEAT(N, GIL_APPLY_FWD_OP, BOOST_PP_EMPTY)
