...
///////////////////////////////////////////////////////////////////////////
////
////     resample_pixels: set each pixel in the destination view as the result
/// of a sampling function over the transformed coordinates of the source view
////
///////////////////////////////////////////////////////////////////////////
/*
...
concept SamplerConcept {
    template <typename DstP,      // Models PixelConcept
              ...
              typename S_COORDS>  // Models PointNDConcept, where
S_COORDS::num_dimensions == SrcView::num_dimensions bool sample(const Sampler&
s, const SrcView& src, const S_COORDS& p, DstP result);
};
*/

/// \brief A sampler that sets the destination pixel to the closest one in the
/// source. If outside the bounds, it doesn't change the destination \ingroup
/// ImageAlgorithms
struct nearest_neighbor_sampler {};

template <typename DstP, typename SrcView, typename F>
bool sample(nearest_neighbor_sampler, SrcView const &src, point<F> const &p,
            DstP &result) {
...
template <typename Weight> struct add_dst_mul_src_channel {
  Weight _w;
...
/// \brief A sampler that sets the destination pixel as the bilinear
/// interpolation of the four closest pixels from the source. If outside the
/// bounds, it doesn't change the destination \ingroup ImageAlgorithms
struct bilinear_sampler {};

template <typename DstP, typename SrcView, typename F>
bool sample(bilinear_sampler, SrcView const &src, point<F> const &p,
            DstP &result) {
  using SrcP = typename SrcView::value_type;
  point_t p0(ifloor(p.x),
