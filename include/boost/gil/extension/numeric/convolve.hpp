...
void correlate_rows_imp(const SrcView &src, const Kernel &ker,
                        const DstView &dst, convolve_boundary_option option,
  ...
  if (option == convolve_option_output_ignore ||
    ...
    pixel_assigns_t<PixelAccum, PIXEL_DST_REF>()(acc_zero, dst_zero);
    ...
    for (int rr = 0; rr < height; ++rr) {
      ...
      if (option == convolve_option_extend_padded) {
        assign_pixels(src.row_begin(rr) - ker.left_size(),
                      src.row_end(rr) + ker.right_size(), it_buffer);
        ...
        PixelAccum filler;
        pixel_assigns_t<PIXEL_SRC_REF, PixelAccum>()(*src.row_begin(rr),
                                                     filler);
        std::fill_n(it_buffer, ker.left_size(), filler);
        it_buffer += ker.left_size();
        assign_pixels(src.row_begin(rr), src.row_end(rr), it_buffer);
        it_buffer += width;
        pixel_assigns_t<PIXEL_SRC_REF, PixelAccum>()(src.row_end(rr)[-1],
                                                     filler);
        std::fill_n(it_buffer, ker.right_size(), filler);
      }
      correlator(&buffer.front(), &buffer.front() + width, ker.begin(),
                 dst.row_begin(rr));
    }
  }
}
...
public:
  ...
  void operator()(SrcIterator src_begin, SrcIterator src_end,
    ...
    correlate_pixels_n<PixelAccum>(src_begin, src_end, ker_begin, _size,
                                   dst_begin);
  }
};
...
public:
  ...
  void operator()(SrcIterator src_begin, SrcIterator src_end,
                  KernelIterator ker_begin, DstIterator dst_begin) {
    correlate_pixels_k<Size, PixelAccum>(src_begin, src_end, ker_begin,
                                         dst_begin);
  }
};
} // namespace detail
...
correlate_rows(const SrcView &src, const Kernel &ker, const DstView &dst,
  ...
  detail::correlate_rows_imp<PixelAccum>(
      src, ker, dst, option, detail::correlator_n<PixelAccum>(ker.size()));
}
...
BOOST_FORCEINLINE void correlate_rows_fixed(
  ...
  detail::correlate_rows_imp<PixelAccum>(
      src, ker, dst, option,
      detail::correlator_k<Kernel::static_size, PixelAccum>());
}
