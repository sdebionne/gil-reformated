  ...
  if (option == convolve_option_output_ignore ||
    ...
    if (width < static_cast<int>(kernel.size())) {
      ...
      for (int rr = 0; rr < height; ++rr) {
        assign_pixels(src_view.row_begin(rr), src_view.row_end(rr),
                      &buffer.front());
    ...
    for (int rr = 0; rr < height; ++rr) {
      ...
      if (option == convolve_option_extend_padded) {
        assign_pixels(src_view.row_begin(rr) - kernel.left_size(),
                      src_view.row_end(rr) + kernel.right_size(), it_buffer);
        ...
        PixelAccum filler;
        pixel_assigns_t<SrcPixelRef, PixelAccum>()(*src_view.row_begin(rr),
                                                   filler);
        std::fill_n(it_buffer, kernel.left_size(), filler);
        it_buffer += kernel.left_size();
        assign_pixels(src_view.row_begin(rr), src_view.row_end(rr), it_buffer);
        it_buffer += width;
        pixel_assigns_t<SrcPixelRef, PixelAccum>()(src_view.row_end(rr)[-1],
                                                   filler);
...
template <typename PixelAccum, typename SrcView, typename Kernel,
          typename DstView>
BOOST_FORCEINLINE void
convolve(SrcView const &src_view, Kernel const &kernel, DstView const &dst_view,
         convolve_boundary_option option = convolve_option_extend_zero) {
