  ...
  if (option == boundary_option::output_ignore ||
    ...
    for (y_coord_t y = 0; y < height; ++y) {
      ...
      if (option == boundary_option::extend_padded) {
        ...
        assign_pixels(src_view.row_begin(y), src_view.row_end(y), it_buffer);
        it_buffer += width;
        pixel_assigns_t<src_pixel_ref_t, PixelAccum>()(src_view.row_end(y)[-1],
                                                       filler);
        std::fill_n(it_buffer, kernel.right_size(), filler);
      }

      correlator(&buffer.front(), &buffer.front() + width, kernel.begin(),
...
public:
  ...
  void operator()(SrcIterator src_begin, SrcIterator src_end,
                  KernelIterator kernel_begin, DstIterator dst_begin) {
    correlate_pixels_n<PixelAccum>(src_begin, src_end, kernel_begin, size_,
                                   dst_begin);
  }
...
template <std::size_t Size, typename PixelAccum> struct correlator_k {
  ...
  void operator()(SrcIterator src_begin, SrcIterator src_end,
                  KernelIterator kernel_begin, DstIterator dst_begin) {
    correlate_pixels_k<Size, PixelAccum>(src_begin, src_end, kernel_begin,
                                         dst_begin);
  }
};
...
void convolve_2d_impl(SrcView const &src_view, DstView const &dst_view,
                      Kernel const &kernel) {
...
/// \ingroup ImageAlgorithms
/// \brief convolve_2d can only use convolve_option_extend_zero as
...
void convolve_2d(SrcView const &src_view, Kernel const &kernel,
  ...
  for (std::size_t i = 0; i < src_view.num_channels(); i++) {
    detail::convolve_2d_impl(nth_channel_view(src_view, i),
                             nth_channel_view(dst_view, i), kernel);
  }
}
