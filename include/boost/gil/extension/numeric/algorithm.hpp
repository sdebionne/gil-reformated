...
/// \brief Returns the reference proxy associated with a type that has a \p
/// "reference" member typedef.
///
/// The reference proxy is the reference type, but with stripped-out C++
/// reference. It models PixelConcept
template <typename T>
...
inline DstIterator assign_pixels(SrcIterator src, SrcIterator src_end,
                                 DstIterator dst) {
  for_each(
      src, src_end, dst,
      pixel_assigns_t<
...
template <std::size_t Size> struct inner_product_k_t {
  ...
  static _Tp apply(_InputIterator1 __first1, _InputIterator2 __first2,
                   _Tp __init, _BinaryOperation1 __binary_op1,
                   _BinaryOperation2 __binary_op2) {
    __init = __binary_op1(__init, __binary_op2(*__first1, *__first2));
    return inner_product_k_t<Size - 1>::template apply(
        __first1 + 1, __first2 + 1, __init, __binary_op1, __binary_op2);
  }
};
...
template <typename PixelAccum, typename SrcIterator, typename KernelIterator,
          typename Integer, typename DstIterator>
inline DstIterator correlate_pixels_n(SrcIterator src_begin,
                                      SrcIterator src_end,
                                      KernelIterator ker_begin,
                                      Integer ker_size, DstIterator dst_begin) {
  typedef typename pixel_proxy<
      typename std::iterator_traits<SrcIterator>::value_type>::type
      PIXEL_SRC_REF;
  typedef typename pixel_proxy<
      typename std::iterator_traits<DstIterator>::value_type>::type
  ...
  while (src_begin != src_end) {
    pixel_assigns_t<PixelAccum, PIXEL_DST_REF>()(
        std::inner_product(src_begin, src_begin + ker_size, ker_begin, acc_zero,
                           ...
                           pixel_multiplies_scalar_t<PIXEL_SRC_REF, kernel_type,
                                                     PixelAccum>()),
        *dst_begin);
    ++src_begin;
    ++dst_begin;
...
inline DstIterator
correlate_pixels_k(SrcIterator src_begin, SrcIterator src_end,
                   KernelIterator ker_begin, DstIterator dst_begin) {
  typedef typename pixel_proxy<
      typename std::iterator_traits<SrcIterator>::value_type>::type
      PIXEL_SRC_REF;
  typedef typename pixel_proxy<
...
inline void view_multiplies_scalar(const SrcView &src, const Scalar &scalar,
  ...
  typedef
      typename pixel_proxy<typename SrcView::value_type>::type PIXEL_SRC_REF;
  typedef
      typename pixel_proxy<typename DstView::value_type>::type PIXEL_DST_REF;
  ...
  for (int rr = 0; rr < height; ++rr) {
    ...
    while (it_src != it_src_end) {
      pixel_assigns_t<PixelAccum, PIXEL_DST_REF>()(
          pixel_multiplies_scalar_t<PIXEL_SRC_REF, Scalar, PixelAccum>()(
              *it_src, scalar),
          *it_dst);
      ++it_src;
      ++it_dst;
