...
/// \brief Reference proxy associated with a type that has a \p "reference"
/// member type alias.
///
/// The reference proxy is the reference type, but with stripped-out C++
/// reference. Models PixelConcept.
template <typename T>
...
BinaryFunction for_each(Iterator1 first1, Iterator1 last1, Iterator2 first2,
                        BinaryFunction f) {
  while (first1 != last1)
    f(*first1++, *first2++);
  return f;
...
inline auto assign_pixels(SrcIterator src, SrcIterator src_end, DstIterator dst)
    -> DstIterator {
  for_each(
      src, src_end, dst,
      pixel_assigns_t<
...
template <std::size_t Size> struct inner_product_k_t {
  template <class InputIterator1, class InputIterator2, class T,
            class BinaryOperation1, class BinaryOperation2>
  static T apply(InputIterator1 first1, InputIterator2 first2, T init,
...
template <> struct inner_product_k_t<0> {
  template <class InputIterator1, class InputIterator2, class T,
            class BinaryOperation1, class BinaryOperation2>
  static T apply(InputIterator1 first1, InputIterator2 first2, T init,
                 BinaryOperation1 binary_op1, BinaryOperation2 binary_op2) {
...
template <std::size_t Size, class InputIterator1, class InputIterator2, class T,
          class BinaryOperation1, class BinaryOperation2>
BOOST_FORCEINLINE T inner_product_k(InputIterator1 first1,
...
inline auto correlate_pixels_n(SrcIterator src_begin, SrcIterator src_end,
                               KernelIterator kernel_begin, Size kernel_size,
  ...
  while (src_begin != src_end) {
    pixel_assigns_t<PixelAccum, dst_pixel_ref_t>()(
        std::inner_product(
            src_begin, src_begin + kernel_size, kernel_begin, accum_zero,
            pixel_plus_t<PixelAccum, PixelAccum, PixelAccum>(),
            pixel_multiplies_scalar_t<src_pixel_ref_t, kernel_value_t,
                                      PixelAccum>()),
        *dst_begin);
  ...
  while (src_begin != src_end) {
    pixel_assigns_t<PixelAccum, dst_pixel_ref_t>()(
        inner_product_k<Size>(
            src_begin, kernel_begin, accum_zero,
            pixel_plus_t<PixelAccum, PixelAccum, PixelAccum>(),
            pixel_multiplies_scalar_t<src_pixel_ref_t, kernel_type,
                                      PixelAccum>()),
        *dst_begin);
...
template <typename PixelAccum, typename SrcView, typename Scalar,
          typename DstView>
inline void view_multiplies_scalar(SrcView const &src_view,
                                   Scalar const &scalar,
  ...
  for (y_coord_t y = 0; y < height; ++y) {
    ...
    while (it_src != it_src_end) {
      pixel_assigns_t<PixelAccum, dst_pixel_ref_t>()(
          pixel_multiplies_scalar_t<src_pixel_ref_t, Scalar, PixelAccum>()(
              *it_src, scalar),
          *it_dst);
  ...
  if (option == boundary_option::extend_constant) {
    for (std::ptrdiff_t i = 0; i < result_view.height(); i++) {
      if (i >= extend_count_ && i < extend_count_ + src_view.height()) {
        assign_pixels(src_view.row_begin(i - extend_count_),
                      src_view.row_end(i - extend_count_),
                      result_view.row_begin(i));
      } else if (i < extend_count_) {
        assign_pixels(src_view.row_begin(0), src_view.row_end(0),
                      result_view.row_begin(i));
      } else {
        assign_pixels(src_view.row_begin(src_view.height() - 1),
...
/// The border provides space for annotations or acts as a boundary when using
/// advanced filtering techniques. \tparam SrcView Models ImageViewConcept
