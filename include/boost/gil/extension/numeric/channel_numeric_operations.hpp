...
/// \note This is a generic implementation; user should specialize it for better
/// performance.
...
struct channel_plus_scalar_t {
  ...
  auto operator()(ChannelRef channel, Scalar const &scalar) const
      -> ChannelResult {
...
struct channel_minus_scalar_t {
  ...
  auto operator()(ChannelRef channel, Scalar const &scalar) const
      -> ChannelResult {
    // TODO: Convertion after subtraction vs conversion of operands in
    // channel_minus_t?
...
template <typename Channel> struct channel_halves_t {
  ...
  auto operator()(ChannelRef channel) const -> ChannelRef {
    // TODO: Split into steps: extract with explicit conversion to double,
    // divide and assign?
    // double const v = ch;
