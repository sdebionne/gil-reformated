...
/*!
/// \file
/// \brief Definitions of 1D fixed-size and variable-size kernels and related
operations
...
public:
  ...
  explicit kernel_1d_adaptor(std::size_t center_in) : _center(center_in) {
    assert(_center < this->size());
  }
  kernel_1d_adaptor(std::size_t size_in, std::size_t center_in)
      : Core(size_in), _center(center_in) {
    assert(_center < this->size());
  }
  kernel_1d_adaptor(const kernel_1d_adaptor &k_in)
      : Core(k_in), _center(k_in._center) {}
...
public:
  kernel_1d() {}
  kernel_1d(std::size_t size_in, std::size_t center_in)
      : parent_t(size_in, center_in) {}
  template <typename FwdIterator>
  kernel_1d(FwdIterator elements, std::size_t size_in, std::size_t center_in)
      : parent_t(size_in, center_in) {
...
public:
  ...
  template <typename FwdIterator>
  explicit kernel_1d_fixed(FwdIterator elements, std::size_t center_in)
      : parent_t(center_in) {
...
template <typename Kernel> inline Kernel reverse_kernel(const Kernel &kernel) {
