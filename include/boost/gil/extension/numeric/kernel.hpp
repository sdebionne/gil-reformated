...
public:
  ...
  explicit kernel_2d_adaptor(std::size_t center_vertical,
  ...
  kernel_2d_adaptor(std::size_t size, std::size_t center_vertical,
                    std::size_t center_horizontal)
      : Core(size * size), square_size(size),
        center_(center_horizontal, center_vertical) {
    BOOST_ASSERT(this->size() > 0);
    BOOST_ASSERT(center_.y < this->size() &&
                 center_.x < this->size()); // also implies `size() > 0`
  }

  kernel_2d_adaptor(kernel_2d_adaptor const &other)
      : Core(other), square_size(other.square_size),
  ...
  std::size_t size() const { return square_size; }
...
public:
  kernel_2d() = default;
  kernel_2d(std::size_t size, std::size_t vertical_center,
            std::size_t center_horizontal)
      : parent_t(size, vertical_center, center_horizontal) {}

  template <typename FwdIterator>
  kernel_2d(FwdIterator elements, std::size_t size, std::size_t vertical_center,
            std::size_t center_horizontal)
      : parent_t(static_cast<int>(std::sqrt(size)), vertical_center,
                 center_horizontal) {
    detail::copy_n(elements, size, this->begin());
  }
...
public:
  ...
  kernel_2d_fixed() { this->square_size = Size; }

  explicit kernel_2d_fixed(std::size_t center_vertical,
                           std::size_t center_horizontal)
  ...
  template <typename FwdIterator>
  explicit kernel_2d_fixed(FwdIterator elements, std::size_t center_vertical,
                           std::size_t center_horizontal)
      : parent_t(center_vertical, center_horizontal) {
    this->square_size = Size;
