...
struct pixel_divides_scalar_t {
  PixelR operator()(const PixelRef &p, const Scalar &s) const {
    ...
    static_transform(
        p, result,
        std::bind(
            channel_divides_scalar_t<typename channel_type<PixelRef>::type,
                                     Scalar,
                                     typename channel_type<PixelR>::type>(),
            std::placeholders::_1, s));
    return result;
...
/// \ingroup PixelNumericOperations
/// definition and a generic implementation for casting and assigning a pixel to
/// another user should specialize it for better performance
template <typename PixelRef,  // models pixel concept
