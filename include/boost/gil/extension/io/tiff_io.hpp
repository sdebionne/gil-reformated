...
template <> struct tiff_read_support_private<bits8, gray_t> {
  BOOST_STATIC_CONSTANT(bool, is_supported = true);
  BOOST_STATIC_CONSTANT(int, bit_depth = 8);
  BOOST_STATIC_CONSTANT(int, color_type = PHOTOMETRIC_MINISBLACK);
};
template <> struct tiff_read_support_private<bits8, rgb_t> {
  BOOST_STATIC_CONSTANT(bool, is_supported = true);
  BOOST_STATIC_CONSTANT(int, bit_depth = 8);
  BOOST_STATIC_CONSTANT(int, color_type = PHOTOMETRIC_RGB);
};
template <> struct tiff_read_support_private<bits16, gray_t> {
  BOOST_STATIC_CONSTANT(bool, is_supported = true);
  BOOST_STATIC_CONSTANT(int, bit_depth = 16);
  BOOST_STATIC_CONSTANT(int, color_type = PHOTOMETRIC_MINISBLACK);
};
template <> struct tiff_read_support_private<bits16, rgb_t> {
  BOOST_STATIC_CONSTANT(bool, is_supported = true);
  BOOST_STATIC_CONSTANT(int, bit_depth = 16);
  BOOST_STATIC_CONSTANT(int, color_type = PHOTOMETRIC_RGB);
};
template <> struct tiff_read_support_private<bits32f, gray_t> {
  BOOST_STATIC_CONSTANT(bool, is_supported = true);
  BOOST_STATIC_CONSTANT(int, bit_depth = 32);
  BOOST_STATIC_CONSTANT(int, color_type = PHOTOMETRIC_MINISBLACK);
};
template <> struct tiff_read_support_private<bits32f, rgb_t> {
  BOOST_STATIC_CONSTANT(bool, is_supported = true);
  BOOST_STATIC_CONSTANT(int, bit_depth = 32);
  BOOST_STATIC_CONSTANT(int, color_type = PHOTOMETRIC_RGB);
};
...
template <> struct tiff_write_support_private<bits8, gray_t> {
  BOOST_STATIC_CONSTANT(bool, is_supported = true);
  BOOST_STATIC_CONSTANT(int, bit_depth = 8);
  BOOST_STATIC_CONSTANT(int, color_type = PHOTOMETRIC_MINISBLACK);
};
template <> struct tiff_write_support_private<bits8, rgb_t> {
  BOOST_STATIC_CONSTANT(bool, is_supported = true);
  BOOST_STATIC_CONSTANT(int, bit_depth = 8);
  BOOST_STATIC_CONSTANT(int, color_type = PHOTOMETRIC_RGB);
};
template <> struct tiff_write_support_private<bits16, gray_t> {
  BOOST_STATIC_CONSTANT(bool, is_supported = true);
  BOOST_STATIC_CONSTANT(int, bit_depth = 16);
  BOOST_STATIC_CONSTANT(int, color_type = PHOTOMETRIC_MINISBLACK);
};
template <> struct tiff_write_support_private<bits16, rgb_t> {
  BOOST_STATIC_CONSTANT(bool, is_supported = true);
  BOOST_STATIC_CONSTANT(int, bit_depth = 16);
  BOOST_STATIC_CONSTANT(int, color_type = PHOTOMETRIC_RGB);
};
template <> struct tiff_write_support_private<bits32f, gray_t> {
  BOOST_STATIC_CONSTANT(bool, is_supported = true);
  BOOST_STATIC_CONSTANT(int, bit_depth = 32);
  BOOST_STATIC_CONSTANT(int, color_type = PHOTOMETRIC_MINISBLACK);
};
template <> struct tiff_write_support_private<bits32f, rgb_t> {
  BOOST_STATIC_CONSTANT(bool, is_supported = true);
  BOOST_STATIC_CONSTANT(int, bit_depth = 32);
  BOOST_STATIC_CONSTANT(int, color_type = PHOTOMETRIC_RGB);
};
...
public:
  ...
  template <typename View> void apply(const View &view) {
    ...
    std::size_t element_size =
        sizeof(pixel<typename channel_type<View>::type,
                     layout<typename color_space_type<View>::type>>);
    std::size_t size_to_allocate = std::max(
        (std::size_t)view.width(),
  ...
  template <typename Image> void read_image(Image &im) {
...
template <typename CC> class tiff_reader_color_convert : public tiff_reader {
private:
  CC _cc;

public:
  tiff_reader_color_convert(const char *filename) : tiff_reader(filename) {}
  ...
  template <typename View> void apply(const View &view) {
    ...
    io_error_if(dims != view.dimensions(),
                "tiff_reader_color_convert::apply(): input view size does not "
                "match TIFF file size");
    switch (photometric) {
    case PHOTOMETRIC_MINISBLACK: {
      ...
      case 8: {
        ...
        std::size_t size_to_allocate =
            std::max((std::size_t)view.width(),
                     (std::size_t)(TIFFScanlineSize(_tp) + element_size - 1) /
                         element_size);
        std::vector<gray8_pixel_t> row(size_to_allocate);
        for (int y = 0; y < view.height(); ++y) {
          io_error_if(TIFFReadScanline(_tp, &row.front(), y) != 1);
          std::transform(
              row.begin(), row.begin() + view.width(), view.row_begin(y),
              color_convert_deref_fn<gray8_ref_t, typename View::value_type,
                                     CC>(_cc));
        }
        break;
      }
      case 16: {
        ...
        for (int y = 0; y < view.height(); ++y) {
          ...
          std::transform(
              row.begin(), row.begin() + view.width(), view.row_begin(y),
              color_convert_deref_fn<gray16_ref_t, typename View::value_type,
                                     CC>(_cc));
        }
        break;
      }
      case 32: {
        ...
        for (int y = 0; y < view.height(); ++y) {
          ...
          std::transform(
              row.begin(), row.begin() + view.width(), view.row_begin(y),
              color_convert_deref_fn<gray32f_ref_t, typename View::value_type,
                                     CC>(_cc));
        }
        break;
      }
      default:
        io_error("tiff_reader_color_convert::apply(): unknown combination of "
    ...
    case PHOTOMETRIC_RGB: {
      ...
      case 8: {
        ...
        for (int y = 0; y < view.height(); ++y) {
          ...
          std::transform(
              row.begin(), row.begin() + view.width(), view.row_begin(y),
              color_convert_deref_fn<rgb8_ref_t, typename View::value_type, CC>(
                  _cc));
        }
        break;
      }
      case 16: {
        ...
        for (int y = 0; y < view.height(); ++y) {
          ...
          std::transform(
              row.begin(), row.begin() + view.width(), view.row_begin(y),
              color_convert_deref_fn<rgb16_ref_t, typename View::value_type,
                                     CC>(_cc));
        }
        break;
      }
      case 32: {
        ...
        for (int y = 0; y < view.height(); ++y) {
          ...
          std::transform(
              row.begin(), row.begin() + view.width(), view.row_begin(y),
              color_convert_deref_fn<rgb32f_ref_t, typename View::value_type,
                                     CC>(_cc));
        }
        break;
      }
...
public:
  ...
  template <typename View> void apply(const View &view) {
    ...
    io_error_if(
        TIFFSetField(_tp, TIFFTAG_PHOTOMETRIC,
                     tiff_write_support_private<
                         typename channel_type<View>::type,
                         typename color_space_type<View>::type>::color_type) !=
        1);
    io_error_if(TIFFSetField(_tp, TIFFTAG_RESOLUTIONUNIT, RESUNIT_NONE) != 1);
    io_error_if(TIFFSetField(_tp, TIFFTAG_PLANARCONFIG, PLANARCONFIG_CONTIG) !=
                1);
    io_error_if(TIFFSetField(_tp, TIFFTAG_ORIENTATION, ORIENTATION_TOPLEFT) !=
                1);
    io_error_if(TIFFSetField(_tp, TIFFTAG_SAMPLESPERPIXEL,
                             num_channels<View>::value) != 1);
    io_error_if(
        TIFFSetField(_tp, TIFFTAG_BITSPERSAMPLE,
                     tiff_write_support_private<
                         typename channel_type<View>::type,
                         typename color_space_type<View>::type>::bit_depth) !=
        1);
    io_error_if(TIFFSetField(_tp, TIFFTAG_ROWSPERSTRIP,
                             TIFFDefaultStripSize(_tp, 0)) != 1);
    std::vector<pixel<typename channel_type<View>::type,
                      layout<typename color_space_type<View>::type>>>
        row(view.width());
    for (int y = 0; y < view.height(); ++y) {
...
template <typename View> struct tiff_read_support {
  BOOST_STATIC_CONSTANT(
      bool,
      is_supported = (detail::tiff_read_support_private<
                      typename channel_type<View>::type,
                      typename color_space_type<View>::type>::is_supported));
  BOOST_STATIC_CONSTANT(
      int, bit_depth = (detail::tiff_read_support_private<
                        typename channel_type<View>::type,
                        typename color_space_type<View>::type>::bit_depth));
  BOOST_STATIC_CONSTANT(
      int, color_type = (detail::tiff_read_support_private<
                         typename channel_type<View>::type,
                         typename color_space_type<View>::type>::color_type));
};

/// \ingroup TIFF_IO
/// \brief Returns the width and height of the TIFF file at the specified
/// location. Throws std::ios_base::failure if the location does not correspond
/// to a valid TIFF file
...
/// location. Throws std::ios_base::failure if the location does not correspond
/// to a valid TIFF file
inline point2<std::ptrdiff_t>
tiff_read_dimensions(const std::string &filename) {
  return tiff_read_dimensions(filename.c_str());
}

/// \ingroup TIFF_IO
/// \brief Loads the image specified by the given tiff image file name into the
/// given view. Triggers a compile assert if the view color space and channel
/// depth are not supported by the TIFF library or by the I/O extension. Throws
/// std::ios_base::failure if the file is not a valid TIFF file, or if its color
/// space or channel depth are not compatible with the ones specified by View,
/// or if its dimensions don't match the ones of the view.
...
/// \ingroup TIFF_IO
/// \brief Allocates a new image whose dimensions are determined by the given
/// tiff image file, and loads the pixels into it. Triggers a compile assert if
/// the image color space or channel depth are not supported by the TIFF library
/// or by the I/O extension. Throws std::ios_base::failure if the file is not a
/// valid TIFF file, or if its color space or channel depth are not compatible
/// with the ones specified by Image
...
/// \ingroup TIFF_IO
/// \brief Allocates a new image whose dimensions are determined by the given
/// tiff image file, and loads the pixels into it.
template <typename Image>
...
/// \ingroup TIFF_IO
/// \brief Loads and color-converts the image specified by the given tiff image
/// file name into the given view. Throws std::ios_base::failure if the file is
/// not a valid TIFF file, or if its dimensions don't match the ones of the
/// view.
...
template <typename View>
inline void tiff_read_and_convert_view(const char *filename, const View &view) {
  detail::tiff_reader_color_convert<default_color_converter> m(
      filename, default_color_converter());
  m.apply(view);
}

/// \ingroup TIFF_IO
/// \brief Loads and color-converts the image specified by the given tiff image
/// file name into the given view.
template <typename View, typename CC>
inline void tiff_read_and_convert_view(const std::string &filename,
                                       const View &view, CC cc) {
...
/// \ingroup TIFF_IO
/// \brief Allocates a new image whose dimensions are determined by the given
/// tiff image file, loads and color-converts the pixels into it. Throws
/// std::ios_base::failure if the file is not a valid TIFF file
...
/// \ingroup TIFF_IO
/// \brief Allocates a new image whose dimensions are determined by the given
/// tiff image file, loads and color-converts the pixels into it.
template <typename Image, typename CC>
...
/// tiff image file, loads and color-converts the pixels into it.
template <typename Image>
inline void tiff_read_and_convert_image(const std::string &filename,
                                        Image &im) {
...
template <typename View> struct tiff_write_support {
  BOOST_STATIC_CONSTANT(
      bool,
      is_supported = (detail::tiff_write_support_private<
                      typename channel_type<View>::type,
                      typename color_space_type<View>::type>::is_supported));
  BOOST_STATIC_CONSTANT(
      int, bit_depth = (detail::tiff_write_support_private<
                        typename channel_type<View>::type,
                        typename color_space_type<View>::type>::bit_depth));
  BOOST_STATIC_CONSTANT(
      int, color_type = (detail::tiff_write_support_private<
                         typename channel_type<View>::type,
                         typename color_space_type<View>::type>::color_type));
...
/// \ingroup TIFF_IO
/// \brief Saves the view to a tiff file specified by the given tiff image file
/// name. Triggers a compile assert if the view color space and channel depth
/// are not supported by the TIFF library or by the I/O extension. Throws
