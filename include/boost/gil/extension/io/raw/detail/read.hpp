...
#define BUILD_INTERLEAVED_VIEW(color_layout, bits_per_pixel)                   \
  {                                                                            \
    color_layout##bits_per_pixel##_view_t build =                              \
...
private:
  typedef reader<Device, raw_tag, ConversionPolicy> this_t;
...
public:
  ...
  template <typename View> void apply(const View &dst_view) {
    ...
    this->_info._unpack_function_name =
        this->_io_dev.get_unpack_function_name();

    return_code = this->_io_dev.dcraw_process();
    io_error_if(return_code != LIBRAW_SUCCESS,
                "Unable to emulate dcraw behavior to process image");

    libraw_processed_image_t *processed_image =
        this->_io_dev.dcraw_make_mem_image(&return_code);
    io_error_if(return_code != LIBRAW_SUCCESS,
                "Unable to dcraw_make_mem_image");
    ...
    // Here, we should use a metafunction to reduce code size and avoid a
    // (compile time) macro
...
struct raw_read_is_supported {
  ...
  struct apply
      : public is_read_supported<typename get_pixel_type<View>::type, raw_tag> {
  };
};
...
class dynamic_image_reader<Device, raw_tag>
  ...
  typedef reader<Device, raw_tag, detail::read_and_no_convert> parent_t;
...
public:
  ...
  template <typename Images> void apply(any_image<Images> &images) {
    ...
    if (!construct_matched(images, format_checker)) {
      ...
      error_message << "No matching image type between those of the given "
                       "any_image and that of the file.\n";
      error_message << "Image type must be {gray||rgb}{8||16} unsigned for RAW "
                       "image files.";
      ...
      detail::dynamic_io_fnobj<detail::raw_read_is_supported, parent_t> op(
          this);
