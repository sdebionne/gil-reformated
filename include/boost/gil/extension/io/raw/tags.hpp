...
// Historically, LibRaw expects WIN32, not _WIN32 (see
...
#endif
#pragma warning(push)
#pragma warning(disable : 4251) // 'type' needs to have dll-interface to be used
                                // by clients of class
#endif
...
template <> struct image_read_info<raw_tag> {
  /// Default contructor.
  image_read_info<raw_tag>() : _valid(false) {}

  // Here, width and height of the image are the ones of the output height (ie
  // after having been processed by dcraw emulator)
...
template <> struct image_write_info<raw_tag> {};
