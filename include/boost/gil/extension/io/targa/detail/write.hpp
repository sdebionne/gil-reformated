...
public:
  ...
  template <typename View> void apply(const View &view) { write(view); }
...
private:
  template <typename View> void write(const View &view) {
    ...
    this->_io_dev.write_uint16(
        static_cast<uint16_t>(view.width())); // width in pixels
    this->_io_dev.write_uint16(
        static_cast<uint16_t>(view.height())); // height in pixels
    ...
    write_image<View, typename detail::get_targa_view_type<
                          num_channels<View>::value>::type>(view);
  }
  ...
  void write_image(const View &view) {
    ...
    for (typename View::y_coord_t y = view.height() - 1; y > -1; --y) {
      copy_pixels(subimage_view(view, 0, static_cast<int>(y),
                                static_cast<int>(view.width()), 1),
                  row);
...
public:
  ...
  void apply(const any_image_view<Views...> &views) {
    detail::dynamic_io_fnobj<detail::targa_write_is_supported, parent_t> op(
        this);
