...
// template <typename Images> void png_read_image(const
// char*,any_image<Images>&) template <typename Images> void
// png_read_image(FILE*,any_image<Images>&,std::size_t) template <typename
// Views> void png_write_view(const char*,const any_image_view<View>&) template
// <typename Views> void png_write_view(FILE*,const any_image_view<View>&)
...
struct png_write_is_supported {
  ...
  struct apply : public mpl::bool_<png_write_support<View>::is_supported> {};
};
...
struct png_read_is_supported {
  template <typename View>
  struct apply : public mpl::bool_<png_read_support<View>::is_supported> {};
};
...
/// Opens the given png file name, selects the first type in Images whose color
...
/// \ingroup PNG_IO
/// \brief Saves the currently instantiated view to a png file specified by the
/// given png image file name. Throws std::ios_base::failure if the currently
/// instantiated view type is not supported for writing by the I/O extension or
/// if it fails to create the file.
template <typename Views>
inline void png_write_view(const char *filename,
...
/// \ingroup PNG_IO
/// \brief Saves the currently instantiated view to a png file specified by the
/// given png image file name.
template <typename Views>
inline void png_write_view(const std::string &filename,
                           const any_image_view<Views> &runtime_view) {
