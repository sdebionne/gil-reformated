...
public:
  ...
  reader(const Device &io_dev, const image_read_settings<bmp_tag> &settings)
      : backend_t(io_dev, settings), _pitch(0) {}

  //
  // Constructor
  //
  reader(const Device &io_dev, const ConversionPolicy &cc,
         const image_read_settings<bmp_tag> &settings)
      : reader_base<bmp_tag, ConversionPolicy>(cc), backend_t(io_dev, settings),
        _pitch(0) {}
  ...
  template <typename View> void apply(const View &dst_view) {
    ...
    if (this->_info._bits_per_pixel < 8) {
      _pitch = static_cast<long>(
          ((this->_info._width * this->_info._bits_per_pixel) + 7) >> 3);
    } else {
      _pitch = static_cast<long>(this->_info._width *
                                 ((this->_info._bits_per_pixel + 7) >> 3));
    }
    ...
    case 1: {
      ...
      read_palette_image<gray1_image_t::view_t,
                         detail::mirror_bits<byte_vector_t, mpl::true_>>(
          dst_view);
      break;
    }
    ...
    case 4: {
      ...
      case bmp_compression::_rgb: {
        ...
        read_palette_image<gray4_image_t::view_t,
                           detail::swap_half_bytes<byte_vector_t, mpl::true_>>(
            dst_view);
        break;
      }
...
private:
  ...
  template <typename View> void read_data_15(const View &view) {
  ...
  void read_data(const View_Dst &view) {
    ...
    typename View_Src::x_iterator beg =
        v.row_begin(0) + this->_settings._top_left.x;
  ...
  void copy_row_if_needed(const Buffer &buf, const View &view,
                          std::ptrdiff_t y) {
    if (y >= this->_settings._top_left.y && y < this->_settings._dim.y) {
      typename Buffer::const_iterator beg =
          buf.begin() + this->_settings._top_left.x;
...
public:
  ...
  template <typename Image> bool apply() {
    if (_bpp < 32) {
      return pixels_are_compatible<typename Image::value_type,
                                   rgb8_pixel_t>::value
                 ? true
                 : false;
    } else {
...
struct bmp_read_is_supported {
  ...
  struct apply
      : public is_read_supported<typename get_pixel_type<View>::type, bmp_tag> {
  };
};
...
public:
  ...
  template <typename Images> void apply(any_image<Images> &images) {
    ...
    if (!construct_matched(images, format_checker)) {
      ...
      detail::dynamic_io_fnobj<detail::bmp_read_is_supported, parent_t> op(
          this);
