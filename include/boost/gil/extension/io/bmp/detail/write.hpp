...
public:
  writer(const Device &io_dev, const image_write_info<bmp_tag> &info)
...
private:
  ...
  template <typename View> void write(const View &view) {
    ...
    /*
            /// todo
            if( bmp_read_write_support_private<channel_t,
       color_space_t>::channel != 8)
            {
    ...
    */
    ...
    /*
            /// @todo: Not supported for now. bit_aligned_images refer to
       indexed images
            ///        in this context.
            if( bpp <= 8 )
            {
    ...
    */
    ...
    std::size_t ofs = bmp_header_size::_size +
                      bmp_header_size::_win32_info_size + entries * 4;
...
class dynamic_image_writer<Device, bmp_tag> : public writer<Device, bmp_tag> {
  typedef writer<Device, bmp_tag> parent_t;
