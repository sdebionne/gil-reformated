...
struct jpeg_write_is_supported {
  ...
  struct apply : public mpl::bool_<jpeg_write_support<View>::is_supported> {};
};
...
public:
  jpeg_writer_dynamic(FILE *file, int quality = 100)
      : jpeg_writer(file), _quality(quality) {}
  jpeg_writer_dynamic(const char *filename, int quality = 100)
      : jpeg_writer(filename), _quality(quality) {}
...
struct jpeg_read_is_supported {
  template <typename View>
  struct apply : public mpl::bool_<jpeg_read_support<View>::is_supported> {};
};
...
public:
  ...
  template <typename Images> void read_image(any_image<Images> &im) {
    if (!construct_matched(
            im, detail::jpeg_type_format_checker(_cinfo.out_color_space))) {
      io_error("jpeg_reader_dynamic::read_image(): no matching image type "
               "between those of the given any_image and that of the file");
    } else {
...
/// Opens the given JPEG file name, selects the first type in Images whose color
/// space and channel are compatible to those of the image file and creates a
/// new image of that type with the dimensions specified by the image file.
/// Throws std::ios_base::failure if none of the types in Images are compatible
/// with the type on disk.
...
/// \brief reads a JPEG image into a run-time instantiated image
template <typename Images>
inline void jpeg_read_image(const std::string &filename,
                            any_image<Images> &im) {
  jpeg_read_image(filename.c_str(), im);
}

/// \ingroup JPEG_IO
/// \brief Saves the currently instantiated view to a jpeg file specified by the
/// given jpeg image file name. Throws std::ios_base::failure if the currently
/// instantiated view type is not supported for writing by the I/O extension or
/// if it fails to create the file.
template <typename Views>
inline void jpeg_write_view(const char *filename,
