...
// template <typename Images> void tiff_read_image(const
// char*,any_image<Images>) template <typename Views> void tiff_write_view(const
// char*,any_image_view<Views>)
//
...
struct tiff_write_is_supported {
  ...
  struct apply : public mpl::bool_<tiff_write_support<View>::is_supported> {};
};
...
struct tiff_read_is_supported {
  template <typename View>
  struct apply : public mpl::bool_<tiff_read_support<View>::is_supported> {};
};
...
public:
  ...
  template <typename Images> void read_image(any_image<Images> &im) {
    ...
    if (!construct_matched(im, tiff_type_format_checker(bps, photometric))) {
      io_error("tiff_reader_dynamic::read_image(): no matching image type "
...
/// Opens the given tiff file name, selects the first type in Images whose color
...
/// \brief reads a TIFF image into a run-time instantiated image
template <typename Images>
inline void tiff_read_image(const std::string &filename,
                            any_image<Images> &im) {
  tiff_read_image(filename.c_str(), im);
}

/// \ingroup TIFF_IO
/// \brief Saves the currently instantiated view to a tiff file specified by the
/// given tiff image file name. Throws std::ios_base::failure if the currently
/// instantiated view type is not supported for writing by the I/O extension or
/// if it fails to create the file.
template <typename Views>
inline void tiff_write_view(const char *filename,
...
/// \ingroup TIFF_IO
/// \brief Saves the currently instantiated view to a tiff file specified by the
/// given tiff image file name.
template <typename Views>
inline void tiff_write_view(const std::string &filename,
                            const any_image_view<Views> &runtime_view) {
