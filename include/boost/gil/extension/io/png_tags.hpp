...
/// PNG info base class. Containing all header information both for reading and
/// writing.
///
...
struct png_info_base {
  ...
  png_info_base()
      : _width(0), _height(0)
        ...
        ,
        _bit_depth(0), _color_type(0), _interlace_method(PNG_INTERLACE_NONE),
...
#ifdef BOOST_GIL_IO_PNG_FLOATING_POINT_SUPPORTED
        ,
        _valid_cie_colors(0), _white_x(0.0), _white_y(0.0), _red_x(0.0),
        _red_y(0.0), _green_x(0.0), _green_y(0.0), _blue_x(0.0), _blue_y(0.0)

        ,
        _valid_file_gamma(0), _file_gamma(1.0)
#else
        ,
        _valid_cie_colors(0), _white_x(0), _white_y(0), _red_x(0), _red_y(0),
        _green_x(0), _green_y(0), _blue_x(0), _blue_y(0)

        ,
        _valid_file_gamma(0), _file_gamma(1)
#endif // BOOST_GIL_IO_PNG_FLOATING_POINT_SUPPORTED

        ,
        _valid_icc_profile(0), _icc_name(),
        _iccp_compression_type(PNG_COMPRESSION_TYPE_BASE), _profile(),
        _profile_length(0)

        ,
        _valid_intent(0), _intent(0)

        ,
        _valid_palette(0), _palette(), _num_palette(0)

        ,
        _valid_background(0), _background()

        ,
        _valid_histogram(0), _histogram()

        ,
        _valid_offset(0), _offset_x(0), _offset_y(0),
        _off_unit_type(PNG_OFFSET_PIXEL)

        ,
        _valid_pixel_calibration(0), _purpose(), _X0(0), _X1(0), _cal_type(0),
        _num_params(0), _units(), _params()

        ,
        _valid_resolution(0), _res_x(0), _res_y(0),
        ...
        ,
        _valid_significant_bits(0), _sig_bits()

        ,
        _valid_scale_factors(0), _scale_unit(PNG_SCALE_UNKNOWN)
#ifdef BOOST_GIL_IO_PNG_FLOATING_POINT_SUPPORTED
        ,
        _scale_width(0.0), _scale_height(0.0)
#else
#ifdef BOOST_GIL_IO_PNG_FIXED_POINT_SUPPORTED
        ,
        _scale_width(0), _scale_height(0)
#else
        ,
        ...
        ,
        _valid_text(0), _text(), _num_text(0)

        ,
        _valid_modification_time(0), _mod_time()

        ,
        _valid_transparency_factors(0), _trans(), _num_trans(0),
        _trans_values() {
  }
...
template <> struct image_read_info<png_tag> : public png_info_base {
  /// Default constructor.
...
struct image_read_settings<png_tag> : public image_read_settings_base,
  ...
  image_read_settings<png_tag>()
      : image_read_settings_base(), png_read_settings_base(),
        _screen_gamma(1.0) {}
  ...
  image_read_settings(const point_t &top_left, const point_t &dim,
      ...
      : image_read_settings_base(top_left, dim), png_read_settings_base(),
        _apply_screen_gamma(apply_screen_gamma), _screen_gamma(screen_gamma) {}
...
struct image_read_settings<png_tag> : public image_read_settings_base,
  ...
  image_read_settings<png_tag>()
      : image_read_settings_base(), png_read_settings_base(),
        _apply_screen_gamma(false), _screen_gamma(2) {}
...
template <> struct image_write_info<png_tag> : public png_info_base {
  image_write_info(
      const png_compression_type::type compression_type =
          png_compression_type::default_value,
      const png_compression_level::type compression_level =
          png_compression_level::default_value,
      ...
      : png_info_base(), _compression_type(compression_type),
