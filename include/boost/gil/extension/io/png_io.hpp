...
/// \ingroup PNG_IO
/// \brief Returns the width and height of the PNG file at the specified
...
template <typename View> struct png_read_support {
  BOOST_STATIC_CONSTANT(
      bool,
      is_supported = (detail::png_read_support_private<
                      typename channel_type<View>::type,
                      typename color_space_type<View>::type>::is_supported));
  BOOST_STATIC_CONSTANT(
      int, bit_depth = (detail::png_read_support_private<
                        typename channel_type<View>::type,
                        typename color_space_type<View>::type>::bit_depth));
  BOOST_STATIC_CONSTANT(
      int, color_type = (detail::png_read_support_private<
                         typename channel_type<View>::type,
...
/// \ingroup PNG_IO
/// \brief Loads the image specified by the given png image file name into the
/// given view. Triggers a compile assert if the view color space and channel
/// depth are not supported by the PNG library or by the I/O extension. Throws
/// std::ios_base::failure if the file is not a valid PNG file, or if its color
...
/// png image file, and loads the pixels into it. Triggers a compile assert if
/// the image color space or channel depth are not supported by the PNG library
/// or by the I/O extension. Throws std::ios_base::failure if the file is not a
/// valid PNG file, or if its color space or channel depth are not compatible
...
/// \ingroup PNG_IO
/// \brief Allocates a new image whose dimensions are determined by the given
/// png image file, and loads the pixels into it.
template <typename Image>
...
/// \ingroup PNG_IO
/// \brief Loads the image specified by the given png image file name and
/// color-converts it into the given view. Throws std::ios_base::failure if the
/// file is not a valid PNG file, or if its dimensions don't match the ones of
/// the view.
...
template <typename View>
inline void png_read_and_convert_view(const char *filename, const View &view) {
  detail::png_reader_color_convert<default_color_converter> m(
...
/// \ingroup PNG_IO
/// \brief Loads the image specified by the given png image file name and
/// color-converts it into the given view.
template <typename View, typename CC>
inline void png_read_and_convert_view(const std::string &filename,
...
/// \ingroup PNG_IO
/// \brief Allocates a new image whose dimensions are determined by the given
/// png image file, loads and color-converts the pixels into it. Throws
/// std::ios_base::failure if the file is not a valid PNG file
...
/// \ingroup PNG_IO
/// \brief Allocates a new image whose dimensions are determined by the given
/// png image file, loads and color-converts the pixels into it.
template <typename Image, typename CC>
...
template <typename View> struct png_write_support {
  BOOST_STATIC_CONSTANT(
      bool,
      is_supported = (detail::png_write_support_private<
                      typename channel_type<View>::type,
                      typename color_space_type<View>::type>::is_supported));
  BOOST_STATIC_CONSTANT(
      int, bit_depth = (detail::png_write_support_private<
                        typename channel_type<View>::type,
                        typename color_space_type<View>::type>::bit_depth));
  BOOST_STATIC_CONSTANT(
      int, color_type = (detail::png_write_support_private<
                         typename channel_type<View>::type,
                         typename color_space_type<View>::type>::color_type));
  BOOST_STATIC_CONSTANT(bool, value = is_supported);
};
...
/// \ingroup PNG_IO
/// \brief Saves the view to a png file specified by the given png image file
/// name. Triggers a compile assert if the view color space and channel depth
/// are not supported by the PNG library or by the I/O extension. Throws
