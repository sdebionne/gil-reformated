...
template <> struct image_read_info<jpeg_tag> {
  image_read_info()
      : _width(0), _height(0)
        ...
        ,
        _density_unit(0), _x_density(0), _y_density(0)

        ,
        _pixel_width_mm(0.0), _pixel_height_mm(0.0) {}
...
struct image_read_settings<jpeg_tag> : public image_read_settings_base {
  ...
  image_read_settings(
      const point_t &top_left, const point_t &dim,
      jpeg_dct_method::type dct_method = jpeg_dct_method::default_value)
      : image_read_settings_base(top_left, dim), _dct_method(dct_method) {}
...
template <> struct image_write_info<jpeg_tag> {
  /// Constructor
  ...
  image_write_info(
      ...
      const jpeg_density_unit::type density_unit =
          jpeg_density_unit::default_value,
      const jpeg_pixel_density::type x_density =
          jpeg_pixel_density::default_value,
      const jpeg_pixel_density::type y_density =
          jpeg_pixel_density::default_value)
      : _quality(quality), _dct_method(dct_method)

        ,
        _density_unit(density_unit), _x_density(x_density),
...
private:
  UINT16 round(double d) { return static_cast<UINT16>(d + 0.5); }
};
