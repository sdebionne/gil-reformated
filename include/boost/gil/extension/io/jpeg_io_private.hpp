...
// lbourdev: What is the advantage of having channel and colorspace together?
// Are there cases where they are interrelated?
...
template <> struct jpeg_read_support_private<bits8, rgb_t> {
  BOOST_STATIC_ASSERT(BITS_IN_JSAMPLE == 8);
  BOOST_STATIC_CONSTANT(bool, is_supported = true);
  BOOST_STATIC_CONSTANT(J_COLOR_SPACE, color_type = JCS_RGB);
};
template <> struct jpeg_read_support_private<bits8, cmyk_t> {
  BOOST_STATIC_ASSERT(BITS_IN_JSAMPLE == 8);
  BOOST_STATIC_CONSTANT(bool, is_supported = true);
...
template <> struct jpeg_write_support_private<bits8, rgb_t> {
  BOOST_STATIC_ASSERT(BITS_IN_JSAMPLE == 8);
  BOOST_STATIC_CONSTANT(bool, is_supported = true);
  BOOST_STATIC_CONSTANT(J_COLOR_SPACE, color_type = JCS_RGB);
};
template <> struct jpeg_write_support_private<bits8, cmyk_t> {
  BOOST_STATIC_ASSERT(BITS_IN_JSAMPLE == 8);
  BOOST_STATIC_CONSTANT(bool, is_supported = true);
  BOOST_STATIC_CONSTANT(J_COLOR_SPACE, color_type = JCS_CMYK);
};
...
public:
  ...
  template <typename View> void apply(const View &view) {
    jpeg_start_decompress(
        &_cinfo); // lbourdev: Can this return an error? You need to check and
                  // throw. Check all other library methods that can return an
                  // error state...
    io_error_if(_cinfo.data_precision != 8,
                "jpeg_reader::apply(): this image file is not supported");
    io_error_if(
        _cinfo.out_color_space !=
            jpeg_read_support_private<
    ...
    io_error_if(view.dimensions() != get_dimensions(),
                "jpeg_reader::apply(): input view dimensions do not match the "
                "image file");
    std::vector<pixel<bits8, layout<typename color_space_type<View>::type>>>
        row(view.width());
    JSAMPLE *row_address = (JSAMPLE *)&row.front();
    for (int y = 0; y < view.height(); ++y) {
      io_error_if(jpeg_read_scanlines(&_cinfo, (JSAMPARRAY)&row_address, 1) !=
                      1,
...
template <typename CC> class jpeg_reader_color_convert : public jpeg_reader {
private:
  CC _cc;

public:
  jpeg_reader_color_convert(FILE *file, CC cc_in)
      : jpeg_reader(file), _cc(cc_in) {}
  jpeg_reader_color_convert(FILE *file) : jpeg_reader(file) {}
  jpeg_reader_color_convert(const char *filename, CC cc_in)
      : jpeg_reader(filename), _cc(cc_in) {}
  ...
  template <typename View> void apply(const View &view) {
    jpeg_start_decompress(
        &_cinfo); // lbourdev: Can this return an error? You need to check and
                  ...
                  // error state...
    io_error_if(
        _cinfo.data_precision != 8,
        "jpeg_reader_color_covert::apply(): this image file is not supported");
    io_error_if(view.dimensions() != get_dimensions(),
                "jpeg_reader_color_covert::apply(): input view dimensions "
                "don't match the image file");
    ...
    case JCS_GRAYSCALE: {
      ...
      for (int y = 0; y < view.height(); ++y) {
        ...
        std::transform(
            ...
            color_convert_deref_fn<gray8_ref_t, typename View::value_type, CC>(
                _cc));
      }
      break;
    }
    ...
    case JCS_CMYK: {
      ...
      for (int y = 0; y < view.height(); ++y) {
        ...
        std::transform(
            row.begin(), row.end(), view.row_begin(y),
            color_convert_deref_fn<cmyk8_ref_t, typename View::value_type, CC>(
                _cc));
      }
      break;
    }
...
public:
  ...
  template <typename View> void apply(const View &view, int quality = 100) {
    ...
    _cinfo.in_color_space = jpeg_write_support_private<
        typename channel_type<View>::type,
        typename color_space_type<View>::type>::color_type;
    ...
    for (int y = 0; y < view.height(); ++y) {
      ...
      io_error_if(jpeg_write_scanlines(&_cinfo, (JSAMPARRAY)&row_address, 1) !=
                      1,
