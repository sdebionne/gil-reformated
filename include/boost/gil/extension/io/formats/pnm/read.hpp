...
#include <boost/gil/extension/io/pnm_tags.hpp>
...
private:
  typedef reader<Device, pnm_tag, ConversionPolicy> this_t;
...
public:
  reader(const Device &io_dev, const image_read_settings<pnm_tag> &settings)
      : reader_base<pnm_tag, ConversionPolicy>(), backend_t(io_dev, settings) {}

  reader(const Device &io_dev, const cc_t &cc,
...
private:
  ...
  template <typename View_Src, typename View_Dst>
  void read_text_row(const View_Dst &dst, byte_vector_t &row,
                     typename View_Dst::y_coord_t y, bool process) {
    View_Src src = interleaved_view(
        this->_info._width, 1, (typename View_Src::value_type *)&row.front(),
        this->_scanline_length);
      ...
      if (process) {
        ...
        if (this->_info._max_value == 1) {
          typedef typename channel_type<
              typename get_pixel_type<View_Dst>::type>::type channel_t;
  ...
  void read_bin_data(const View_Dst &view) {
    typedef typename View_Dst::y_coord_t y_t;
    typedef typename is_bit_aligned<typename View_Src::value_type>::type
        is_bit_aligned_t;
    ...
    detail::negate_bits<typename rh_t::buffer_t, is_bit_aligned_t> neg;

    detail::swap_half_bytes<typename rh_t::buffer_t, is_bit_aligned_t> swhb;
...
struct pnm_type_format_checker {
  ...
  template <typename Image> bool apply() {
    typedef is_read_supported<
        typename get_pixel_type<typename Image::view_t>::type, pnm_tag>
        is_supported_t;
...
struct pnm_read_is_supported {
  ...
  struct apply
      : public is_read_supported<typename get_pixel_type<View>::type, pnm_tag> {
  };
};
...
class dynamic_image_reader<Device, pnm_tag>
  ...
  typedef reader<Device, pnm_tag, detail::read_and_no_convert> parent_t;
...
public:
  ...
  template <typename Images> void apply(any_image<Images> &images) {
    ...
    if (!construct_matched(images, format_checker)) {
      ...
      detail::dynamic_io_fnobj<detail::pnm_read_is_supported, parent_t> op(
          this);
