...
public:
  writer(const Device &io_dev, const image_write_info<pnm_tag> &info)
  ...
  template <typename View> void apply(const View &view) {
    ...
    unsigned int type =
        get_type<num_channels<View>::value>(is_bit_aligned<pixel_t>());

    // write header

    // Add a white space at each string so read_int() can decide when a numbers
    // ends.
...
private:
  ...
  void write_data(const View &src, std::size_t pitch,
    ...
    detail::negate_bits<byte_vector_t, mpl::true_> neg;

    detail::mirror_bits<byte_vector_t, mpl::true_> mirror;
...
class dynamic_image_writer<Device, pnm_tag> : public writer<Device, pnm_tag> {
  typedef writer<Device, pnm_tag> parent_t;
