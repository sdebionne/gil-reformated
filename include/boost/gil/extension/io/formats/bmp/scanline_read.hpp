...
class scanline_reader<Device, bmp_tag>
    : public reader_backend<Device, bmp_tag> {
private:
  typedef scanline_reader<Device, bmp_tag> this_t;
...
public:
  ...
  void read(byte_t *dst, int pos) {
    ...
    if (this->_info._height > 0) {
      ...
      offset =
          this->_info._offset + (this->_info._height - 1 - pos) * this->_pitch;
    } else {
...
private:
  void initialize() {
    ...
    case 4: {
      ...
      case bmp_compression::_rle4: {
        io_error("Cannot read run-length encoded images in iterator mode. Try "
                 "to read as whole image.");
    ...
    case 16: {
      ...
      if (this->_info._compression == bmp_compression::_bitfield) {
        ...
        this->_mask.green.shift =
            detail::trailing_zeros(this->_mask.green.mask);
  ...
  template <typename View> void read_bit_row(byte_t *dst) {
    typedef View src_view_t;
    typedef rgba8_image_t::view_t dst_view_t;

    src_view_t src_view = interleaved_view(
        this->_info._width, 1,
        (typename src_view_t::x_iterator) & _buffer.front(), this->_pitch);
  ...
  void read_15_bits_row(byte_t *dst) {
    ...
    dst_view_t dst_view =
        interleaved_view(this->_info._width, 1,
                         (typename dst_view_t::value_type *)dst, this->_pitch);
  ...
  void read_row(byte_t *dst) { this->_io_dev.read(dst, _pitch); }
