...
public:
  reader_backend(const Device &io_dev,
      ...
      : _io_dev(io_dev)
        ...
        ,
        _settings(settings), _info(), _scanline_length(0)
  ...
  void read_header() {
    ...
    get()->_struct =
        png_create_read_struct(PNG_LIBPNG_VER_STRING, NULL // user_error_ptr
                               ,
                               NULL // user_error_fn
                               ,
    ...
    user_chunk_data[2] = 0;
    user_chunk_data[3] = 0;
    png_set_read_user_chunk_fn(get_struct(), user_chunk_data,
    ...
    if (get_info() == NULL) {
      png_destroy_read_struct(&get()->_struct, NULL, NULL);
    ...
    if (setjmp(png_jmpbuf(get_struct()))) {
      ...
      png_destroy_read_struct(&get()->_struct, &get()->_info, NULL);

      io_error("png is invalid");
    }

    png_set_read_fn(get_struct(), static_cast<png_voidp>(&this->_io_dev),
                    this_t::read_data);
    ...
    // @todo
    png_set_read_user_transform_fn(get_struct(), png_user_transform_ptr(NULL));

    png_set_keep_unknown_chunks(get_struct(), PNG_HANDLE_CHUNK_ALWAYS, NULL, 0);
    ...
    png_get_IHDR(get_struct(), get_info(), &this->_info._width,
                 &this->_info._height, &this->_info._bit_depth,
                 &this->_info._color_type, &this->_info._interlace_method,
                 &this->_info._compression_method, &this->_info._filter_method);
    ...
    if (this->_settings._read_cie_chromacities) {
      this->_info._valid_cie_colors = png_get_cHRM(
          get_struct(), get_info(), &this->_info._white_x,
          &this->_info._white_y, &this->_info._red_x, &this->_info._red_y,
          &this->_info._green_x, &this->_info._green_y, &this->_info._blue_x,
          &this->_info._blue_y);
    }
    ...
    // get the gamma value
    if (this->_settings._read_file_gamma) {
      this->_info._valid_file_gamma = png_get_gAMA_fixed(
          get_struct(), get_info(), &this->_info._file_gamma);
    ...
    if (this->_settings._read_icc_profile) {
      ...
      this->_info._valid_icc_profile =
          png_get_iCCP(get_struct(), get_info(), &icc_name,
                       &this->_info._iccp_compression_type, &profile,
    ...
    if (this->_settings._read_palette) {
      ...
      this->_info._valid_palette = png_get_PLTE(
          get_struct(), get_info(), &palette, &this->_info._num_palette);

      if (this->_info._num_palette > 0) {
        this->_info._palette.resize(this->_info._num_palette);
        std::copy(palette, palette + this->_info._num_palette,
    ...
    if (this->_settings._read_background) {
      ...
      this->_info._valid_background =
          png_get_bKGD(get_struct(), get_info(), &background);
      if (background) {
    ...
    if (this->_settings._read_histogram) {
      ...
      this->_info._valid_histogram =
          png_get_hIST(get_struct(), get_info(), &histogram);
      ...
      if (histogram) {
        ...
        if (this->_settings._read_palette == false) {
          png_colorp palette = png_colorp(NULL);
          png_get_PLTE(get_struct(), get_info(), &palette,
                       &this->_info._num_palette);
        }

        std::copy(histogram, histogram + this->_info._num_palette,
    ...
    if (this->_settings._read_screen_offsets) {
      this->_info._valid_offset =
          png_get_oFFs(get_struct(), get_info(), &this->_info._offset_x,
                       &this->_info._offset_y, &this->_info._off_unit_type);
    }
    ...
    if (this->_settings._read_pixel_calibration) {
      ...
      this->_info._valid_pixel_calibration =
          png_get_pCAL(get_struct(), get_info(), &purpose, &this->_info._X0,
                       &this->_info._X1, &this->_info._cal_type,
                       &this->_info._num_params, &units, &params);
      if (purpose) {
    ...
    if (this->_settings._read_physical_resolution) {
      this->_info._valid_resolution =
          png_get_pHYs(get_struct(), get_info(), &this->_info._res_x,
                       &this->_info._res_y, &this->_info._phy_unit_type);
    }

    // get the image resolution in pixels per meter.
    if (this->_settings._read_pixels_per_meter) {
      this->_info._pixels_per_meter =
          png_get_pixels_per_meter(get_struct(), get_info());
    }
    ...
    if (this->_settings._read_number_of_significant_bits) {
      ...
      this->_info._valid_significant_bits =
          png_get_sBIT(get_struct(), get_info(), &sig_bits);
    ...
    if (this->_settings._read_scale_factors) {
      this->_info._valid_scale_factors =
          png_get_sCAL(get_struct(), get_info(), &this->_info._scale_unit,
                       &this->_info._scale_width, &this->_info._scale_height);
    }
#else
#ifdef BOOST_GIL_IO_PNG_FIXED_POINT_SUPPORTED
    if (this->_settings._read_scale_factors) {
      this->_info._valid_scale_factors = png_get_sCAL_fixed(
          get_struct(), get_info(), &this->_info._scale_unit,
          &this->_info._scale_width, &this->_info._scale_height);
    }
#else
    if (this->_settings._read_scale_factors) {
      ...
      if (this->_info._valid_scale_factors =
              png_get_sCAL_s(get_struct(), get_info(), &this->_info._scale_unit,
    ...
    if (this->_settings._read_transparency_data) {
      ...
      this->_info._valid_transparency_factors =
          png_get_tRNS(get_struct(), get_info(), &trans,
                       &this->_info._num_trans, &trans_values);
      ...
      if (this->_info._num_trans) {
        ...
        std::copy(trans_values, trans_values + this->_info._num_trans,
    ...
    /*
            if( false )
            {
                ...
                int num_unknowns = static_cast< int >( png_get_unknown_chunks(
       get_struct() , get_info() , &unknowns
                                                                             )
                                                     );
            }
    */
