...
struct writer_backend<Device, png_tag>
...
public:
  ...
  writer_backend(const Device &io_dev, const image_write_info<png_tag> &info)
      : png_struct_info_wrapper(false), _io_dev(io_dev), _info(info) {
    ...
    get()->_struct =
        png_create_write_struct(PNG_LIBPNG_VER_STRING, NULL // user_error_ptr
                                ,
                                NULL // user_error_fn
                                ,
    ...
    if (get_info() == NULL) {
      png_destroy_write_struct(&get()->_struct, NULL);
...
protected:
  template <typename View> void write_header(const View &view) {
    ...
    png_set_IHDR(get_struct(), get_info(),
    ...
    if (_info._valid_cie_colors) {
      png_set_cHRM(get_struct(), get_info(), _info._white_x, _info._white_y,
                   _info._red_x, _info._red_y, _info._green_x, _info._green_y,
                   _info._blue_x, _info._blue_y);
    }
...
#else
    if (_info._valid_cie_colors) {
      png_set_cHRM_fixed(get_struct(), get_info(), _info._white_x,
                         _info._white_y, _info._red_x, _info._red_y,
                         _info._green_x, _info._green_y, _info._blue_x,
                         _info._blue_y);
    }
    ...
    if (_info._valid_icc_profile) {
      ...
      png_set_iCCP(get_struct(), get_info(),
    ...
    if (_info._valid_pixel_calibration) {
      ...
      png_set_pCAL(get_struct(), get_info(),
                   const_cast<png_charp>(_info._purpose.c_str()), _info._X0,
                   _info._X1, _info._cal_type, _info._num_params,
                   const_cast<png_charp>(_info._units.c_str()),
                   const_cast<png_charpp>(&params.front()));
    }

    if (_info._valid_resolution) {
      png_set_pHYs(get_struct(), get_info(), _info._res_x, _info._res_y,
                   _info._phy_unit_type);
    }

    if (_info._valid_significant_bits) {
      png_set_sBIT(get_struct(), get_info(),
    ...
    if (_info._valid_scale_factors) {
      png_set_sCAL_fixed(get_struct(), get_info(), this->_info._scale_unit,
                         this->_info._scale_width, this->_info._scale_height);
    }
#else
    if (_info._valid_scale_factors) {
      png_set_sCAL_s(get_struct(), get_info(), this->_info._scale_unit,
    ...
    if (_info._valid_modification_time) {
      png_set_tIME(get_struct(), get_info(),
    ...
    if (_info._set_dithering) {
      png_set_dither(get_struct(), &_info._dithering_palette.front(),
                     _info._dithering_num_palette,
                     _info._dithering_maximum_colors,
                     &_info._dithering_histogram.front(), _info._full_dither);
    }
...
private:
  void init_io(png_structp png_ptr) {
    png_set_write_fn(png_ptr, static_cast<void *>(&this->_io_dev),
