...
private:
  typedef reader<Device, jpeg_tag, ConversionPolicy> this_t;
...
public:
  ...
  template <typename View> void apply(const View &view) {
    ...
    case JCS_RGB:
    //!\todo add Y'CbCr? We loose image quality when reading JCS_YCbCr as
    //! JCS_RGB
    case JCS_YCbCr: {
    ...
    case JCS_CMYK:
    //!\todo add Y'CbCrK? We loose image quality when reading JCS_YCCK as
    //! JCS_CMYK
    case JCS_YCCK: {
      this->get()->out_color_space = JCS_CMYK;
      this->_scanline_length =
          this->_info._width * num_channels<cmyk8_view_t>::value;
...
private:
  ...
  void read_rows(const View &view) {
    ...
    for (int y = 0; y < this->_settings._top_left.y; ++y) {
      io_error_if(jpeg_read_scanlines(this->get(), &row_adr, 1) != 1,
    ...
    for (int y = 0; y < view.height(); ++y) {
      ...
      typename buffer_t::iterator beg =
          buffer.begin() + this->_settings._top_left.x;
...
struct jpeg_type_format_checker {
  ...
  template <typename Image> bool apply() {
    return is_read_supported<
               typename get_pixel_type<typename Image::view_t>::type,
               jpeg_tag>::_color_space == _color_space;
  }
...
class dynamic_image_reader<Device, jpeg_tag>
  ...
  typedef reader<Device, jpeg_tag, detail::read_and_no_convert> parent_t;
...
public:
  ...
  template <typename Images> void apply(any_image<Images> &images) {
    ...
    if (!construct_matched(images, format_checker)) {
      ...
      detail::dynamic_io_fnobj<detail::jpeg_read_is_supported, parent_t> op(
          this);
