...
private:
  ...
  void read_rle_data(const View_Dst &view) {
    ...
    size_t image_size =
        this->_info._width * this->_info._height * bytes_per_pixel;
...
public:
  ...
  template <typename Images> void apply(any_image<Images> &images) {
    detail::targa_type_format_checker format_checker(
        this->_info._bits_per_pixel);
    ...
    if (!construct_matched(images, format_checker)) {
      ...
      detail::dynamic_io_fnobj<detail::targa_read_is_supported, parent_t> op(
          this);
