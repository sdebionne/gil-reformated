...
public:
  ...
  template <typename V> inline bool get_property_data(ttag_t tag, V &value) {
    // @todo: defaulted, really?
    return (1 == TIFFGetFieldDefaulted(_tiff_file.get(), tag, &value));
  }
  // Specialisation for multi-valued properties. @todo: add one of
  // these for the three-parameter fields too.
  template <typename V, typename W>
  inline bool get_property_data(ttag_t tag, fusion::vector2<V, W> &vw) {
    return (1 == TIFFGetFieldDefaulted(_tiff_file.get(), tag,
  ...
  // For single-valued properties
  template <typename V> inline bool set_property_data(ttag_t tag, V const &v) {
    return (1 == TIFFSetField(_tiff_file.get(), tag, v));
  }

  // Specialisation for multi-valued properties. @todo: add one of
  // these for the three-parameter fields too.
  template <typename V, typename W>
  inline bool set_property_data(ttag_t tag, fusion::vector2<V, W> const &vw) {
    return (1 == TIFFSetField(_tiff_file.get(), tag, fusion::at_c<0>(vw),
                              fusion::at_c<1>(vw)));
  }
