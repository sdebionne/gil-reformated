...
protected:
  template <typename View> void write_header(const View &view) {
    ...
    if (this->_info._photometric_interpretation_user_defined) {
      ...
      // subjective. The user should better set this value itself. There is no
      // way to decide if a image is PHOTOMETRIC_MINISWHITE or
      // PHOTOMETRIC_MINISBLACK. This writer will assume PHOTOMETRIC_MINISBLACK
      // for gray_t images and PHOTOMETRIC_RGB for rgb_t images.
    ...
    // write compression
    this->_io_dev.template set_property<tiff_compression>(
        this->_info._compression);

    // write orientation
    this->_io_dev.template set_property<tiff_orientation>(
        this->_info._orientation);
