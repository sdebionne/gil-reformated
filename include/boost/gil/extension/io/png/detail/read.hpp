...
public:
  reader(const Device &io_dev, const image_read_settings<png_tag> &settings)
      : reader_base<png_tag, ConversionPolicy>(), backend_t(io_dev, settings) {}
  ...
  template <typename View> void apply(const View &view) {
    ...
    if (this->_settings._apply_screen_gamma) {
      ...
      png_set_gamma(this->get_struct(), this->_settings._screen_gamma,
    ...
    this->_info._bit_depth =
        png_get_bit_depth(this->get_struct(), this->get_info());

    this->_info._num_channels =
        png_get_channels(this->get_struct(), this->get_info());

    this->_info._color_type =
        png_get_color_type(this->get_struct(), this->get_info());

    this->_scanline_length =
        png_get_rowbytes(this->get_struct(), this->get_info());
    ...
    case PNG_COLOR_TYPE_GRAY: {
      ...
      case 16:
        ...
        break;
      default:
        io_error("png_reader::read_data(): unknown combination of color type "
                 "and bit depth");
      }
    ...
    case PNG_COLOR_TYPE_RGBA: {
      ...
      case 16:
        ...
        break;
      default:
        io_error("png_reader_color_convert::read_data(): unknown combination "
                 "of color type and bit depth");
      }
  ...
  void read_rows(const View &view) {
    ...
    for (std::size_t pass = 0; pass < this->_number_passes; pass++) {
      if (pass == this->_number_passes - 1) {
        ...
        std::ptrdiff_t remaining_rows =
            static_cast<std::ptrdiff_t>(this->_info._height) -
...
struct png_read_is_supported {
  ...
  struct apply
      : public is_read_supported<typename get_pixel_type<View>::type, png_tag> {
  };
};
...
public:
  ...
  template <typename Images> void apply(any_image<Images> &images) {
    ...
    if (!construct_matched(images, format_checker)) {
      ...
      detail::dynamic_io_fnobj<detail::png_read_is_supported, parent_t> op(
          this);
