...
public:
  ...
  void read(byte_t *dst, int) { read_scanline(dst); }
...
private:
  void initialize() {
    ...
    if (this->_settings._apply_screen_gamma) {
      ...
      png_set_gamma(this->get()->_struct, this->_settings._screen_gamma,
    ...
    this->_info._bit_depth =
        png_get_bit_depth(this->get()->_struct, this->get()->_info);

    this->_info._num_channels =
        png_get_channels(this->get()->_struct, this->get()->_info);

    this->_info._color_type =
        png_get_color_type(this->get()->_struct, this->get()->_info);

    this->_scanline_length =
        png_get_rowbytes(this->get()->_struct, this->get()->_info);
  }
