...
private:
  using pixel_t =
      typename std::remove_reference<PixelReference>::type::value_type;

public:
  using type =
      typename iterator_type_from_pixel<pixel_t, false, false, true>::type;
};
...
struct tiff_write_is_supported {
  ...
  struct apply : public is_write_supported<typename get_pixel_type<View>::type,
                                           tiff_tag> {};
};
...
public:
  writer(const Device &io_dev, const image_write_info<tiff_tag> &info)
      : backend_t(io_dev, info) {}

  template <typename View> void apply(const View &view) { write_view(view); }

private:
  template <typename View> void write_view(const View &view) {
  ...
  template <typename View>
  void write_tiled_data(const View &view, tiff_tile_width::type tw,
  ...
  template <typename View>
  void write_data(const View &view, std::size_t,
  ...
  void write_tiled_data(const View &view, tiff_tile_width::type tw,
    ...
    using x_iterator = typename detail::
  ...
  void write_tiled_view_to_dev(const View &view, IteratorType it,
                               const std::true_type & // has_alpha
  ) {
    auto pm_view = premultiply_view<typename View::value_type>(view);
    ...
    while (i < view.height()) {
      while (j < view.width()) {
        if (j + tw < view.width() && i + th < view.height()) {
          ...
          std::ptrdiff_t current_tile_length =
              (i + th < height) ? th : height - i;
          ...
          for (typename View::y_coord_t y = 0; y < tile_subimage_view.height();
               ++y) {
            std::copy(tile_subimage_view.row_begin(y),
                      tile_subimage_view.row_end(y), it);
            std::advance(it, tw);
          }
        ...
        this->_io_dev.write_tile(row, static_cast<uint32>(j),
                                 static_cast<uint32>(i), 0, 0);
        j += tw;
      }
      j = 0;
      i += th;
...
public:
  ...
  template <typename Views> void apply(const any_image_view<Views> &views) {
    detail::dynamic_io_fnobj<detail::tiff_write_is_supported, parent_t> op(
        this);
