...
template <> struct get_property_f<2> {
  ...
  bool call_me(typename Property::type &vs, std::shared_ptr<TIFF> &file) const {
    ...
    if (1 ==
        TIFFGetFieldDefaulted(file.get(), Property::tag, &length, &pointer)) {
      std::copy_n(static_cast<typename Property::type::const_pointer>(pointer),
                  length, std::back_inserter(vs));
      return true;
    } else
      return false;
  }
...
public:
  ostream_device(std::ostream &out) : _out(out) {
    ...
    io_error_if((tiff = TIFFStreamOpen("", &_out)) == nullptr,
...
public:
  istream_device(std::istream &in) : _in(in) {
    ...
    io_error_if((tiff = TIFFStreamOpen("", &_in)) == nullptr,
...
template <>
struct sample_format<uint8_t> : public mpl::int_<SAMPLEFORMAT_UINT> {};
template <>
struct sample_format<uint16_t> : public mpl::int_<SAMPLEFORMAT_UINT> {};
template <>
struct sample_format<uint32_t> : public mpl::int_<SAMPLEFORMAT_UINT> {};
template <>
struct sample_format<float32_t> : public mpl::int_<SAMPLEFORMAT_IEEEFP> {};
template <>
struct sample_format<double> : public mpl::int_<SAMPLEFORMAT_IEEEFP> {};
template <>
struct sample_format<int8_t> : public mpl::int_<SAMPLEFORMAT_INT> {};
template <>
struct sample_format<int16_t> : public mpl::int_<SAMPLEFORMAT_INT> {};
template <>
struct sample_format<int32_t> : public mpl::int_<SAMPLEFORMAT_INT> {};
