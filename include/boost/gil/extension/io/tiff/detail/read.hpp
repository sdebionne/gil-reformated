...
template <int K> struct plane_recursion {
  template <typename View, typename Device, typename ConversionPolicy>
  static void read_plane(const View &dst_view,
...
template <> struct plane_recursion<-1> {
  template <typename View, typename Device, typename ConversionPolicy>
  static void read_plane(const View & /* dst_view */
                         ,
                         reader<Device, tiff_tag, ConversionPolicy> * /* p */
...
private:
  typedef reader<Device, tiff_tag, ConversionPolicy> this_t;
...
public:
  reader(const Device &io_dev, const image_read_settings<tiff_tag> &settings)
      : reader_base<tiff_tag, ConversionPolicy>(), backend_t(io_dev, settings) {
  }
  ...
  template <typename View> void apply(View &dst_view) {
...
private:
  ...
  void read_palette_image(const View &dst_view) {
    ...
    read_data<detail::row_buffer_helper_view<typename PaletteImage::view_t>>(
        view(indices), 0);
  ...
  void read_palette_image(const View &dst_view,
    ...
    typedef typename channel_traits<typename element_type<
  ...
  template <typename View, typename Indices_View>
  inline void read_palette_image(const View & /* dst_view     */
                                 ,
                                 const Indices_View & /* indices_view */
                                 ,
  ...
  template <typename Buffer> void skip_over_rows(Buffer &buffer, int plane) {
    if (this->_info._compression != COMPRESSION_NONE) {
      // Skipping over rows is not possible for compressed images(  no random
      // access ). See man page ( diagnostics section ) for more information.
  ...
  void read_tiled_data_subimage(const View &dst_view, int plane) {
    ...
    for (unsigned int y = 0; y < image_height; y += tile_height) {
      for (unsigned int x = 0; x < image_width; x += tile_width) {
        uint32_t current_tile_width =
            (x + tile_width < image_width) ? tile_width : image_width - x;
        uint32_t current_tile_length =
            (y + tile_height < image_height) ? tile_height : image_height - y;

        this->_io_dev.read_tile(row_buffer_helper.buffer(), x, y, 0,
                                static_cast<tsample_t>(plane));

        // these are all whole image coordinates
        point_t tile_top_left(x, y);
        point_t tile_lower_right(x + current_tile_width - 1,
                                 y + current_tile_length - 1);
        ...
        if (tile_top_left.x > view_lower_right.x ||
          ...
          std::ptrdiff_t img_x0 = (tile_top_left.x >= view_top_left.x)
                                      ? tile_top_left.x
                                      : view_top_left.x;
          std::ptrdiff_t img_y0 = (tile_top_left.y >= view_top_left.y)
                                      ? tile_top_left.y
                                      : view_top_left.y;
          ...
          assert(tile_x0 < tile_width && tile_y0 < tile_height &&
                 tile_x1 < tile_width && tile_y1 < tile_height);
          ...
          View dst_subimage_view = subimage_view(
              dst_view, (int)dst_x0, (int)dst_y0, (int)tile_subimage_view_width,
          ...
          for (std::ptrdiff_t dst_row = 0; dst_row < dst_subimage_view.height();
            ...
            this->_cc_policy.read(begin, end,
  ...
  void read_tiled_data_full(const View &dst_view, int plane) {
    ...
    for (unsigned int y = 0; y < image_height; y += tile_height) {
      for (unsigned int x = 0; x < image_width; x += tile_width) {
        ...
        View dst_subimage_view = subimage_view(
            dst_view, x, y, current_tile_width, current_tile_length);
  ...
  void read_stripped_data(const View &dst_view, int plane) {
    typedef typename is_bit_aligned<typename View::value_type>::type
    ...
    std::size_t size_to_allocate = buffer_size<typename View::value_type>(
        dst_view.width(), is_view_bit_aligned_t());
    ...
    for (; row < row_end; ++row, ++dst_row) {
      this->_io_dev.read_scanline(row_buffer_helper.buffer(), row,
                                  static_cast<tsample_t>(plane));
  ...
  std::size_t buffer_size(std::size_t width, mpl::false_ // is_bit_aligned
    ...
    std::size_t ret = std::max(
        width, ((scanline_size_in_bytes + element_size - 1) / element_size));
...
class dynamic_image_reader<Device, tiff_tag>
  ...
  typedef reader<Device, tiff_tag, detail::read_and_no_convert> parent_t;
