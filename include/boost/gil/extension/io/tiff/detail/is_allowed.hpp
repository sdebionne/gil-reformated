...
struct Format_Type<View, Channel,
                   typename std::enable_if<is_bit_aligned<
                       typename get_pixel_type<View>::type>::value>::type> {
...
struct Format_Type<View, Channel,
                   typename std::enable_if<mpl::and_<
                       ...
                       is_unsigned<Channel>>::value>::type> {
  static const int value = SAMPLEFORMAT_UINT;
};
...
// The following two functions look the same but are different since one is
// using a pixel_t as template parameter whereas the other is using reference_t.
...
bool compare_channel_sizes(const channel_sizes_t &channel_sizes // in bits
  ...
  using channel_t =
      typename channel_traits<typename element_type<pixel_t>::type>::value_type;
...
bool compare_channel_sizes(const channel_sizes_t &channel_sizes // in bits
  ...
  using channel_t =
      typename channel_traits<typename element_type<ref_t>::type>::value_type;
  channel_t c;
...
struct compare_channel_sizes_fn {
  ...
  template <typename ChannelSize> void operator()(ChannelSize x) {
    if (x != *_a++) {
      _b = false;
    }
  }
...
template <typename T> struct channel_sizes_type {};
...
bool is_allowed(const image_read_info<tiff_tag> &info,
  ...
  // const num_channel_t dst_sample_format     = format_value< channel_t >(
  ...
  return (dst_samples_per_pixel == info._samples_per_pixel &&
          compare_channel_sizes<View>(
              channel_sizes, typename is_bit_aligned<pixel_t>::type(),
