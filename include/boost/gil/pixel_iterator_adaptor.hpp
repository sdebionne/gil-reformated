...
/// \brief An iterator that invokes a provided function object upon dereference.
/// Models: IteratorAdaptorConcept, PixelIteratorConcept

/// \ingroup PixelIteratorModelDerefPtr PixelBasedModel
/// \brief An adaptor over an existing iterator that provides for custom filter
/// on dereferencing the object. Models: IteratorAdaptorConcept,
...
public:
  ...
  // computes distance and compares for zero. it is often faster to just apply
  // the relation operator to the base
...
struct const_iterator_type<dereference_iterator_adaptor<I, DFn>> {
  typedef dereference_iterator_adaptor<typename const_iterator_type<I>::type,
...
struct dynamic_x_step_type<dereference_iterator_adaptor<Iterator, DFn>> {
  typedef dereference_iterator_adaptor<
      typename dynamic_x_step_type<Iterator>::type, DFn>
      type;
};

/// \brief Returns the type (and creates an instance) of an iterator that
...
/// \brief For dereference iterator adaptors, compose the new function object
/// after the old one
...
struct iterator_add_deref<dereference_iterator_adaptor<Iterator, PREV_DEREF>,
  ...
  static type make(const dereference_iterator_adaptor<Iterator, PREV_DEREF> &it,
                   const Deref &d) {
