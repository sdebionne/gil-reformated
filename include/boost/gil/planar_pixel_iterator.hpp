...
/// \brief A homogeneous color base whose element is a channel iterator. Models
/// HomogeneousColorBaseValueConcept This class is used as an iterator to a
/// planar pixel.
...
public:
  ...
  planar_pixel_iterator(const ChannelPtr &v0, const ChannelPtr &v1)
      : color_base_parent_t(v0, v1) {}
  planar_pixel_iterator(const ChannelPtr &v0, const ChannelPtr &v1,
                        const ChannelPtr &v2)
      : color_base_parent_t(v0, v1, v2) {}
  planar_pixel_iterator(const ChannelPtr &v0, const ChannelPtr &v1,
                        const ChannelPtr &v2, const ChannelPtr &v3)
  ...
  /// Copy constructor and operator= from pointers to compatible planar pixels
  /// or planar pixel references. That allow constructs like pointer = &value or
  /// pointer = &reference Since we should not override operator& that's the
  /// best we can do.
  template <typename P>
  ...
  template <typename P> planar_pixel_iterator &operator=(P *pix) {
    function_requires<PixelsCompatibleConcept<P, value_type>>();
...
private:
  ...
  void advance(std::ptrdiff_t d) {
    static_transform(
        *this, *this,
        std::bind(detail::plus_asymmetric<ChannelPtr, std::ptrdiff_t>(),
