...
struct premultiply {
  ...
  void operator()(const SrcP &src, DstP &dst) const {
    ...
    typedef typename mpl::remove<src_colour_space_t, alpha_t>::type
        src_colour_channels;

    typedef mpl::bool_<mpl::contains<dst_colour_space_t, alpha_t>::value>
        has_alpha_t;
    mpl::for_each<src_colour_channels>(
        channel_premultiply<SrcP, DstP>(src, dst));
...
template <typename SrcView, typename DstP> struct premultiplied_view_type {
private:
  typedef typename SrcView::const_t::reference
      src_pix_ref; // const reference to pixel in SrcView
  typedef premultiply_deref_fn<src_pix_ref, DstP>
      deref_t; // the dereference adaptor that performs color conversion
...
inline typename premultiplied_view_type<View, DstP>::type
premultiply_view(const View &src) {
