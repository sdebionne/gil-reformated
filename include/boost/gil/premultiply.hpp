...
struct premultiply {
  ...
  void operator()(const SrcP &src, DstP &dst) const {
    ...
    using has_alpha_t = std::integral_constant<
        bool, mp11::mp_contains<dst_colour_space_t, alpha_t>::value>;
    mp11::mp_for_each<src_colour_channels>(
...
private:
  using src_pix_ref =
      typename SrcView::const_t::reference; // const reference to pixel in
                                            // SrcView
  using deref_t =
      premultiply_deref_fn<src_pix_ref, DstP>; // the dereference adaptor that
