...
public:
  typedef typename mpl::if_c<Mutable, unsigned char, const unsigned char>::type
      byte_t;
...
bits for green, 3 bits for blue) typedef const
bit_aligned_pixel_reference<unsigned char, mpl::vector3_c<int,1,2,3>,
rgb_layout_t, true>  rgb123_ref_t;
...
struct bit_aligned_pixel_reference {
  ...
  typedef typename packed_pixel_type<bitfield_t, ChannelBitSizes, Layout>::type
      value_type;
  typedef const bit_aligned_pixel_reference reference;
  typedef const bit_aligned_pixel_reference<BitField, ChannelBitSizes, Layout,
                                            false>
      const_reference;
...
public:
  typedef const packed_dynamic_channel_reference<
      BitField, mpl::at_c<ChannelBitSizes, K>::type::value, IsMutable>
      type;
};
...
at_c(const bit_aligned_pixel_reference<BitField, ChannelBitSizes, L, Mutable>
         &p) {
  typedef bit_aligned_pixel_reference<BitField, ChannelBitSizes, L, Mutable>
      pixel_t;
...
template <typename T> struct k_copies<0, T> { typedef mpl::vector0<> type; };
...
private:
  ...
  typedef typename detail::k_copies<size_t::value,
                                    mpl::integral_c<unsigned, NumBits>>::type
      channel_bit_sizes_t;

public:
  typedef bit_aligned_pixel_reference<BitField, channel_bit_sizes_t, Layout,
                                      false>
      type;
};
