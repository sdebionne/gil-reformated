...
template <int RangeSize, bool IsMutable> class bit_range {
public:
...
/// bit_aligned_pixel_reference<unsigned char, mp11::mp_list_c<int,1,2,3>,
...
///
/// \tparam BitField
/// \tparam ChannelBitSizes Boost.MP11-compatible list of integral types
/// defining the number of bits for each channel. For example, for 565RGB,
/// mp_list_c<int,5,6,5> \tparam Layout \tparam IsMutable
...
struct bit_aligned_pixel_reference {
  ...
  using data_ptr_t =
      mp11::mp_if_c<IsMutable, unsigned char *, const unsigned char *>;
  ...
  template <typename P> bool operator==(P const &p) const {
...
private:
  ...
  template <typename Pixel> void assign(Pixel const &p, std::true_type) const {
    check_compatible<Pixel>();
    static_copy(p, *this);
  }

  template <typename Pixel> bool equal(Pixel const &p, std::true_type) const {
    check_compatible<Pixel>();
...
struct kth_element_type<
  ...
  using type = packed_dynamic_channel_reference<
      BitField, mp11::mp_at_c<ChannelBitSizes, K>::value, IsMutable> const;
};
...
struct sum_k
    : mp11::mp_plus<sum_k<IntegralVector, K - 1>,
...
template <int K, typename BitField, typename ChannelBitSizes, typename L,
          bool IsMutable>
inline auto at_c(const bit_aligned_pixel_reference<BitField, ChannelBitSizes, L,
                                                   IsMutable> &p) ->
...
struct pixel_reference_type<
    packed_dynamic_channel_reference<BitField, NumBits, false> const, Layout,
    false, false> {
private:
  using channel_bit_sizes_t =
      mp11::mp_repeat<mp11::mp_list<std::integral_constant<unsigned, NumBits>>,
...
template <typename BitField, int NumBits, typename Layout>
struct pixel_reference_type<
    packed_dynamic_channel_reference<BitField, NumBits, true> const, Layout,
    false, true> {
private:
  using channel_bit_sizes_t =
      mp11::mp_repeat<mp11::mp_list<std::integral_constant<unsigned, NumBits>>,
