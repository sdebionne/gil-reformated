...
bit_aligned_pixel_reference<mpl::vector3_c<int,1,2,3>, rgb_layout_t, true>
rgb123_ref_t;
...
/// PixelBasedConcept
template <typename ChannelBitSizes, // MPL integral vector defining the number
...
struct bit_aligned_pixel_reference {
  ...
  typedef const bit_aligned_pixel_reference<ChannelBitSizes, Layout, false>
      const_reference;
  ...
  bit_aligned_pixel_reference(
      const bit_aligned_pixel_reference<ChannelBitSizes, Layout, IsMutable2> &p)
...
struct kth_element_type<
    bit_aligned_pixel_reference<ChannelBitSizes, L, IsMutable>, K> {
private:
  typedef
      typename bit_aligned_pixel_reference<ChannelBitSizes, L,
                                           IsMutable>::bitfield_t bitfield_t;

public:
  typedef const packed_dynamic_channel_reference<
      bitfield_t, mpl::at_c<ChannelBitSizes, K>::type::value, IsMutable>
      type;
};
...
inline typename kth_element_reference_type<
    bit_aligned_pixel_reference<ChannelBitSizes, L, Mutable>, K>::type
...
// reference Note: BitField must be the same type as
// pixel_reference_type<...>::type::bitfield_t, but it is too complicated to
// ensure this
...
// cases because this triggers ambiguity Note: BitField must be the same type as
// pixel_reference_type<...>::type::bitfield_t, but it is too complicated to
// ensure this
template <typename BitField, int NumBits, typename Layout>
struct pixel_reference_type<
...
inline void swap(boost::gil::bit_aligned_pixel_reference<C, L, true> x, R &y) {
  boost::gil::swap_proxy<
      typename boost::gil::bit_aligned_pixel_reference<C, L, true>::value_type>(
      x, y);
}

template <typename C, typename L>
inline void swap(
    typename boost::gil::bit_aligned_pixel_reference<C, L, true>::value_type &x,
    boost::gil::bit_aligned_pixel_reference<C, L, true> y) {
  boost::gil::swap_proxy<
      typename boost::gil::bit_aligned_pixel_reference<C, L, true>::value_type>(
      x, y);
}
...
template <typename C, typename L>
inline void swap(boost::gil::bit_aligned_pixel_reference<C, L, true> x,
                 boost::gil::bit_aligned_pixel_reference<C, L, true> y) {
  boost::gil::swap_proxy<
      typename boost::gil::bit_aligned_pixel_reference<C, L, true>::value_type>(
      x, y);
