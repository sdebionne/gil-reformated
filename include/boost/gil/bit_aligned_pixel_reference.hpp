...
//  bit_range
//
//  Represents a range of bits that can span multiple consecutive bytes. The
//  range has a size fixed at compile time, but the offset is specified at run
//  time.
/////////////////////////////

template <int RangeSize, bool Mutable> class bit_range {
public:
  using byte_t =
...
private:
  ...
  int _bit_offset;       // offset from the beginning of the current byte.
                         // 0<=_bit_offset<=7
...
public:
  ...
  bit_range(const bit_range &br)
      : _current_byte(br._current_byte), _bit_offset(br._bit_offset) {}
  template <bool M>
  ...
  difference_type bit_distance_to(const bit_range &b) const {
    return (b.current_byte() - current_byte()) * 8 + b.bit_offset() -
           bit_offset();
  }
...
/// \brief A heterogeneous color base representing pixel that may not be byte
/// aligned, i.e. it may correspond to a bit range that does not start/end at a
/// byte boundary. Models ColorBaseConcept.
///
/// \defgroup PixelModelNonAlignedPixel bit_aligned_pixel_reference
/// \ingroup PixelModel
/// \brief A heterogeneous pixel reference used to represent non-byte-aligned
/// pixels. Models PixelConcept
///
/// Example:
/// \code
/// unsigned char data=0;
///
/// // A mutable reference to a 6-bit BGR pixel in "123" format (1 bit for red,
/// 2 bits for green, 3 bits for blue) using rgb123_ref_t =
/// bit_aligned_pixel_reference<unsigned char, mpl::vector3_c<int,1,2,3>,
/// rgb_layout_t, true> const;
///
...
/// \endcode
///
/// \ingroup ColorBaseModelNonAlignedPixel PixelModelNonAlignedPixel
/// PixelBasedModel \brief Heterogeneous pixel reference corresponding to
/// non-byte-aligned bit range. Models ColorBaseConcept, PixelConcept,
/// PixelBasedConcept
///
/// \tparam BitField
/// \tparam ChannelBitSizes MPL integral vector defining the number of bits for
/// each channel. For example, for 565RGB, vector_c<int,5,6,5> \tparam Layout
/// \tparam IsMutable
template <typename BitField, typename ChannelBitSizes, typename Layout,
          bool IsMutable>
struct bit_aligned_pixel_reference {
  static constexpr int bit_size =
      mpl::accumulate<ChannelBitSizes, mpl::int_<0>,
                      mpl::plus<mpl::_1, mpl::_2>>::type::value;

  using bit_range_t = boost::gil::bit_range<bit_size, IsMutable>;
  using bitfield_t = BitField;
  using data_ptr_t = typename mpl::if_c<IsMutable, unsigned char *,
                                        const unsigned char *>::type;

  using layout_t = Layout;

  using value_type =
      typename packed_pixel_type<bitfield_t, ChannelBitSizes, Layout>::type;
  using reference = const bit_aligned_pixel_reference<BitField, ChannelBitSizes,
                                                      Layout, IsMutable>;
  using const_reference = bit_aligned_pixel_reference<BitField, ChannelBitSizes,
                                                      Layout, false> const;
  ...
  bit_aligned_pixel_reference(const bit_aligned_pixel_reference<
                              BitField, ChannelBitSizes, Layout, IsMutable2> &p)
  ...
  bit_aligned_pixel_reference(packed_pixel<BF, CR, Layout> &p)
      : _bit_range(static_cast<data_ptr_t>(&gil::at_c<0>(p)),
...
private:
  ...
  template <typename Pixel> static void check_compatible() {
    gil_function_requires<
        PixelsCompatibleConcept<Pixel, bit_aligned_pixel_reference>>();
  }

  template <typename Pixel> void assign(const Pixel &p, mpl::true_) const {
...
template <typename BitField, typename ChannelBitSizes, typename L,
          bool IsMutable, int K>
struct kth_element_type<
    bit_aligned_pixel_reference<BitField, ChannelBitSizes, L, IsMutable>, K> {
  using type = packed_dynamic_channel_reference<
      BitField, mpl::at_c<ChannelBitSizes, K>::type::value, IsMutable> const;
};
...
template <typename B, typename C, typename L, bool M, int K>
struct kth_element_const_reference_type<bit_aligned_pixel_reference<B, C, L, M>,
                                        K>
    : public kth_element_type<bit_aligned_pixel_reference<B, C, L, M>, K> {};
...
template <int K, typename BitField, typename ChannelBitSizes, typename L,
          bool Mutable>
inline typename kth_element_reference_type<
    bit_aligned_pixel_reference<BitField, ChannelBitSizes, L, Mutable>, K>::type
at_c(const bit_aligned_pixel_reference<BitField, ChannelBitSizes, L, Mutable>
         &p) {
  using pixel_t =
      bit_aligned_pixel_reference<BitField, ChannelBitSizes, L, Mutable>;
...
/// Metafunction predicate that flags bit_aligned_pixel_reference as a model of
/// PixelConcept. Required by PixelConcept
template <typename B, typename C, typename L, bool M>
...
template <typename T> struct k_copies<0, T> { using type = mpl::vector0<>; };
template <unsigned K, typename T>
struct k_copies : public mpl::push_back<typename k_copies<K - 1, T>::type, T> {
};
} // namespace detail

// Constructs a homogeneous bit_aligned_pixel_reference given a channel
// reference
template <typename BitField, int NumBits, typename Layout>
struct pixel_reference_type<
    const packed_dynamic_channel_reference<BitField, NumBits, false>, Layout,
    false, false> {
private:
  using size_t = typename mpl::size<typename Layout::color_space_t>::type;
  using channel_bit_sizes_t =
      typename detail::k_copies<size_t::value,
                                mpl::integral_c<unsigned, NumBits>>::type;
...
// Same but for the mutable case. We cannot combine the mutable and read-only
// cases because this triggers ambiguity
template <typename BitField, int NumBits, typename Layout>
struct pixel_reference_type<
    const packed_dynamic_channel_reference<BitField, NumBits, true>, Layout,
    false, true> {
private:
  using size_t = typename mpl::size<typename Layout::color_space_t>::type;
  using channel_bit_sizes_t =
...
public:
  using type =
      bit_aligned_pixel_reference<BitField, channel_bit_sizes_t, Layout, true>;
};
...
// Having three overloads allows us to swap between different (but compatible)
