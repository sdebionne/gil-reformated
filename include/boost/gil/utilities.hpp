...
/// \ingroup PointModel
template <typename T>
GIL_FORCEINLINE point2<T> operator*(const point2<T> &p, int t) {
  return point2<T>(p.x * t, p.y * t);
}
/// \ingroup PointModel
template <typename T>
GIL_FORCEINLINE point2<T> operator*(int t, const point2<T> &p) {
  return point2<T>(p.x * t, p.y * t);
}
...
inline point2<int> iround(const point2<float> &p) {
  return point2<int>(iround(p.x), iround(p.y));
}
/// \ingroup PointAlgorithm
inline point2<int> iround(const point2<double> &p) {
  return point2<int>(iround(p.x), iround(p.y));
}
/// \ingroup PointAlgorithm
inline point2<int> ifloor(const point2<float> &p) {
  return point2<int>(ifloor(p.x), ifloor(p.y));
}
/// \ingroup PointAlgorithm
inline point2<int> ifloor(const point2<double> &p) {
  return point2<int>(ifloor(p.x), ifloor(p.y));
}
/// \ingroup PointAlgorithm
inline point2<int> iceil(const point2<float> &p) {
  return point2<int>(iceil(p.x), iceil(p.y));
}
/// \ingroup PointAlgorithm
inline point2<int> iceil(const point2<double> &p) {
  return point2<int>(iceil(p.x), iceil(p.y));
}
...
/// std::unary_compose but needs to pull some typedefs from the component types.
/// Models: PixelDereferenceAdaptorConcept \ingroup PixelDereferenceAdaptorModel
///
...
class deref_compose
    : public deref_base<
          ...
          typename D1::const_reference, typename D2::argument_type,
          typename D1::result_type, D1::is_mutable && D2::is_mutable> {
public:
  D1 _fn1;
  D2 _fn2;
...
inline std::pair<RAIter, OutputIter> _copy_n(RAIter first, Size count,
                                             OutputIter result,
...
template <class InputIter, class Size, class OutputIter>
inline std::pair<InputIter, OutputIter> _copy_n(InputIter first, Size count,
                                                OutputIter result) {
...
template <class InputIter, class Size, class OutputIter>
inline std::pair<InputIter, OutputIter> copy_n(InputIter first, Size count,
                                               OutputIter result) {
...
template <typename T> struct inc : public std::unary_function<T, T> {
...
/// \brief operator-- wrapped in a function object
template <typename T> struct dec : public std::unary_function<T, T> {
  T operator()(T x) const { return --x; }
};
