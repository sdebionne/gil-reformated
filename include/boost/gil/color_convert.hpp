...
/// \ingroup ColorConvert
/// \brief Gray to CMYK
/// \todo FIXME: Where does this calculation come from? Shouldn't gray be
/// inverted?
...
///
/// \todo FIXME: The original implementation did not handle properly signed CMYK
/// pixels as destination
///
template <> struct default_color_converter_impl<rgb_t, cmyk_t> {
  ...
  void operator()(SrcPixel const &src, DstPixel &dst) const {
    ...
    // s = 1 / (1 - k) for k < 1, where 1 denotes dst_t max, otherwise s = 1
    // (literal).
