...
/////////////////////////////////////////
/// \defgroup AHE AHE
/// \brief Contains implementation and description of the algorithm used to
/// compute
///        adaptive histogram equalization of input images. Naming for the AHE
///        functions are done in the following way
///             <feature-1>_<feature-2>_.._<feature-n>ahe
///        For example, for AHE done using local (non-overlapping) tiles/blocks
///        and final output interpolated among tiles , it is called
...
/// \ingroup AHE-helpers
/// \brief Computes the actual clip limit given a clip limit value using binary
/// search.
...
/// \ingroup AHE-helpers
/// \brief Clips and redistributes excess pixels based on the actual clip limit
/// value
...
void clip_and_redistribute(SrcHist const &src_hist, DstHist &dst_hist,
...
/// \brief Performs local histogram equalization on tiles of size (tile_width_x,
/// tile_width_y)
///        Then uses the clip limit to redistribute excess pixels above the
///        limit uniformly to other bins. The clip limit is specified as a
///        fraction i.e. a bin's value is clipped if bin_value >= clip_limit *
...
void non_overlapping_interpolated_clahe(
    ...
    std::size_t tile_width_x = 20, std::size_t tile_width_y = 20,
    double clip_limit = 0.03, std::size_t bin_width = 1.0, bool mask = false,
  ...
  auto extend_right =
      (tile_width_x - width % tile_width_x) % tile_width_x + tile_width_x;
  auto extend_bottom =
      (tile_width_y - height % tile_width_y) % tile_width_y + tile_width_y;
  ...
  copy_pixels(src_view, subimage_view(view(padded_img), top_left_x, top_left_y,
                                      width, height));
    ...
    for (std::ptrdiff_t i = top_left_y; i < bottom_right_y; ++i) {
      ...
      for (std::ptrdiff_t j = top_left_x; j < bottom_right_x; ++j) {
        ...
        if ((j - sample_x1) / tile_width_x == 0)
          prev_col_mask = false;
        else if ((j - sample_x1) / tile_width_x + 1 ==
                 new_width / tile_width_x - 1)
          next_col_mask = false;

        // Bilinear interpolation
        point_t top_left((j - sample_x1) / tile_width_x * tile_width_x +
                             sample_x1,
        ...
        point_t bottom_right(top_left.x + tile_width_x,
                             top_left.y + tile_width_y);
        ...
        long double numerator =
            ((prev_row_mask & prev_col_mask) * x2 *
                 prev_map[(top_left.x - sample_x1) / tile_width_x]
                         [channel_view(j, i)] +
            ...
            ((next_row_mask & prev_col_mask) * x2 *
                 next_map[(bottom_left.x - sample_x1) / tile_width_x]
                         [channel_view(j, i)] +
        ...
        if (mask && !src_mask[i - top_left_y][j - top_left_x]) {
          ...
          dst_view(j - top_left_x, i - top_left_y) =
              channel_convert<dst_channel_t>(
                  static_cast<source_channel_t>(numerator));
        }
      }
    }
  }
