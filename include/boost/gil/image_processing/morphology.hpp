...
/// \param dst_view - View which stores the final result of operations performed
/// by this function. \param kernel - Kernel matrix/structuring element
/// containing 0's and 1's which will be used for applying the required
/// morphological operation. \param identifier - Indicates the type of
/// morphological operation to be applied. \tparam SrcView type of source image.
...
void morph_impl(SrcView const &src_view, DstView const &dst_view,
                Kernel const &kernel, morphological_operation identifier) {
  ...
  for (std::ptrdiff_t view_row = 0; view_row < src_view.height(); ++view_row) {
    for (std::ptrdiff_t view_col = 0; view_col < src_view.width(); ++view_col) {
      ...
      for (std::size_t kernel_row = 0; kernel_row < kernel.size();
           ++kernel_row) {
        flip_ker_row =
            kernel.size() - 1 - kernel_row; // row index of flipped kernel

        for (std::size_t kernel_col = 0; kernel_col < kernel.size();
             ++kernel_col) {
          flip_ker_col =
          ...
          if (row_boundary >= 0 && row_boundary < src_view.height() &&
            ...
            if (identifier == morphological_operation::dilation) {
              target_element =
                  (std::max)(src_view(col_boundary, row_boundary)[0],
                             target_element);
            } else if (identifier == morphological_operation::erosion) {
              target_element =
                  (std::min)(src_view(col_boundary, row_boundary)[0],
                             target_element);
            }
          }
        }
...
void morph(SrcView const &src_view, DstView const &dst_view,
  ...
  for (std::size_t i = 0; i < src_view.num_channels(); i++) {
    morph_impl(nth_channel_view(src_view, i),
               nth_channel_view(view(intermediate_img), i), ker_mat,
               identifier);
  }
...
/// \param diff_view - View containing result of the subtraction of second view
/// from the first view. \tparam SrcView type of source/Input images used for
/// subtraction. \tparam DiffView type of image view containing the result of
/// subtraction.
template <typename SrcView, typename DiffView>
void difference_impl(SrcView const &src_view1, SrcView const &src_view2,
...
/// \param int_op_view - view for writing output and performing intermediate
/// operations. \param ker_mat - Kernel matrix/structuring element containing
/// 0's and 1's which will be used for applying dilation. \param iterations -
/// Specifies the number of times dilation is to be applied on the input image
/// view.
...
void dilate(SrcView const &src_view, IntOpView const &int_op_view,
...
/// \param int_op_view - view for writing output and performing intermediate
/// operations. \param ker_mat - Kernel matrix/structuring element containing
/// 0's and 1's which will be used for applying erosion. \param iterations -
/// Specifies the number of times erosion is to be applied on the input image
/// view. \tparam SrcView type of source image, models gil::ImageViewConcept.
/// \tparam IntOpView type of output image, models gil::MutableImageViewConcept.
/// \tparam Kernel type of structuring element.
template <typename SrcView, typename IntOpView, typename Kernel>
void erode(SrcView const &src_view, IntOpView const &int_op_view,
           Kernel const &ker_mat, int iterations) {
  copy_pixels(src_view, int_op_view);
  for (int i = 0; i < iterations; ++i)
    morph(int_op_view, int_op_view, ker_mat,
...
/// \param int_op_view - view for writing output and performing intermediate
/// operations. \param ker_mat - Kernel matrix/structuring element containing
/// 0's and 1's which will be used for applying the opening operation. \tparam
/// SrcView type of source image, models gil::ImageViewConcept. \tparam
/// IntOpView type of output image, models gil::MutableImageViewConcept. \tparam
/// Kernel type of structuring element.
template <typename SrcView, typename IntOpView, typename Kernel>
void opening(SrcView const &src_view, IntOpView const &int_op_view,
...
/// \param int_op_view - view for writing output and performing intermediate
/// operations. \param ker_mat - Kernel matrix/structuring element containing
/// 0's and 1's which will be used for applying the closing operation. \tparam
/// SrcView type of source image, models gil::ImageViewConcept. \tparam
/// IntOpView type of output image, models gil::MutableImageViewConcept. \tparam
/// Kernel type of structuring element.
template <typename SrcView, typename IntOpView, typename Kernel>
void closing(SrcView const &src_view, IntOpView const &int_op_view,
             Kernel const &ker_mat) {
...
/// \param src_view - Source/input image view.
/// \param dst_view - Destination view which will store the final result of
/// morphological gradient operation. \param ker_mat - Kernel matrix/structuring
/// element containing 0's and 1's which will be used for applying the
/// morphological gradient operation. \tparam SrcView type of source image,
/// models gil::ImageViewConcept. \tparam DstView type of output image, models
/// gil::MutableImageViewConcept. \tparam Kernel type of structuring element.
template <typename SrcView, typename DstView, typename Kernel>
...
/// \param src_view - Source/input image view.
/// \param dst_view - Destination view which will store the final result of top
/// hat operation. \param ker_mat - Kernel matrix/structuring element containing
/// 0's and 1's which will be used for applying the top hat operation. \tparam
/// SrcView type of source image, models gil::ImageViewConcept. \tparam DstView
/// type of output image, models gil::MutableImageViewConcept. \tparam Kernel
...
/// \param src_view - Source/input image view.
/// \param dst_view - Destination view which will store the final result of
/// black hat operation. \param ker_mat - Kernel matrix/structuring element
/// containing 0's and 1's which will be used for applying the black hat
/// operation. \tparam SrcView type of source image, models
/// gil::ImageViewConcept. \tparam DstView type of output image, models
/// gil::MutableImageViewConcept. \tparam Kernel type of structuring element.
template <typename SrcView, typename DstView, typename Kernel>
