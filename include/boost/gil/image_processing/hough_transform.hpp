...
///
/// Hough transform is a method of mapping (voting) an object which can be
/// described by equation to single point in accumulator array (also called
/// parameter space). Each set pixel in edge map votes for every shape it can be
/// part of. Circle and ellipse transforms are very costly to brute force, while
...
///
/// The input must be an edge map with grayscale pixels. Be aware of overflow
/// inside accumulator array. The theta parameter is best computed through
/// factory function provided in hough_parameter.hpp
template <typename InputView, typename OutputView>
void hough_line_transform(const InputView &input_view,
  ...
  std::ptrdiff_t r_upper_bound =
      r_lower_bound + radius.step_size * (radius.step_count - 1);
  ...
  for (std::ptrdiff_t y = 0; y < input_view.height(); ++y) {
    for (std::ptrdiff_t x = 0; x < input_view.width(); ++x) {
      ...
      for (std::size_t theta_index = 0; theta_index < theta.step_count;
           ++theta_index) {
        double theta_current =
            theta.start_point +
            theta.step_size * static_cast<double>(theta_index);
...
/// \ingroup HoughTransform
/// \brief Vote for best fit of a circle in parameter space according to
/// rasterizer
///
/// The input must be an edge map with grayscale pixels. Be aware of overflow
/// inside accumulator array. Rasterizer is used to rasterize a circle for
/// voting. The circle then is translated for every origin (x, y) in x y
/// parameter space. For available circle rasterizers, please look at
/// rasterization/circle.hpp
...
void hough_circle_transform_brute(
    ...
    const hough_parameter<std::ptrdiff_t> &y_parameter, ForwardIterator d_first,
  ...
  for (std::size_t radius_index = 0; radius_index < radius_parameter.step_count;
    ...
    const auto translate = [](std::vector<point_t> &points, point_t offset) {
      std::transform(points.begin(), points.end(), points.begin(),
                     [offset](point_t point) {
                       return point_t(point.x + offset.x, point.y + offset.y);
                     });
    };

    // in case somebody passes iterator to likes of std::vector<bool>
    typename std::iterator_traits<ForwardIterator>::reference current_image =
        *d_first;

    // the algorithm has to traverse over parameter space and look at input,
    // instead of vice versa, as otherwise it will call translate too many
    // times, as input is usually bigger than the coordinate portion of
    // parameter space. This might cause extensive cache misses
    for (std::size_t x_index = 0; x_index < x_parameter.step_count; ++x_index) {
      for (std::size_t y_index = 0; y_index < y_parameter.step_count;
           ++y_index) {
        const std::ptrdiff_t x =
            x_parameter.start_point + x_index * x_parameter.step_size;
        const std::ptrdiff_t y =
            y_parameter.start_point + y_index * y_parameter.step_size;
