...
///
///        Algorithm :-
///        1. Calculate histogram A(pixel) of input image and G(pixel) of
///        reference image.
///        2. Compute the normalized cumulative(CDF) histograms of A and G.
///        3. Match the histograms using transofrmation  => CDF(A(px)) =
///        CDF(G(px'))
///                                                      => px' = Inv-CDF
///                                                      (CDF(px))
///
...
/// \brief Overload for histogram matching algorithm, takes in a single source
/// histogram &
///        reference histogram and returns the color map used for histogram
///        matching.
///
template <typename SrcKeyType, typename RefKeyType>
std::map<SrcKeyType, SrcKeyType>
histogram_matching(histogram<SrcKeyType> const &src_hist,
...
/// \brief Overload for histogram matching algorithm, takes in source histogram,
/// reference
///        histogram & destination histogram and returns the color map used for
///        histogram matching as well as transforming the destination histogram.
///
...
/// \brief Overload for histogram matching algorithm, takes in both source,
/// reference &
///        destination image views and histogram matches the input image using
///        the reference image.
///
...
void histogram_matching(SrcView const &src_view, ReferenceView const &ref_view,
                        DstView const &dst_view, std::size_t bin_width = 1,
                        bool mask = false,
  ...
  for (std::size_t i = 0; i < channels; i++) {
    ...
    fill_histogram(nth_channel_view(src_view, i), src_histogram, bin_width,
                   false, false, mask, src_mask,
                   std::tuple<source_channel_t>(src_pixel_min),
                   std::tuple<source_channel_t>(src_pixel_max), true);
    fill_histogram(nth_channel_view(ref_view, i), ref_histogram, bin_width,
                   false, false, mask, ref_mask,
                   std::tuple<ref_channel_t>(ref_pixel_min),
                   std::tuple<ref_channel_t>(ref_pixel_max), true);
