...
struct perona_malik_conductivity {
  double kappa;
  template <typename Pixel> Pixel operator()(Pixel input) {
...
template <typename PixelType> using stencil_type = std::array<PixelType, 8>;
...
struct stencil_5points {
  ...
  compute_laplace(SubImageView view, point_t origin) {
    auto current = view(origin);
    stencil_type<typename SubImageView::value_type> stencil;
    using channel_type =
        typename channel_type<typename SubImageView::value_type>::type;
  ...
  template <typename Pixel> Pixel reduce(const stencil_type<Pixel> &stencil) {
    ...
    for (std::size_t index : {1u, 3u, 5u, 7u}) {
      static_transform(result, stencil[index], result,
                       std::plus<channel_type>{});
    }
    Pixel delta_t_pixel;
    static_fill(delta_t_pixel, delta_t);
    static_transform(result, delta_t_pixel, result,
...
struct stencil_9points_standard {
  ...
  compute_laplace(SubImageView view, point_t origin) {
    ...
    for (auto offset : offsets) {
      static_transform(view(origin.x + offset.x, origin.y + offset.y), current,
                       *out++, std::minus<channel_type>{});
    }
...
struct rgb_luminance {
  ...
  stencil_type<pixel_type> operator()(const stencil_type<pixel_type> &stencil) {
    ...
    std::transform(stencil.begin(), stencil.end(), output.begin(),
                   [](const pixel_type &pixel) {
                     float32_t luminance = 0.2126f * pixel[0] +
                                           0.7152f * pixel[1] +
                                           0.0722f * pixel[2];
...
enum class matlab_connectivity { minimal, maximal };

enum class matlab_conduction_method { exponential, quadratic };

template <typename InputView, typename OutputView>
void classic_anisotropic_diffusion(const InputView &input,
                                   const OutputView &output,
                                   unsigned int num_iter, double kappa) {
  anisotropic_diffusion(input, output, num_iter,
...
template <typename InputView, typename OutputView>
void matlab_anisotropic_diffusion(const InputView &input,
                                  const OutputView &output,
                                  unsigned int num_iter, double kappa,
...
template <typename InputView, typename OutputView>
void default_anisotropic_diffusion(const InputView &input,
                                   const OutputView &output,
                                   unsigned int num_iter, double kappa) {
  anisotropic_diffusion(input, output, num_iter,
...
void anisotropic_diffusion(const InputView &input, const OutputView &output,
                           unsigned int num_iter, LaplaceStrategy laplace,
  ...
  transform_pixels(input, subimage_view(result, 1, 1, width, height),
                   [](const input_pixel_type &pixel) {
                     ...
                     for (std::size_t i = 0; i < num_channels<pixel_type>{};
                          ++i) {
                       converted[i] = pixel[i];
                     }
                     return converted;
  ...
  for (unsigned int iteration = 0; iteration < num_iter; ++iteration) {
    for (std::ptrdiff_t relative_y = 0; relative_y < height; ++relative_y) {
      for (std::ptrdiff_t relative_x = 0; relative_x < width; ++relative_x) {
        ...
        std::transform(
            stencil.begin(), stencil.end(), diffusivity_stencil.begin(),
