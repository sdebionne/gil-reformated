...
///
///        Algorithm :-
///        1. If histogram A is to be equalized compute the cumulative histogram
///        of A.
...
/// \brief Overload for histogram equalization algorithm, takes in a single
/// source histogram
...
/// \brief Overload for histogram equalization algorithm, takes in both source
/// histogram &
///        destination histogram and returns the color map used for histogram
///        equalization as well as transforming the destination histogram.
///
template <typename SrcKeyType, typename DstKeyType>
std::map<SrcKeyType, DstKeyType>
histogram_equalization(histogram<SrcKeyType> const &src_hist,
                       histogram<DstKeyType> &dst_hist) {
  static_assert(std::is_integral<SrcKeyType>::value &&
  ...
  std::for_each(cumltv_srchist.begin(), cumltv_srchist.end(),
                [&](value_t const &v) {
                  DstKeyType trnsfrmd_key = static_cast<DstKeyType>(
                      (v.second * (max_key - min_key)) / sum + min_key);
...
void histogram_equalization(SrcView const &src_view, DstView const &dst_view,
                            std::size_t bin_width = 1, bool mask = false,
  ...
  for (std::size_t i = 0; i < channels; i++) {
    ...
    fill_histogram(nth_channel_view(src_view, i), h, bin_width, false, false,
                   mask, src_mask);
    h.normalize();
