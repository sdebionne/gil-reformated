...
values greater than or equal to threshold_value will be set to max_value else
set to 0 If direction is inverse: values greater than or equal to
threshold_value will be set to 0 else set to max_value
*/
...
Takes an image view and performes binary thresholding operation on each chennel.
If direction is regular:
values more or equal to threshold_value will be set to maximum numeric limit of
channel else 0 If direction is inverse: values more than or equal to
threshold_value will be set to 0 else maximum numeric limit of channel
*/
template <typename SrcView, typename DstView>
void threshold_binary(
    SrcView const &src_view, DstView const &dst_view,
...
chennel. If mode is truncate and direction is regular: values greater than
threshold_value will be set to threshold_value else no change If mode is
truncate and direction is inverse: values less than threshold_value will be set
to threshold_value else no change If mode is zeo and direction is regular:
