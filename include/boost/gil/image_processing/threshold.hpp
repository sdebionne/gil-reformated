...
template <typename SourceChannelT, typename ResultChannelT, typename SrcView,
...
/// \param threshold_direction - if regular, values greater than threshold_value
/// are set to max_value else set to 0; if inverse, values greater than
/// threshold_value are set to 0 else set to max_value.
...
/// Performs image binarization by thresholding channel value of each
/// pixel of given image view.
/// This variant of threshold_binary automatically deduces maximum value for
/// each channel of pixel based on channel type. If direction is regular, values
/// greater than threshold_value will be set to maximum numeric limit of channel
/// else 0. If direction is inverse, values greater than threshold_value will be
/// set to 0 else maximum numeric limit of channel.
template <typename SrcView, typename DstView>
void threshold_binary(
    SrcView const &src_view, DstView const &dst_view,
    typename channel_type<DstView>::type threshold_value,
...
template <typename SrcView, typename DstView>
void otsu_impl(SrcView const &src_view, DstView const &dst_view,
               threshold_direction direction) {
  // deciding output channel type and creating functor
  using source_channel_t = typename channel_type<SrcView>::type;

  std::array<std::size_t, 256> histogram{};
  // initial value of min is set to maximum possible value to compare histogram
  // data initial value of max is set to minimum possible value to compare
  // histogram data
  ...
  for (int t = 0; t < 256; t++) {
    ...
    var_intra_class = weight_back * weight_fore * (mean_back - mean_fore) *
                      (mean_back - mean_fore);
  ...
  if (sizeof(source_channel_t) > 1 &&
      std::is_unsigned<source_channel_t>::value) {
    threshold_binary(src_view, dst_view, (threshold * (max - min) / 255) + min,
                     direction);
  } else {
...
void threshold_optimal(
  ...
  if (mode == threshold_optimal_value::otsu) {
    for (std::size_t i = 0; i < src_view.num_channels(); i++) {
      detail::otsu_impl(nth_channel_view(src_view, i),
                        nth_channel_view(dst_view, i), direction);
    }
  }
}
...
template <typename SourceChannelT, typename ResultChannelT, typename SrcView,
          typename DstView, typename Operator>
void adaptive_impl(SrcView const &src_view, SrcView const &convolved_view,
                   DstView const &dst_view, Operator const &threshold_op) {
...
void threshold_adaptive(
    SrcView const &src_view, DstView const &dst_view,
    typename channel_type<DstView>::type max_value, std::size_t kernel_size,
  ...
  image<typename SrcView::value_type> temp_img(src_view.width(),
                                               src_view.height());
  typename image<typename SrcView::value_type>::view_t temp_view =
      view(temp_img);
  ...
  if (method == threshold_adaptive_method::mean) {
    ...
    kernel_1d<float> kernel(mean_kernel_values.begin(), kernel_size,
                            kernel_size / 2);
    ...
    kernel_2d<float> kernel(kernel_size, kernel_size / 2, kernel_size / 2);

    std::transform(gaussian_kernel_view.begin(), gaussian_kernel_view.end(),
                   kernel.begin(), [](gray32f_pixel_t pixel) -> float {
  ...
  if (direction == threshold_direction::regular) {
    detail::adaptive_impl<source_channel_t, result_channel_t>(
        src_view, temp_conv, dst_view,
        [max_value, constant](source_channel_t px,
                              source_channel_t threshold) -> result_channel_t {
...
template <typename SrcView, typename DstView>
void threshold_adaptive(
    SrcView const &src_view, DstView const &dst_view, std::size_t kernel_size,
    threshold_adaptive_method method = threshold_adaptive_method::mean,
    threshold_direction direction = threshold_direction::regular,
    int constant = 0) {
  // deciding output channel type and creating functor
  ...
  threshold_adaptive(src_view, dst_view, max_value, kernel_size, method,
                     direction, constant);
}
