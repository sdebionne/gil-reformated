...
// fixes ambigious call to std::abs,
// https://stackoverflow.com/a/30084734/4593721
#include <cmath>
#include <cstdlib>
...
#pragma warning(push)
#pragma warning(                                                               \
    disable : 4244) // 'argument': conversion from 'const Channel' to
                    // 'BaseChannelValue', possible loss of data
#endif

inline void compute_tensor_entries(boost::gil::gray16s_view_t dx,
...
inline detail::kernel_2d<T, Allocator>
generate_normalized_mean(std::size_t side_length) {
  ...
  detail::kernel_2d<T, Allocator> result(side_length, side_length / 2,
                                         side_length / 2);
...
template <typename T = float, typename Allocator = std::allocator<T>>
inline detail::kernel_2d<T, Allocator>
generate_gaussian_kernel(std::size_t side_length, double sigma) {
  if (side_length % 2 != 1)
    throw std::invalid_argument("kernel dimensions should be odd and equal");
  ...
  return detail::kernel_2d<T, Allocator>(values.begin(), values.size(), middle,
                                         middle);
}
...
template <typename T = float, typename Allocator = std::allocator<T>>
inline detail::kernel_2d<T, Allocator>
generate_dx_scharr(unsigned int degree = 1) {
  switch (degree) {
  case 0: {
    return detail::get_identity_kernel<T, Allocator>();
  }
  case 1: {
    detail::kernel_2d<T, Allocator> result(3, 1, 1);
    std::copy(detail::dx_scharr.begin(), detail::dx_scharr.end(),
              result.begin());
    return result;
  }
  default:
...
generate_dy_scharr(unsigned int degree = 1) {
  ...
  case 1: {
    detail::kernel_2d<T, Allocator> result(3, 1, 1);
    std::copy(detail::dy_scharr.begin(), detail::dy_scharr.end(),
              result.begin());
    return result;
  }
  default:
...
inline void compute_hessian_entries(GradientView dx, GradientView dy,
                                    OutputView ddxx, OutputView dxdy,
                                    OutputView ddyy) {
