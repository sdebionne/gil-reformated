...
void lanczos_at(ImageView input_view, ImageView output_view,
  ...
  boost::gil::static_transform(result_pixel, result_pixel, [](channel_t) {
  ...
  for (y_coord_t y_i =
    ...
    for (x_coord_t x_i =
      ...
      boost::gil::static_transform(result_pixel, input_view(source_x, source_y),
                                   result_pixel, op);
    }
  }
...
void scale_lanczos(ImageView input_view, ImageView output_view,
  ...
  for (y_coord_t y = 0; y < output_view.height(); ++y) {
    for (x_coord_t x = 0; x < output_view.width(); ++x) {
      boost::gil::lanczos_at(input_view, output_view, x / scale_x, y / scale_y,
                             x, y, a);
    }
  }
}
