...
///
/// Image views provide 1D iteration of their pixels via \p begin() and \p end()
/// methods, which makes it possible to use STL algorithms with them. However,
/// using nested loops over X and Y is in many cases more efficient. The
/// algorithms in this section resemble STL algorithms, but they abstract away
...
/// 1D-traversable image view has no gaps at the end of the rows. In other
/// words, if an x_iterator of that view is advanced past the last pixel in a
/// row it will move to the first pixel of the next row. When image views are
...
/// algorithms. For example, \p copy_pixels calls \p std::copy either for each
/// row, or, when the images are 1D-traversable, once for all pixels.
///
/// In addition, overloads are sometimes provided for the STL algorithms. For
/// example, std::copy for planar iterators is overloaded to perform \p
/// std::copy for each of the planes. \p std::copy over bitwise-copiable pixels
/// results in std::copy over unsigned char, which STL typically implements via
/// \p memmove.
///
/// As a result \p copy_pixels may result in a single call to \p memmove for
...
/// \ingroup ImageViewAlgorithm
/// \brief overloads of STL algorithms allowing more efficient implementation
/// when used with GIL constructs

/// \brief A generic binary operation on views
/// \ingroup ImageViewSTLAlgorithms
///
/// Use this class as a convenience superclass when defining an operation for
/// any image views. Many operations have different behavior when the two views
/// are compatible. This class checks for compatibility and invokes
/// apply_compatible(V1,V2) or apply_incompatible(V1,V2) of the subclass. You
/// must provide apply_compatible(V1,V2) method in your subclass, but
...
private:
  ...
  template <typename V1, typename V2>
  BOOST_FORCEINLINE result_type apply(V1 const &v1, V2 const &v2,
  ...
  template <typename V1, typename V2>
  BOOST_FORCEINLINE result_type apply_incompatible(V1 const & /*v1*/,
                                                   V2 const & /*v2*/) const {
    throw std::bad_cast();
  }
};
...
BOOST_FORCEINLINE auto copy(boost::gil::pixel<T, CS> *first,
                            boost::gil::pixel<T, CS> *last,
                            boost::gil::pixel<T, CS> *dst)
    -> boost::gil::pixel<T, CS> * {
  auto p = std::copy((unsigned char *)first, (unsigned char *)last,
                     (unsigned char *)dst);
...
///
/// Versions taking static and runtime views are provided. Versions taking
/// user-defined color convered are provided.
...
///
/// Invoked when one calls std::fill(I,I,V) with I being a iterator_from_2d
/// (which is a 1D iterator over the pixels in an image). For contiguous images
/// (i.e. images that have no alignment gap at the end of each row) it is more
/// efficient to use the underlying pixel iterator that does not check for the
/// end of rows. For non-contiguous images fill resolves to fill of each row
/// using the underlying pixel iterator, which is still faster
template <typename IL, typename V>
void fill(boost::gil::iterator_from_2d<IL> first,
          boost::gil::iterator_from_2d<IL> last, const V &val) {
  boost::gil::gil_function_requires<
      boost::gil::MutablePixelLocatorConcept<IL>>();
  if (first.is_1d_traversable()) {
...
template <typename It, typename P>
BOOST_FORCEINLINE void fill_aux(It first, It last, P const &p,
                                std::false_type) {
...
BOOST_FORCEINLINE void fill_pixels(View const &view, Value const &value) {
  if (view.is_1d_traversable()) {
    detail::fill_aux(view.begin().x(), view.end().x(), value,
                     is_planar<View>());
  } else {
    for (std::ptrdiff_t y = 0; y < view.height(); ++y)
      detail::fill_aux(view.row_begin(y), view.row_end(y), value,
                       is_planar<View>());
  }
}
...
BOOST_FORCEINLINE void destruct_range_impl(
    ...
    typename std::enable_if<mp11::mp_and<
        ...
        mp11::mp_not<std::is_trivially_destructible<
            typename std::iterator_traits<Iterator>>>>::value>::type * /*ptr*/
    = 0) {
  while (first != last) {
    first->~value_t();
    ++first;
...
BOOST_FORCEINLINE void destruct_range_impl(
    ...
    typename std::enable_if<
        mp11::mp_or<mp11::mp_not<std::is_pointer<Iterator>>,
                    std::is_trivially_destructible<
                        typename std::iterator_traits<Iterator>>>::value>::type
        * /* ptr */
    = nullptr) {}
...
BOOST_FORCEINLINE void destruct_pixels(View const &view) {
  if (view.is_1d_traversable()) {
    ...
    for (std::ptrdiff_t y = 0; y < view.height(); ++y)
      detail::destruct_aux(view.row_begin(y), view.row_end(y),
                           is_planar<View>());
  }
}
...
/// \defgroup ImageViewSTLAlgorithmsUninitializedFillPixels
/// uninitialized_fill_pixels \ingroup ImageViewSTLAlgorithms \brief
...
void uninitialized_fill_pixels(const View &view, const Value &val) {
  if (view.is_1d_traversable())
    detail::uninitialized_fill_aux(view.begin().x(), view.end().x(), val,
                                   is_planar<View>());
  else {
    typename View::y_coord_t y = 0;
    try {
      for (y = 0; y < view.height(); ++y)
        detail::uninitialized_fill_aux(view.row_begin(y), view.row_end(y), val,
                                       is_planar<View>());
    } catch (...) {
      for (typename View::y_coord_t y0 = 0; y0 < y; ++y0)
        detail::destruct_aux(view.row_begin(y0), view.row_end(y0),
                             is_planar<View>());
      throw;
    }
  }
...
/// \defgroup ImageViewSTLAlgorithmsDefaultConstructPixels
/// default_construct_pixels \ingroup ImageViewSTLAlgorithms \brief invokes the
...
BOOST_FORCEINLINE void default_construct_range(It first, It last) {
  default_construct_range_impl(first, last,
...
BOOST_FORCEINLINE void default_construct_pixels_impl(
  ...
  if (view.is_1d_traversable()) {
    detail::default_construct_aux(view.begin().x(), view.end().x(),
                                  is_planar<View>());
  } else {
    typename View::y_coord_t y = 0;
    try {
      for (y = 0; y < view.height(); ++y)
        detail::default_construct_aux(view.row_begin(y), view.row_end(y),
                                      is_planar<View>());
    } catch (...) {
      for (typename View::y_coord_t y0 = 0; y0 < y; ++y0)
        detail::destruct_aux(view.row_begin(y0), view.row_end(y0),
...
/// \brief Invokes the in-place default constructor on every pixel of the
...
template <typename View> void default_construct_pixels(View const &view) {
  detail::default_construct_pixels_impl<
      View, detail::has_trivial_pixel_constructor<
                View, is_planar<View>::value>::value>(view);
}
...
/// \defgroup ImageViewSTLAlgorithmsUninitializedCopyPixels
/// uninitialized_copy_pixels \ingroup ImageViewSTLAlgorithms \brief
...
void uninitialized_copy_pixels(View1 const &view1, View2 const &view2) {
  using is_planar = std::integral_constant<bool, is_planar<View1>::value &&
    ...
    try {
      for (y = 0; y < view1.height(); ++y)
        detail::uninitialized_copy_aux(view1.row_begin(y), view1.row_end(y),
                                       view2.row_begin(y), is_planar());
    } catch (...) {
      for (typename View1::y_coord_t y0 = 0; y0 < y; ++y0)
        detail::destruct_aux(view2.row_begin(y0), view2.row_end(y0),
                             is_planar());
      throw;
    }
  }
...
///
/// For contiguous images (i.e. images that have no alignment gap at the end of
/// each row) it is more efficient to use the underlying pixel iterator that
/// does not check for the end of rows. For non-contiguous images for_each_pixel
/// resolves to for_each of each row using the underlying pixel iterator, which
/// is still faster

/// \ingroup ImageViewSTLAlgorithmsForEachPixel
template <typename View, typename F> F for_each_pixel(View const &view, F fun) {
  if (view.is_1d_traversable()) {
...
/// \ingroup ImageViewSTLAlgorithms
/// \brief adobe::for_each_position for image views (passes locators, instead of
/// pixel references, to the function object)
...
/// GIL pixels are bitwise comparable, so memcmp is used. User-defined pixels
/// that are not bitwise comparable need to provide an overload
template <typename T, typename CS>
struct equal_n_fn<pixel<T, CS> const *, pixel<T, CS> const *> {
  BOOST_FORCEINLINE
  bool operator()(pixel<T, CS> const *i1, std::ptrdiff_t n,
                  pixel<T, CS> const *i2) const {
...
/// EqualPixels
/// Equal when both ranges are planar pointers of the same type. memcmp is
/// invoked for each channel plane
///  User-defined channels that are not bitwise comparable need to provide an
///  overload
template <typename IC, typename CS>
struct equal_n_fn<planar_pixel_iterator<IC, CS>,
                  planar_pixel_iterator<IC, CS>> {
  BOOST_FORCEINLINE
  bool operator()(planar_pixel_iterator<IC, CS> const i1, std::ptrdiff_t n,
...
struct equal_n_fn<boost::gil::iterator_from_2d<Loc>, It> {
  BOOST_FORCEINLINE
  bool operator()(boost::gil::iterator_from_2d<Loc> i1, std::ptrdiff_t n,
                  It i2) const {
    gil_function_requires<boost::gil::PixelLocatorConcept<Loc>>();
    gil_function_requires<boost::gil::PixelIteratorConcept<It>>();
    while (n > 0) {
      std::ptrdiff_t const num =
          std::min<std::ptrdiff_t>(n, i1.width() - i1.x_pos());
...
struct equal_n_fn<It, boost::gil::iterator_from_2d<Loc>> {
  BOOST_FORCEINLINE
  bool operator()(It i1, std::ptrdiff_t n,
                  boost::gil::iterator_from_2d<Loc> i2) const {
    gil_function_requires<boost::gil::PixelIteratorConcept<It>>();
    gil_function_requires<boost::gil::PixelLocatorConcept<Loc>>();
    while (n > 0) {
      std::ptrdiff_t const num =
          std::min<std::ptrdiff_t>(n, i2.width() - i2.x_pos());
...
struct equal_n_fn<boost::gil::iterator_from_2d<Loc1>,
  ...
  operator()(boost::gil::iterator_from_2d<Loc1> i1, std::ptrdiff_t n,
    ...
    while (n > 0) {
      std::ptrdiff_t num =
          std::min<const std::ptrdiff_t>(n, i2.width() - i2.x_pos());
...
/// iterator_from_2d (which is a 1D iterator over the pixels in an image).
/// Attempts to demote the source and destination iterators to simpler/faster
/// types if the corresponding range is contiguous. For contiguous images (i.e.
/// images that have no alignment gap at the end of each row) it is more
/// efficient to use the underlying pixel iterator that does not check for the
/// end of rows. If the underlying pixel iterator happens to be a fundamental
/// planar/interleaved pointer, the call may further resolve to memcmp.
...
BOOST_FORCEINLINE bool equal(boost::gil::iterator_from_2d<Loc1> first,
  ...
  if (first.is_1d_traversable()) {
    if (first2.is_1d_traversable())
      return boost::gil::detail::equal_n_fn<typename Loc1::x_iterator,
                                            typename Loc2::x_iterator>()(
          first.x(), n, first2.x());
    else
      return boost::gil::detail::equal_n_fn<
          typename Loc1::x_iterator, boost::gil::iterator_from_2d<Loc2>>()(
          first.x(), n, first2);
  } else {
    if (first2.is_1d_traversable())
      return boost::gil::detail::equal_n_fn<boost::gil::iterator_from_2d<Loc1>,
                                            typename Loc2::x_iterator>()(
          first, n, first2.x());
    else
      return boost::gil::detail::equal_n_fn<
...
/// \defgroup ImageViewSTLAlgorithmsTransformPixelPositions
/// transform_pixel_positions \ingroup ImageViewSTLAlgorithms \brief
/// adobe::transform_positions for image views (passes locators, instead of
/// pixel references, to the function object)
