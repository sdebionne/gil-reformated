...
////////////////////////////////////////////////////////////////////////////////////////
/// \file
/// \brief pixel step iterator, pixel image iterator and pixel dereference
/// iterator \author Lubomir Bourdev and Hailin Jin \n
...
/// \brief An iterator over two-dimensional locator. Useful for iterating over
/// the pixels of an image view. Models PixelIteratorConcept, PixelBasedConcept,
/// HasDynamicXStepTypeConcept

/// \ingroup PixelIteratorModelFromLocator PixelBasedModel
/// \brief Provides 1D random-access navigation to the pixels of the image.
/// Models: PixelIteratorConcept, PixelBasedConcept, HasDynamicXStepTypeConcept
///
/// Pixels are traversed from the top to the bottom row and from the left to the
/// right within each row

template <typename Loc2> // Models PixelLocatorConcept
class iterator_from_2d
    : public iterator_facade<iterator_from_2d<Loc2>, typename Loc2::value_type,
                             random_access_traversal_tag,
                             typename Loc2::reference, typename Loc2::coord_t> {
  GIL_CLASS_REQUIRE(Loc2, boost::gil, PixelLocatorConcept);

public:
  typedef iterator_facade<iterator_from_2d<Loc2>, typename Loc2::value_type,
                          random_access_traversal_tag, typename Loc2::reference,
  ...
  iterator_from_2d() {}
  iterator_from_2d(const Loc2 &p, int width, int x = 0)
      : _x(x), _width(width), _p(p) {}
  iterator_from_2d(const iterator_from_2d &pit)
      : _x(pit._x), _width(pit._width), _p(pit._p) {}
...
private:
  ...
  GIL_FORCEINLINE void advance(difference_type d) {
    if (_width == 0)
      return; // unfortunately we need to check for that. Default-constructed
              // images have width of 0 and the code below will throw if
              // executed.
    point_t delta;
...
template <typename Loc> struct dynamic_x_step_type<iterator_from_2d<Loc>> {
