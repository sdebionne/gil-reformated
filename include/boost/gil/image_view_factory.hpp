...
/// \brief Methods for constructing image views from raw data and for getting
/// raw data from views
...
interleaved_view(std::size_t width, std::size_t height, Iterator pixels,
...
template <typename Iterator>
typename type_from_x_iterator<Iterator>::view_t
interleaved_view(point2<std::size_t> dim, Iterator pixels,
                 std::ptrdiff_t rowsize_in_bytes) {
  typedef typename type_from_x_iterator<Iterator>::view_t RView;
...
/////////////////////////////
//  interleaved_view_get_raw_data, planar_view_get_raw_data - return pointers to
//  the raw data (the channels) of a basic homogeneous view.
/////////////////////////////
...
/// \ingroup ImageViewConstructors
/// \brief Returns C pointer to the the channels of a given color plane of a
/// planar homogeneous view.
template <typename HomogeneousView>
typename detail::channel_pointer_type<HomogeneousView>::type
...
/// \brief Function object that given a source pixel, returns it converted to a
/// given color space and channel depth. Models: PixelDereferenceAdaptorConcept
///
/// Useful in constructing a color converted view over a given image view
template <typename SrcConstRefP, typename DstP,
          typename CC =
              default_color_converter> // const_reference to the source pixel
                                       // and destination pixel value
class color_convert_deref_fn
    : public deref_base<color_convert_deref_fn<SrcConstRefP, DstP, CC>, DstP,
                        DstP, const DstP &, SrcConstRefP, DstP, false> {
private:
...
private:
  typedef color_convert_deref_fn<typename SrcView::const_t::reference, DstP, CC>
      deref_t;
...
// If the Src view has the same pixel type as the target, there is no need for
// color conversion
...
/// \brief Returns the type of a view that does color conversion upon
/// dereferencing its pixels \ingroup ImageViewTransformationsColorConvert
...
inline typename color_converted_view_type<View, DstP, CC>::type
color_converted_view(const View &src, CC cc) {
  return color_converted_view_type<View, DstP, CC>::make(src, cc);
}

/// \ingroup ImageViewTransformationsColorConvert
/// \brief overload of generic color_converted_view with the default
/// color-converter
...
template <typename View>
inline typename dynamic_y_step_type<View>::type
flipped_up_down_view(const View &src) {
  typedef typename dynamic_y_step_type<View>::type RView;
  return RView(src.dimensions(),
...
template <typename View>
inline typename dynamic_x_step_type<View>::type
flipped_left_right_view(const View &src) {
  typedef typename dynamic_x_step_type<View>::type RView;
  return RView(src.dimensions(), typename RView::xy_locator(
                                     src.xy_at(src.width() - 1, 0), -1, 1));
}
...
template <typename View>
inline typename dynamic_xy_step_transposed_type<View>::type
transposed_view(const View &src) {
  typedef typename dynamic_xy_step_transposed_type<View>::type RView;
  return RView(src.height(), src.width(),
               typename RView::xy_locator(src.xy_at(0, 0), 1, 1, true));
}
...
rotated180_view(const View &src) {
  typedef typename dynamic_xy_step_type<View>::type RView;
  return RView(src.dimensions(),
               typename RView::xy_locator(
                   src.xy_at(src.width() - 1, src.height() - 1), -1, -1));
}
...
/// \ingroup ImageViewTransformations
/// \brief view of a subsampled version of an image_view, stepping over a number
/// of channels in X and number of rows in Y
...
/// \ingroup ImageViewTransformations
/// \brief single-channel (grayscale) view of the N-th channel of a given
/// image_view
...
// nth_channel_view when the channels are not adjacent in memory. This can
// happen for multi-channel interleaved images or images with a step
template <typename View> struct __nth_channel_view_basic<View, false> {
  typedef
      typename view_type<typename channel_type<View>::type, gray_layout_t,
                         false, true, view_is_mutable<View>::value>::type type;
  ...
  static type make(const View &src, int n) {
    ...
    typedef typename iterator_adaptor_get_base<x_iterator_t>::type
        x_iterator_base_t;
    x_iterator_t sit(x_iterator_base_t(&(src(0, 0)[n])),
                     src.pixels().pixel_size());
    return type(src.dimensions(), locator_t(sit, src.pixels().row_size()));
  }
};

// nth_channel_view when the channels are together in memory (true for simple
// grayscale or planar images)
template <typename View> struct __nth_channel_view_basic<View, true> {
  typedef
      typename view_type<typename channel_type<View>::type, gray_layout_t,
                         false, false, view_is_mutable<View>::value>::type type;
  static type make(const View &src, int n) {
    typedef typename type::x_iterator x_iterator_t;
    return interleaved_view(src.width(), src.height(),
                            (x_iterator_t) & (src(0, 0)[n]),
                            src.pixels().row_size());
  }
};
...
private:
  ...
  // Determines whether the channels of a given pixel iterator are adjacent in
...
/// \brief Function object that returns a grayscale reference of the N-th
/// channel of a given reference. Models: PixelDereferenceAdaptorConcept.
...
/// n-th channel)
template <typename SrcP> // SrcP is a reference to PixelConcept (could be pixel
                         // value or const/non-const reference) Examples:
                         // pixel<T,L>, pixel<T,L>&, const pixel<T,L>&,
...
private:
  ...
  typedef typename pixel_reference_type<channel_t, gray_layout_t, false,
                                        is_mutable>::type ref_t;
...
public:
  ...
  static type make(const View &src, int n) { return AD::make(src, deref_t(n)); }
};
} // namespace detail

/// \brief Given a source image view type View, returns the type of an image
/// view over a single channel of View \ingroup
/// ImageViewTransformationsNthChannel
///
/// If the channels in the source view are adjacent in memory (such as planar
/// non-step view or single-channel view) then the return view is a
/// single-channel non-step view. If the channels are non-adjacent (interleaved
...
template <typename View>
typename nth_channel_view_type<View>::type nth_channel_view(const View &src,
                                                            int n) {
...
/// \ingroup ImageViewTransformations
/// \brief single-channel (grayscale) view of the K-th channel of a given
/// image_view. The channel index is a template parameter

namespace detail {
template <int K, typename View, bool AreChannelsTogether>
struct __kth_channel_view_basic;

// kth_channel_view when the channels are not adjacent in memory. This can
// happen for multi-channel interleaved images or images with a step
template <int K, typename View>
struct __kth_channel_view_basic<K, View, false> {
private:
  typedef
      typename kth_element_type<typename View::value_type, K>::type channel_t;

public:
  typedef typename view_type<channel_t, gray_layout_t, false, true,
...
// kth_channel_view when the channels are together in memory (true for simple
// grayscale or planar images)
template <int K, typename View> struct __kth_channel_view_basic<K, View, true> {
private:
  typedef
      typename kth_element_type<typename View::value_type, K>::type channel_t;

public:
  typedef typename view_type<channel_t, gray_layout_t, false, false,
                             view_is_mutable<View>::value>::type type;
  static type make(const View &src) {
    typedef typename type::x_iterator x_iterator_t;
    return interleaved_view(src.width(), src.height(),
...
/// PixelDereferenceAdaptorConcept. \ingroup PixelDereferenceAdaptorModel
///
/// If the input is a pixel value or constant reference, the function object is
/// immutable. Otherwise it is mutable (and returns non-const reference to the
/// k-th channel)
...
/// \brief Given a source image view type View, returns the type of an image
/// view over a given channel of View. \ingroup
