...
such that the minimum/maximum value of the source maps to the minimum/maximum
value of the destination. One implication of this is that the value 0 of signed
channels may not be preserved!

When creating new channel models, it is often a good idea to provide
specializations for the channel conversion algorithms, for example, for
performance optimizations. If the new model is an integral type that can be
signed, it is easier to define the conversion only for the unsigned type (\p
channel_converter_unsigned) and provide specializations of \p
channel_convert_to_unsigned and \p channel_convert_to_signed to convert between
the signed and unsigned type.
*/

/**
\defgroup ChannelConvertUnsignedAlgorithm channel_converter_unsigned
\ingroup ChannelConvertAlgorithm
\brief Convert one unsigned/floating point channel to another. Converts both the
channel type and range
 @{
 */

/// \brief This is the default implementation. Performance specializatons are
/// provided
template <typename SrcChannelV,
          typename DstChannelV> // Model ChannelValueConcept
struct channel_converter_unsigned
    : public std::unary_function<DstChannelV, SrcChannelV> {
  DstChannelV operator()(SrcChannelV src) const {
    return DstChannelV(src / channel_range<SrcChannelV>() *
                       channel_range<DstChannelV>());
  }

private:
  template <typename C> static double channel_range() {
    return double(channel_traits<C>::max_value()) -
           double(channel_traits<C>::min_value());
  }
};
...
template <>
struct channel_converter_unsigned<bits16, bits8>
    : public std::unary_function<bits16, bits8> {
  bits8 operator()(bits16 x) const { return static_cast<bits8>(x / 257); }
};
/// \brief 8 bit <-> 16 bit channel conversion
template <>
struct channel_converter_unsigned<bits8, bits16>
    : public std::unary_function<bits8, bits16> {
  bits16 operator()(bits8 x) const { return static_cast<bits16>(x * 257); }
};

/// \brief 8 bit <-> 32 bit channel conversion
template <>
struct channel_converter_unsigned<bits32, bits8>
    : public std::unary_function<bits32, bits8> {
  bits8 operator()(bits32 x) const { return static_cast<bits8>(x / 16843009); }
};
/// \brief 8 bit <-> 32 bit channel conversion
template <>
struct channel_converter_unsigned<bits8, bits32>
    : public std::unary_function<bits8, bits32> {
  bits32 operator()(bits8 x) const { return static_cast<bits32>(x * 16843009); }
};

/// \brief 8 bit <-> float0..1 channel conversion
template <>
struct channel_converter_unsigned<bits32f, bits8>
    : public std::unary_function<bits32f, bits8> {
  bits8 operator()(bits32f x) const {
    return static_cast<bits8>(x * 255 + 0.5f);
  }
};
/// \brief 8 bit <-> float0..1 channel conversion
template <>
struct channel_converter_unsigned<bits8, bits32f>
    : public std::unary_function<bits8, bits32f> {
  bits32f operator()(bits8 x) const { return static_cast<bits32f>(x / 255.0f); }
};

/// \brief 16 bit <-> float0..1 channel conversion
template <>
struct channel_converter_unsigned<bits32f, bits16>
    : public std::unary_function<bits32f, bits16> {
  bits16 operator()(bits32f x) const {
    return static_cast<bits16>(x * 65535 + 0.5f);
  }
};
/// \brief 16 bit <-> float0..1 channel conversion
template <>
struct channel_converter_unsigned<bits16, bits32f>
    : public std::unary_function<bits16, bits32f> {
  bits32f operator()(bits16 x) const {
    return static_cast<bits32f>(x / 65535.0f);
  }
};

/// \brief 16 bit <-> 32 bit channel conversion
template <>
struct channel_converter_unsigned<bits32, bits16>
    : public std::unary_function<bits32, bits16> {
  bits16 operator()(bits32 x) const { return static_cast<bits16>(x / 65537); }
};
/// \brief 16 bit <-> 32 bit channel conversion
template <>
struct channel_converter_unsigned<bits16, bits32>
    : public std::unary_function<bits16, bits32> {
...
struct channel_converter_unsigned<bits32, bits32f>
  ...
  bits32f operator()(bits32 x) const {
    // unfortunately without an explicit check it is possible to get a round-off
    // error. We must ensure that max_value of bits32 matches max_value of
    // bits32f
...
\brief Convert signed channel to unsigned. By default it is an identity
operation. Specializations are provided for signed integral channels \ingroup
ChannelConvertAlgorithm

@{
*/

/// \brief Converting from signed to unsigned integral channel.
/// It is both a unary function, and a metafunction (thus requires the 'type'
/// nested typedef, which equals result_type)
...
\brief Convert unsigned channel to signed. By default it is an identity
operation. Specializations are provided for signed integral channels \ingroup
ChannelConvertAlgorithm
...
template <>
struct channel_convert_to_signed<bits8s>
...
template <>
struct channel_convert_to_signed<bits16s>
...
template <>
struct channel_convert_to_signed<bits32s>
...
struct channel_converter
  ...
  DstChannelV operator()(SrcChannelV src) const {
    ...
    typedef channel_converter_unsigned<typename to_unsigned::result_type,
                                       typename to_signed::argument_type>
        converter_unsigned;
...
/// \ingroup ChannelConvertAlgorithm
/// \brief Same as channel_converter, except it takes the destination channel by
/// reference, which allows
///        us to move the templates from the class level to the method level.
...
/// \ingroup ChannelAlgorithm
/// \brief Multiplying unsigned channel values of the same type. Performs scaled
/// multiplication result = a * b / max_value
/// @{
...
struct channel_multiplier_unsigned
    : public std::binary_function<ChannelValue, ChannelValue, ChannelValue> {
  ChannelValue operator()(ChannelValue a, ChannelValue b) const {
    return ChannelValue(a / double(channel_traits<ChannelValue>::max_value()) *
                        b);
  }
};
...
/// \brief A function object to multiply two channels. result = a * b /
/// max_value
template <typename ChannelValue>
struct channel_multiplier
  ...
  ChannelValue operator()(ChannelValue a, ChannelValue b) const {
    ...
    typedef channel_multiplier_unsigned<typename to_unsigned::result_type>
        multiplier_unsigned;
    return to_signed()(
        multiplier_unsigned()(to_unsigned()(a), to_unsigned()(b)));
  }
};

/// \brief A function multiplying two channels. result = a * b / max_value
template <typename Channel> // Models ChannelConcept (could be a channel
                            // reference)
inline typename channel_traits<Channel>::value_type
channel_multiply(Channel a, Channel b) {
  return channel_multiplier<typename channel_traits<Channel>::value_type>()(a,
                                                                            b);
}
/// @}
