...
///
/// Conversion is done as a simple linear mapping of one channel range to the
/// other, such that the minimum/maximum value of the source maps to the
/// minimum/maximum value of the destination. One implication of this is that
/// the value 0 of signed channels may not be preserved!
///
...
/// detail::channel_convert_to_unsigned and \p
/// detail::channel_convert_from_unsigned to convert between the signed and
/// unsigned type.
///
/// Example:
/// \code
...
struct channel_converter_unsigned_integral_impl<uintmax_t, DstChannelV, false,
  ...
  DstChannelV operator()(uintmax_t src) const {
    static const uintmax_t div =
        unsigned_integral_max_value<uint32_t>::value /
        unsigned_integral_max_value<DstChannelV>::value;
...
template <> struct channel_converter_unsigned<uint32_t, float32_t> {
  ...
  float32_t operator()(uint32_t x) const {
    ...
    // error. We must ensure that max_value of uint32_t matches max_value of
    // float32_t
...
// nested alias, which equals result_type)
...
struct channel_converter {
  ...
  DstChannelV operator()(const SrcChannelV &src) const {
    ...
    using converter_unsigned =
        channel_converter_unsigned<typename to_unsigned::result_type,
...
template <typename ChannelValue> struct channel_multiplier {
  ...
  ChannelValue operator()(ChannelValue a, ChannelValue b) const {
    ...
    using multiplier_unsigned =
        channel_multiplier_unsigned<typename to_unsigned::result_type>;
