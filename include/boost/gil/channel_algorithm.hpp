...
template <> struct channel_convert_to_unsigned<bits16s> {
  ...
  type operator()(bits16s val) const {
    return static_cast<bits16>(val + 32768);
  }
};
...
template <> struct channel_convert_from_unsigned<bits16s> {
  ...
  type operator()(bits16 val) const {
    return static_cast<bits16s>(val - 32768);
  }
};
...
/// \brief This is the default implementation. Performance specializatons are
/// provided
template <typename ChannelValue> struct channel_multiplier_unsigned {
...
/// max_value
template <typename ChannelValue> struct channel_multiplier {
  typedef ChannelValue first_argument_type;
  typedef ChannelValue second_argument_type;
  typedef ChannelValue result_type;
  ChannelValue operator()(ChannelValue a, ChannelValue b) const {
