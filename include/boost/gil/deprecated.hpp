...
///        This file is provided as a courtesy to ease upgrading GIL client
///        code. Please make sure your code compiles when this file is not
///        included.
///
...
template <typename Img>
void resize_clobber_image(Img &img, const typename Img::x_coord_t &width,
                          const typename Img::y_coord_t &height) {
  img.recreate(width, height);
}

template <typename T> typename T::x_coord_t get_width(const T &a) {
  return a.width();
}
template <typename T> typename T::y_coord_t get_height(const T &a) {
  return a.height();
}
template <typename T> typename T::point_t get_dimensions(const T &a) {
  return a.dimensions();
}
template <typename T> std::size_t get_num_channels(const T &a) {
...
//#define row_bytes    row_size         // commented out because row_bytes is
// commonly used
