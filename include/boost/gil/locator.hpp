...
// The type of a locator or a view that has X and Y swapped. By default it is
// the same
template <typename T> struct transposed_type { typedef T type; };

/// \class pixel_2d_locator_base
/// \brief base class for models of PixelLocatorConcept
/// \ingroup PixelLocatorModel PixelBasedModel
///
/// Pixel locator is similar to a pixel iterator, but allows for 2D navigation
/// of pixels within an image view. It has a 2D difference_type and supports
/// random access operations like: \code
///     difference_type offset2(2,3);
///     locator+=offset2;
///     locator[offset2]=my_pixel;
/// \endcode
///
/// In addition, each coordinate acts as a random-access iterator that can be
/// modified separately:
/// "++locator.x()" or "locator.y()+=10" thereby moving the locator horizontally
/// or vertically.
///
/// It is called a locator because it doesn't implement the complete interface
/// of a random access iterator. For example, increment and decrement operations
/// don't make sense (no way to specify dimension). Also 2D difference between
/// two locators cannot be computed without knowledge of the X position within
/// the image.
///
/// This base class provides most of the methods and typedefs needed to create a
/// model of a locator. GIL provides two locator models as subclasses of \p
/// pixel_2d_locator_base. A memory-based locator, \p
/// byte_addressable_2d_locator and a virtual locator, \p virtual_2d_locator.
/// The minimum functionality a subclass must provide is this:
/// \code
/// class my_locator : public pixel_2d_locator_base<my_locator, ..., ...> {  //
/// supply the types for x-iterator and y-iterator
///        typedef ... const_t;                      // read-only locator
///
///        template <typename Deref> struct add_deref {
///            typedef ... type;                     // locator that invokes the
///            Deref dereference object upon pixel access static type make(const
///            my_locator& loc, const Deref& d);
///        };
///
///        my_locator();
///        my_locator(const my_locator& pl);
///
///        // constructors with dynamic step in y (and x). Only valid for
///        locators with dynamic steps my_locator(const my_locator& loc, coord_t
///        y_step); my_locator(const my_locator& loc, coord_t x_step, coord_t
///        y_step, bool transpose);
///
///        bool              operator==(const my_locator& p) const;
///
///        // return _references_ to horizontal/vertical iterators. Advancing
///        them moves this locator x_iterator&       x(); y_iterator&       y();
///        x_iterator const& x() const;
///        y_iterator const& y() const;
///
///        // return the vertical distance to another locator. Some models need
///        the horizontal distance to compute it y_coord_t y_distance_to(const
///        my_locator& loc2, x_coord_t xDiff) const;
///
///        // return true iff incrementing an x-iterator located at the last
///        column will position it at the first
///        // column of the next row. Some models need the image width to
///        determine that. bool              is_1d_traversable(x_coord_t width)
///        const;
/// };
/// \endcode
///
/// Models may choose to override some of the functions in the base class with
/// more efficient versions.
///

template <typename Loc, typename XIterator,
          typename YIterator> // The concrete subclass, the X-iterator and the
                              // Y-iterator
                              class pixel_2d_locator_base {
public:
  ...
  typedef typename std::iterator_traits<x_iterator>::reference
  ...
  template <std::size_t D> typename axis<D>::iterator &axis_iterator() {
  ...
  // Some locators can cache 2D coordinates for faster subsequent access. By
  // default there is no caching
...
namespace detail {
template <typename Loc> class locator_axis<0, Loc> {
...
template <typename Loc> class locator_axis<1, Loc> {
  typedef typename Loc::point_t point_t;
...
/// \brief Memory-based pixel locator. Models:
/// PixelLocatorConcept,HasDynamicXStepTypeConcept,HasDynamicYStepTypeConcept,HasTransposedTypeConcept
/// \ingroup PixelLocatorModel PixelBasedModel
///
/// The class takes a step iterator as a parameter. The step iterator provides
/// navigation along the vertical axis while its base iterator provides
/// horizontal navigation.
///
/// Each instantiation is optimal in terms of size and efficiency.
/// For example, xy locator over interleaved rgb image results in a step
/// iterator consisting of one std::ptrdiff_t for the row size and one native
/// pointer (8 bytes total). ++locator.x() resolves to pointer increment. At the
/// other extreme, a 2D navigation of the even pixels of a planar CMYK image
/// results in a step iterator consisting of one std::ptrdiff_t for the doubled
/// row size, and one step iterator consisting of one std::ptrdiff_t for the
/// horizontal step of two and a CMYK planar_pixel_iterator consisting of 4
/// pointers (24 bytes). In this case ++locator.x() results in four native
/// pointer additions.
///
/// Note also that \p byte_addressable_2d_locator does not require that its
/// element type be a pixel. It could be instantiated with an iterator whose \p
/// value_type models only \p Regular. In this case the locator models the
/// weaker RandomAccess2DLocatorConcept, and does not model PixelBasedConcept.
...
class byte_addressable_2d_locator
    : public pixel_2d_locator_base<
          byte_addressable_2d_locator<StepIterator>,
...
public:
  typedef pixel_2d_locator_base<
      ...
      parent_t;
  typedef byte_addressable_2d_locator<
      typename const_iterator_type<StepIterator>::type>
  ...
  template <typename Deref> struct add_deref {
    typedef byte_addressable_2d_locator<
        typename iterator_add_deref<StepIterator, Deref>::type>
        type;
    static type make(const byte_addressable_2d_locator<StepIterator> &loc,
                     const Deref &nderef) {
      return type(
          iterator_add_deref<StepIterator, Deref>::make(loc.y(), nderef));
    }
  };
  ...
  byte_addressable_2d_locator(const byte_addressable_2d_locator<SI> &loc,
                              coord_t y_step)
      : _p(loc.x(), loc.row_bytes() * y_step) {}
  template <typename SI>
  byte_addressable_2d_locator(const byte_addressable_2d_locator<SI> &loc,
                              coord_t x_step, coord_t y_step,
                              bool transpose = false)
      : _p(make_step_iterator(
               loc.x(),
               (transpose ? loc.row_bytes() : loc.pix_bytestep()) * x_step),
           (transpose ? loc.pix_bytestep() : loc.row_bytes()) * y_step) {}

  byte_addressable_2d_locator(x_iterator xit, std::ptrdiff_t row_bytes)
      : _p(xit, row_bytes) {}
  template <typename X>
  byte_addressable_2d_locator(const byte_addressable_2d_locator<X> &pl)
      : _p(pl._p) {}
  byte_addressable_2d_locator(const byte_addressable_2d_locator &pl)
      : _p(pl._p) {}
  ...
  x_iterator x_at(x_coord_t dx, y_coord_t dy) const {
    return byte_advanced(x(), byte_offset(dx, dy));
  }
  x_iterator x_at(const difference_type &d) const {
    return byte_advanced(x(), byte_offset(d.x, d.y));
  }
  this_t xy_at(x_coord_t dx, y_coord_t dy) const {
    return this_t(x_at(dx, dy), row_bytes());
  }
  this_t xy_at(const difference_type &d) const {
    return this_t(x_at(d.x, d.y), row_bytes());
  }
  reference operator()(x_coord_t dx, y_coord_t dy) const {
    return byte_advanced_ref(x(), byte_offset(dx, dy));
  }
  reference operator[](const difference_type &d) const {
    return byte_advanced_ref(x(), byte_offset(d.x, d.y));
  }
  this_t &operator+=(const difference_type &d) {
    byte_advance(x(), byte_offset(d.x, d.y));
    return *this;
  }
  this_t &operator-=(const difference_type &d) {
    byte_advance(x(), byte_offset(-d.x, -d.y));
  ...
  cached_location_t cache_location(const difference_type &d) const {
    return byte_offset(d.x, d.y);
  }
  cached_location_t cache_location(x_coord_t dx, y_coord_t dy) const {
    return byte_offset(dx, dy);
  }
  reference operator[](const cached_location_t &loc) const {
    return byte_advanced_ref(x(), loc);
  }
  ...
  std::ptrdiff_t pix_bytestep() const {
    return byte_step(x());
  } // distance in bytes between adjacent pixels on the same row

  bool is_1d_traversable(x_coord_t width) const {
    return row_bytes() - pix_bytestep() * width == 0;
  } // is there no gap at the end of each row?

  // Returns the vertical distance (it2.y-it1.y) between two x_iterators given
  // the difference of their x positions
  std::ptrdiff_t y_distance_to(const this_t &p2, x_coord_t xDiff) const {
    std::ptrdiff_t rowDiff =
        byte_distance(x(), p2.x()) - pix_bytestep() * xDiff;
...
private:
  ...
  std::ptrdiff_t byte_offset(x_coord_t x, y_coord_t y) const {
    return y * row_bytes() + x * pix_bytestep();
  }
  StepIterator _p;
};
...
template <typename SI>
struct color_space_type<byte_addressable_2d_locator<SI>>
    : public color_space_type<
          typename byte_addressable_2d_locator<SI>::parent_t> {};

template <typename SI>
struct channel_mapping_type<byte_addressable_2d_locator<SI>>
    : public channel_mapping_type<
          typename byte_addressable_2d_locator<SI>::parent_t> {};

template <typename SI>
struct is_planar<byte_addressable_2d_locator<SI>>
    : public is_planar<typename byte_addressable_2d_locator<SI>::parent_t> {};

template <typename SI>
struct channel_type<byte_addressable_2d_locator<SI>>
    : public channel_type<typename byte_addressable_2d_locator<SI>::parent_t> {
};
...
// Take the base iterator of SI (which is typically a step iterator) and change
// it to have a step in x
template <typename SI>
...
private:
  ...
  typedef
      typename dynamic_x_step_type<base_iterator_t>::type base_iterator_step_t;
  typedef typename iterator_adaptor_rebind<SI, base_iterator_step_t>::type
      dynamic_step_base_t;
