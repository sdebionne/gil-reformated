...
/// \brief Contains description of the boost.gil.histogram class, extensions
/// provided in place
///        of the default class, algorithms over the histogram class (both
///        extensions and the default class)
///
...
///
template <typename... T> struct hash_tuple {
...
///        Comparison is not in a lexicographic manner but on every element of
///        the tuple hence (2, 2) > (1, 3) evaluates to false
///
template <typename Tuple> bool tuple_compare(Tuple const &t1, Tuple const &t2) {
...
///
template <typename Tuple> struct tuple_limit {
  static constexpr Tuple min() {
    return min_impl(
        boost::mp11::make_index_sequence<std::tuple_size<Tuple>::value>{});
  }
...
/// \ingroup Histogram-Helpers
/// \brief Filler is used to fill the histogram class with all values between a
/// specified range
///        This functor is used when sparsefill is false, since all the keys
///        need to be present in that case. Currently on 1D implementation is
///        available, extend by adding specialization for 2D and higher
///        dimensional cases.
///
template <std::size_t Dimension> struct filler {
...
template <> struct filler<1> {
  template <typename Container, typename Tuple>
  void operator()(Container &hist, Tuple &lower, Tuple &upper,
                  std::size_t bin_width = 1) {
    for (auto i = std::get<0>(lower); std::get<0>(upper) - i >= bin_width;
         i += bin_width) {
      hist(i / bin_width) = 0;
    }
...
///
/// The class inherits over the std::unordered_map provided by STL. A complete
/// example/tutorial of how to use the class resides in the docs. Simple calling
/// syntax for a 3D dimensional histogram : \code histogram<int, int , int> h;
/// h(1, 1, 1) = 0;
/// \endcode
/// This is just a starter to what all can be achieved with it, refer to the
/// docs for the full demo.
///
template <typename... T>
class histogram : public std::unordered_map<std::tuple<T...>, double,
                                            detail::hash_tuple<T...>> {
  using base_t =
...
public:
  ...
  mapped_t &operator()(T... indices) {
    ...
    std::size_t const index_dimension =
        std::tuple_size<std::tuple<T...>>::value;
    std::size_t const histogram_dimension = dimension();
    static_assert(histogram_dimension == index_dimension,
  ...
  template <typename OtherType> bool equals(OtherType const &otherhist) const {
    bool check = (dimension() == otherhist.dimension());

    using other_value_t = typename OtherType::value_type;
    std::for_each(
        otherhist.begin(), otherhist.end(), [&](other_value_t const &v) {
  ...
  template <typename Tuple> bool is_tuple_compatible(Tuple const &) {
    std::size_t const tuple_size = std::tuple_size<Tuple>::value;
  ...
  key_t key_from_tuple(Tuple const &t) const {
    ...
    using new_index_list =
        typename std::conditional<index_list_size == 0,
                                  boost::mp11::mp_list_c<std::size_t, 0>,
                                  index_list>::type;

    std::size_t const min =
        boost::mp11::mp_min_element<new_index_list,
                                    boost::mp11::mp_less>::value;

    std::size_t const max =
        boost::mp11::mp_max_element<new_index_list,
                                    boost::mp11::mp_less>::value;

    static_assert((0 <= min && max < tuple_size) || index_list_size == 0,
                  "Index out of Range");
  ...
  key_t key_from_pixel(Pixel const &p) const {
    ...
    static_assert((0 <= min && max < pixel_dimension) || index_list_size == 0,
                  "Index out of Range");
  ...
  void fill(SrcView const &srcview, std::size_t bin_width = 1,
            bool applymask = false, std::vector<std::vector<bool>> mask = {},
            key_t lower = key_t(), key_t upper = key_t(),
    ...
    for (std::ptrdiff_t src_y = 0; src_y < srcview.height(); ++src_y) {
      ...
      for (std::ptrdiff_t src_x = 0; src_x < srcview.width(); ++src_x) {
        ...
        if (!setlimits || (detail::tuple_compare(lower, key) &&
                           detail::tuple_compare(key, upper)))
          base_t::operator[](key)++;
      }
    }
  }
  ...
  sub_histogram() {
    ...
    std::for_each(base_t::begin(), base_t::end(), [&](value_t const &v) {
      auto sub_key = detail::tuple_to_tuple(
          v.first, boost::mp11::index_sequence<Dimensions...>{});
  ...
  void normalize() {
    double sum = 0.0;
    std::for_each(base_t::begin(), base_t::end(),
                  [&](value_t const &v) { sum += v.second; });
  ...
  std::vector<key_t> sorted_keys() const {
    ...
    std::for_each(base_t::begin(), base_t::end(),
                  [&](value_t const &v) { sorted_keys.push_back(v.first); });
...
private:
  template <typename Tuple, std::size_t... I>
  key_t make_histogram_key(Tuple const &t,
                           boost::mp11::index_sequence<I...>) const {
    return std::make_tuple(
        static_cast<
            typename boost::mp11::mp_at<bin_t, boost::mp11::mp_size_t<I>>>(
            std::get<I>(t))...);
  }

  template <typename Tuple, std::size_t... I>
  static constexpr bool
  is_tuple_type_compatible(boost::mp11::index_sequence<I...>) {
  ...
  template <typename Tuple> static constexpr bool is_tuple_size_compatible() {
...
/// \brief Overload this function to provide support for boost::gil::histogram
/// or any other external histogram
///
/// Example :
/// \code
/// histogram<int> h;
...
/// @param accumulate  Input  Specify whether to accumulate over the values
/// already present in h (default = false)
/// @param sparsaefill Input  Specify whether to have a sparse or continuous
/// histogram (default = true)
/// @param applymask   Input  Specify if image mask is to be specified
/// @param mask        Input  Mask as a 2D vector. Used only if prev argument
/// specified
/// @param lower       Input  Lower limit on the values in histogram (default
/// numeric_limit::min() on axes)
/// @param upper       Input  Upper limit on the values in histogram (default
/// numeric_limit::max() on axes)
/// @param setlimits   Input  Use specified limits if this is true (default is
/// false) \brief Overload version of fill_histogram
///
/// Takes a third argument to determine whether to clear container before
/// filling. For eg, when there is a need to accumulate the histograms do \code
/// fill_histogram(view(img), hist, true);
/// \endcode
///
template <std::size_t... Dimensions, typename SrcView, typename... T>
void fill_histogram(
    SrcView const &srcview, histogram<T...> &hist, std::size_t bin_width = 1,
    bool accumulate = false, bool sparsefill = true, bool applymask = false,
  ...
  hist.template fill<Dimensions...>(srcview, bin_width, applymask, mask, lower,
                                    upper, setlimits);
}
...
/// #bins * log( #bins ) by sorting the keys and then calculating the cumulative
/// version.
///
template <typename Container> Container cumulative_histogram(Container const &);
...
histogram<T...> cumulative_histogram(histogram<T...> const &hist) {
  ...
  using pair_t = std::pair<typename histogram_t::key_type,
                           typename histogram_t::mapped_type>;
  ...
  if (dims == 1) {
    ...
    std::for_each(hist.begin(), hist.end(), [&](value_t const &v1) {
      auto cumulative_counter =
          static_cast<typename histogram_t::mapped_type>(0);
