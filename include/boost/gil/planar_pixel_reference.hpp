...
/// \brief A homogeneous color base whose element is a channel reference. Models
/// HomogeneousColorBaseConcept, HomogeneousPixelConcept. This class is used as
...
struct planar_pixel_reference
  ...
  typedef detail::homogeneous_color_base<ChannelReference, layout<ColorSpace>,
...
public:
  ...
  typedef planar_pixel_reference<channel_const_reference, ColorSpace>
      const_reference;
...
private:
  template <typename Pixel> static void check_compatible() {
    gil_function_requires<
        PixelsCompatibleConcept<Pixel, planar_pixel_reference>>();
  }
};
...
struct kth_element_type<planar_pixel_reference<ChannelReference, ColorSpace>,
                        K> {
...
/// \brief Metafunction predicate that flags planar_pixel_reference as a model
...
/// by PixelBasedConcept \ingroup PixelModelPlanarRef
template <typename ChannelReference, typename ColorSpace>
...
/// \brief Specifies that planar_pixel_reference represents a planar construct.
...
inline void swap(boost::gil::planar_pixel_reference<CR, CS> x, R &y) {
...
inline void
swap(typename boost::gil::planar_pixel_reference<CR, CS>::value_type &x,
     boost::gil::planar_pixel_reference<CR, CS> y) {
  boost::gil::swap_proxy<
      typename boost::gil::planar_pixel_reference<CR, CS>::value_type>(x, y);
}
...
/// \ingroup PixelModelPlanarRef
template <typename CR, typename CS>
inline void swap(boost::gil::planar_pixel_reference<CR, CS> x,
                 boost::gil::planar_pixel_reference<CR, CS> y) {
  boost::gil::swap_proxy<
      typename boost::gil::planar_pixel_reference<CR, CS>::value_type>(x, y);
}
