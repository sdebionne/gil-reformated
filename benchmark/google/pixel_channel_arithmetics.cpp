...
static void std_algo(benchmark::State &state) {
  ...
  if (!std::all_of(img.begin(), img.end(),
...
static void gil_algo_stdfunction(benchmark::State &state) {
  ...
  // Pixel operation
  auto op = std::bind(
      gil::pixel_plus_t<gray16_pixel_t, gray16_pixel_t, gray16_pixel_t>(), _1,
      gray16_pixel_t{1});
...
static void gil_algo_lambda(benchmark::State &state) {
  ...
  auto op2 = [second_arg = gray16_pixel_t{1}](gray16_pixel_t value) {
    return gil::pixel_plus_t<gray16_pixel_t, gray16_pixel_t, gray16_pixel_t>()(
        value, second_arg);
  };
...
static void gil_algo_lambda_copy(benchmark::State &state) {
  ...
  if (!std::all_of(output_view.begin(), output_view.end(),
                   [&state](auto p) { return p == 1; }))
...
static void gil_algo_pixel_vector(benchmark::State &state) {
  ...
  for (auto _ : state) {
    for (auto &pixel : img)
      pixel.at(std::integral_constant<int, 0>{}) =
          pixel.at(std::integral_constant<int, 0>{}) + 1;
  }
