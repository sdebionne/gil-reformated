...
//
// Benchmark of various pixel iteration and access methods for a contiguous
// images composed of homogeneous pixels with single 8-bit integer channel
// (grayscale).
//
#include <algorithm>
#include <array>
...
public:
  ...
  getExperimentValues() const final {
    // Generate sequence of square image sizes
    int const initial_power = static_cast<int>(
        std::ceil(std::log(initial_image_size_) / std::log(2)));
  ...
  void
  setUp(const celero::TestFixture::ExperimentValue &experimentValue) final {
  ...
  void clear_histogram() { this->histogram_.fill(0); }

  void assert_histogram() {
#ifndef NDEBUG
    std::size_t const total =
        std::accumulate(this->histogram_.cbegin(), this->histogram_.cend(), 0U);
...
BASELINE_F(HistogramGray8, 1d_raw_pointer, RandomImageGray8Fixture, samples,
           iterations) {
...
BENCHMARK_F(HistogramGray8, 1d_index_operator, RandomImageGray8Fixture, samples,
            iterations) {
  this->clear_histogram();
  auto &h = this->histogram_;
...
BENCHMARK_F(HistogramGray8, 1d_index_at, RandomImageGray8Fixture, samples,
            iterations) {
  this->clear_histogram();
  auto &h = this->histogram_;
...
BENCHMARK_F(HistogramGray8, 1d_iterator, RandomImageGray8Fixture, samples,
            iterations) {
  this->clear_histogram();
  auto &h = this->histogram_;
...
BENCHMARK_F(HistogramGray8, 1d_reverse_iterator, RandomImageGray8Fixture,
            samples, iterations) {
  this->clear_histogram();
  auto &h = this->histogram_;
...
BENCHMARK_F(HistogramGray8, 1d_for_each_pixel, RandomImageGray8Fixture, samples,
            iterations) {
  this->clear_histogram();
  auto &h = this->histogram_;
  ...
  gil::for_each_pixel(view,
                      [&h](gil::gray8c_pixel_t const &pixel) { ++h[pixel]; });

  this->assert_histogram();
}

BENCHMARK_F(HistogramGray8, 2d_call_operator, RandomImageGray8Fixture, samples,
            iterations) {
  this->clear_histogram();
  auto &h = this->histogram_;
  ...
  for (std::ptrdiff_t y = 0; y < view_h; y++) {
    for (std::ptrdiff_t x = 0; x < view_w; x++) {
      // The binary call operator() computes the location of the pixel in a 2D
      // grid, which involves addition and multiplication, thus it costs
      // performance.
      ++h[view(x, y)];
    }
  }
...
BENCHMARK_F(HistogramGray8, 2d_xy_locator, RandomImageGray8Fixture, samples,
            iterations) {
  this->clear_histogram();
  auto &h = this->histogram_;
...
BENCHMARK_F(HistogramGray8, 2d_x_iterator, RandomImageGray8Fixture, samples,
            iterations) {
  this->clear_histogram();
  auto &h = this->histogram_;
...
BENCHMARK_F(HistogramGray8, 2d_y_iterator, RandomImageGray8Fixture, samples,
            iterations) {
  this->clear_histogram();
  auto &h = this->histogram_;
