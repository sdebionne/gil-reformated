...
void hstack(const std::vector<View> &views, const View &output_view) {
  if (views.size() == 0) {
    throw std::invalid_argument(
        "empty views vector is passed - cannot create stacked image");
  }
  ...
  for (const auto &view : views) {
    if (view.height() != height) {
      throw std::invalid_argument(
          "one or many views are not of the same height");
    }
  }

  std::ptrdiff_t full_width = std::accumulate(
  ...
  if (output_view.width() != full_width || output_view.height() != height) {
    throw std::invalid_argument(
        "the destination view is not of the right dimensions");
  }

  std::ptrdiff_t current_x = 0;
  for (std::size_t i = 0; i < views.size(); ++i) {
    auto subview = subimage_view(output_view, current_x, 0, views[i].width(),
                                 views[i].height());
...
void vstack(const std::vector<View> &views, const View &output_view) {
  ...
  auto full_height = std::accumulate(
      views.begin(), views.end(), 0,
  ...
  for (const auto &view : views) {
    if (view.width() != width) {
      throw std::invalid_argument(
          "one or many views are not of the same width");
    }
  }
  ...
  for (std::size_t i = 0; i < views.size(); ++i) {
    auto subview = subimage_view(output_view, 0, current_y, views[i].width(),
                                 views[i].height());
    copy_pixels(views[i], subview);
