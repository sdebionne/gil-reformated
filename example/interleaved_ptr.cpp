...
// FIXME: Review and remove if possible: gcc doesn't compile unless we
// forward-declare at_c before we include gil...
namespace boost {
namespace gil {
...
int main(int argc, char *argv[]) {
  ...
  boost::function_requires<MemoryBasedIteratorConcept<
      memory_based_step_iterator<rgb8_interleaved_ptr>>>();
  ...
  rgb8_interleaved_view_t src_view =
      interleaved_view(img.width(), img.height(), rgb8_interleaved_ptr(raw_ptr),
                       view(img).pixels().row_size());

  // Apply view transformations and algorithms on it
  write_view("out-interleaved_ptr.jpg",
             nth_channel_view(flipped_up_down_view(src_view), 1), jpeg_tag{});
