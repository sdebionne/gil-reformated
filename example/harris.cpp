  ...
  for (long int y = 0; y < original.height(); ++y) {
    for (long int x = 0; x < original.width(); ++x) {
      ...
      double red_intensity =
          original(x, y).at(std::integral_constant<int, 0>{}) /
          max_channel_intensity;
      double green_intensity =
          original(x, y).at(std::integral_constant<int, 1>{}) /
          max_channel_intensity;
      double blue_intensity =
          original(x, y).at(std::integral_constant<int, 2>{}) /
          max_channel_intensity;
...
void calculate_gradients(gil::gray8_view_t input, gil::gray16_view_t x_gradient,
  ...
  using channel_t = typename std::remove_reference<
      decltype(std::declval<gil::gray16_pixel_t>().at(
          std::integral_constant<int, 0>{}))>::type;

  constexpr double x_kernel[3][3] = {{1, 0, -1}, {2, 0, -2}, {1, 0, -1}};
  constexpr double y_kernel[3][3] = {{1, 2, 1}, {0, 0, 0}, {-1, -2, -1}};
  ...
  for (y_coord_t y = 1; y < input.height() - 1; ++y) {
    for (x_coord_t x = 1; x < input.width() - 1; ++x) {
      ...
      boost::gil::static_transform(x_result, x_result, [](channel_t) {
        return static_cast<channel_t>(0);
      });
      gil::gray16_pixel_t y_result;
      boost::gil::static_transform(y_result, y_result, [](channel_t) {
        return static_cast<channel_t>(0);
      });
...
int main(int argc, char *argv[]) {
  if (argc != 6) {
    std::cout << "usage: " << argv[0]
              << " <input.png> <odd-window-size>"
                 " <discrimination-constant> <harris-response-threshold> "
                 "<output.png>\n";
    return -1;
  }
  ...
  gil::compute_tensor_entries(x_gradient, y_gradient, gil::view(m11),
  ...
  gil::compute_harris_responses(gil::view(m11), gil::view(m12_21),
                                gil::view(m22), gil::view(gaussian_kernel),
                                discrimnation_constant,
                                gil::view(harris_response));

  auto corner_points =
      suppress(gil::view(harris_response), harris_response_threshold);
