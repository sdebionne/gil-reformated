...
int main() {
  // Create a histogram class. Use uint or unsigned short as the default axes
  // type in most cases.
  ...
  fill_histogram(
      ...
      false, // Specify whether to accumulate over the values already present in
             // h (default = false)
      true, // Specify whether to have a sparse or continuous histogram (default
            // = true)
      false, // Specify if image mask is to be specified
      {{}},  // Mask as a 2D vector. Used only if prev argument specified
      {0},   // Lower limit on the values in histogram (default
             // numeric_limit::min() on axes)
      {255}, // Upper limit on the values in histogram (default
