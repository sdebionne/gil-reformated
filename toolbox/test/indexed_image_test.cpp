...
BOOST_AUTO_TEST_CASE(index_image_test) {
  ...
  }
  {
    ...
    index =
        *img.get_indices_view().xy_at(0, 0); // verify values along first row
...
BOOST_AUTO_TEST_CASE(index_image_view_test) {
  ...
  auto indices_view = bg::interleaved_view(
      width, height, (bg::gray8_image_t::view_t::x_iterator)indices.data(),
      width); // row size in bytes

  auto palette_view = bg::interleaved_view(
      100, 1, (bg::rgb8_image_t::view_t::x_iterator)palette.data(),
