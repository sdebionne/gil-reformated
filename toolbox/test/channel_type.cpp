...
BOOST_AUTO_TEST_CASE(channel_type_test) {
  static_assert(boost::is_same<unsigned char,
                               bg::channel_type<bg::rgb8_pixel_t>::type>::value,
                "");

  // float32_t is a scoped_channel_value object
  static_assert(
      boost::is_same<bg::float32_t,
                     bg::channel_type<bg::rgba32f_pixel_t>::type>::value,
      "");

  // channel_type for bit_aligned images doesn't work with standard gil.
  using image_t =
      bg::bit_aligned_image4_type<4, 4, 4, 4, bg::rgb_layout_t>::type;
