...
write_view(filename,
           create_mandel_view(1000, 600, rgb8_pixel_t(0, 0, 255),
...
BOOST_AUTO_TEST_CASE(subimage_test) {
  run_subimage_test<rgb8_image_t, tag_t>(bmp_filename, point_t(0, 0),
                                         point_t(1000, 1));

  run_subimage_test<rgb8_image_t, tag_t>(bmp_filename, point_t(39, 7),
                                         point_t(50, 50));
}
...
BOOST_AUTO_TEST_CASE(dynamic_image_test) {
  ...
  read_image(bmp_filename.c_str(), runtime_image, tag_t());

#ifdef BOOST_GIL_IO_TEST_ALLOW_WRITING_IMAGES
  write_view(bmp_out + "dynamic_image_test.bmp", view(runtime_image), tag_t());
#endif // BOOST_GIL_IO_TEST_ALLOW_WRITING_IMAGES
}
