...
#define GENERATE_SUBIMAGE_TEST(z, n, data)                                     \
  BOOST_AUTO_TEST_CASE(BOOST_PP_CAT(                                           \
      BOOST_PP_CAT(BOOST_PP_CAT(subimage_test, data), n), bit_bit_aligned)) {  \
    using namespace std;                                                       \
    using namespace boost;                                                     \
    using namespace gil;                                                       \
    ...
    BOOST_CHECK(equal_pixels(const_view(img1),                                 \
                             subimage_view(view(img3), top_left, dim)));       \
  }
...
BOOST_AUTO_TEST_CASE(subimage_test_8) {
  ...
  BOOST_CHECK(equal_pixels(const_view(img1), const_view(img2)));
  BOOST_CHECK(
      equal_pixels(const_view(img1), subimage_view(view(img3), top_left, dim)));
}
