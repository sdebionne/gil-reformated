...
BOOST_PP_REPEAT_FROM_TO(1, 8, GENERATE_TILE_STRIP_COMPARISON_BIT_ALIGNED_RGB,
                        (rgb, contig))
BOOST_PP_REPEAT_FROM_TO(9, 11, GENERATE_TILE_STRIP_COMPARISON_BIT_ALIGNED_RGB,
                        (rgb, contig))
...
BOOST_AUTO_TEST_CASE(read_tile_and_compare_with_rgb_contig_strip_8) {
  ...
  BOOST_CHECK_EQUAL(equal_pixels(const_view(img_strip), const_view(img_tile)),
                    true);
}
