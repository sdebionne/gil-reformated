...
BOOST_AUTO_TEST_CASE(read_pixel_density_test) {
  ...
  backend_t backend = read_image_info(jpeg_in + "EddDawson/36dpi.jpg", tag_t());
  ...
  write_settings.set_pixel_dimensions(
      backend._info._width, backend._info._height,
      backend._info._pixel_width_mm, backend._info._pixel_height_mm);

  stringstream in_memory(ios_base::in | ios_base::out | ios_base::binary);
  write_view(in_memory, view(img), write_settings);

  typedef get_reader_backend<stringstream, tag_t>::type backend2_t;

  backend2_t backend2 = read_image_info(in_memory, tag_t());

  // Because of rounding the two results differ slightly.
  if (std::abs(backend._info._pixel_width_mm - backend2._info._pixel_width_mm) >
          10.0 ||
      std::abs(backend._info._pixel_height_mm -
               backend2._info._pixel_height_mm) > 10.0) {
...
BOOST_AUTO_TEST_CASE(read_reference_images_test) {
  ...
  write_view(jpeg_out + "rgb8_test.jpg", view(img), tag_t());
...
#ifdef BOOST_GIL_IO_TEST_ALLOW_WRITING_IMAGES
  write_view(jpeg_out + "fast_dct_read_test.jpg", view(img), tag_t());
#endif // BOOST_GIL_IO_TEST_ALLOW_WRITING_IMAGES
}
