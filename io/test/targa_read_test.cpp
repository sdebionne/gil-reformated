...
template <typename Image> void test_targa_scanline_reader(string filename) {
...
template <typename Image> void write(Image &img, const string &file_name) {
  ...
  write_view(targa_out + file_name, view(img), tag_t());
...
BOOST_AUTO_TEST_CASE(read_reference_images_test) {
  ...
  // 32BPP_compressed.tga
  {
    ...
    BOOST_CHECK(view(img)(width - 1, height - 1) ==
                rgba8_pixel_t(248, 0, 248, 255));
...
#ifdef BOOST_GIL_IO_TEST_ALLOW_WRITING_IMAGES
    write_view(targa_out + "targa_partial.tga", view(img), tag_t());
#endif // BOOST_GIL_IO_TEST_ALLOW_WRITING_IMAGES
  }
}
