...
BOOST_AUTO_TEST_CASE(write_test) {
  mandel_view<rgb8_pixel_t>::type v = create_mandel_view(
      200, 200, rgb8_pixel_t(0, 0, 255), rgb8_pixel_t(0, 255, 0));
  ...
  // test writing all supported image types
  {
    ...
    write_view(pnm_out + "p4_write_test.pnm", view(dst), pnm_tag());
  }

  {
    ...
    write_view(pnm_out + "p5_write_test.pnm", view(dst), pnm_tag());
  }

  { write_view(pnm_out + "p6_write_test.pnm", v, pnm_tag()); }
}
