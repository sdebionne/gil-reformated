...
BOOST_AUTO_TEST_CASE(read_image_info_using_string) {
  {
    ...
    backend_t backend = read_image_info(pnm_filename, tag_t());
...
BOOST_AUTO_TEST_CASE(subimage_test) {
  run_subimage_test<rgb8_image_t, tag_t>(pnm_filename, point_t(0, 0),
                                         point_t(50, 50));

  run_subimage_test<rgb8_image_t, tag_t>(pnm_filename, point_t(103, 103),
                                         point_t(50, 50));
}
...
BOOST_AUTO_TEST_CASE(dynamic_image_test) {
  ...
  read_image(pnm_filename.c_str(), runtime_image, tag_t());

#ifdef BOOST_GIL_IO_TEST_ALLOW_WRITING_IMAGES
  write_view(pnm_out + "dynamic_image_test.pnm", view(runtime_image), tag_t());
#endif // BOOST_GIL_IO_TEST_ALLOW_WRITING_IMAGES
}
