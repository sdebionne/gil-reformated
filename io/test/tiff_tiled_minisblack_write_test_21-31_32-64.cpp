...
BOOST_PP_REPEAT_FROM_TO(21, 32, GENERATE_WRITE_TILE_BIT_ALIGNED_MINISBLACK,
                        minisblack)
...
BOOST_AUTO_TEST_CASE(write_tile_and_compare_with_minisblack_strip_32) {
  ...
  write_view(tiff_out + "write_minisblack_tile_and_compare_with_32.tif",
             view(img_strip), info);
  read_image(tiff_out + "write_minisblack_tile_and_compare_with_32.tif",
             img_saved, tag_t());

  BOOST_CHECK_EQUAL(equal_pixels(const_view(img_strip), const_view(img_saved)),
                    true);
...
#ifdef BOOST_GIL_IO_TEST_ALLOW_WRITING_IMAGES
  write_view(tiff_out + "write_minisblack_tile_and_compare_with_64.tif",
             view(img_strip), info);
  read_image(tiff_out + "write_minisblack_tile_and_compare_with_64.tif",
             img_saved, tag_t());
