...
BOOST_AUTO_TEST_CASE(write_view_test) {
...

  {
    ...
    write_view(filename,
               create_mandel_view(320, 240, bgr8_pixel_t(255, 0, 0),
...
BOOST_AUTO_TEST_CASE(subimage_test) {
  run_subimage_test<rgba8_image_t, tag_t>(png_filename, point_t(0, 0),
                                          point_t(50, 50));

  run_subimage_test<rgba8_image_t, tag_t>(png_filename, point_t(135, 95),
                                          point_t(50, 50));
}
...
BOOST_AUTO_TEST_CASE(dynamic_image_test) {
  ...
  read_image(png_filename.c_str(), runtime_image, tag_t());

#ifdef BOOST_GIL_IO_TEST_ALLOW_WRITING_IMAGES
  write_view(png_out + "dynamic_image_test.png", view(runtime_image), tag_t());
#endif // BOOST_GIL_IO_TEST_ALLOW_WRITING_IMAGES
}
