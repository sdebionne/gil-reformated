...
template <typename Image> void write(Image &img, const string &file_name) {
#ifdef BOOST_GIL_IO_TEST_ALLOW_WRITING_IMAGES
  write_view(bmp_out + file_name, view(img), tag_t());
...
BOOST_AUTO_TEST_CASE(read_header_test) {
  {
    ...
    backend_t backend = read_image_info(bmp_filename, tag_t());
...
BOOST_AUTO_TEST_CASE(read_reference_images_image_iterator_test) {
  ...
  // g01bw.bmp - black and white palette (#000000,#FFFFFF)
  test_scanline_reader<rgba8_image_t, bmp_tag>(
  ...
  // not supported
  // g04rle.bmp - RLE compressed.
  // test_scanline_reader< bgra8_image_t, bmp_tag >( string( bmp_in +
  // "g01bg.bmp" ).c_str() );
  ...
  // that is still encountered sometimes. It has 3-byte palette
  // entries (instead of 4), and 16-bit width/height fields (instead of 32).
  test_scanline_reader<rgb8_image_t, bmp_tag>(
  ...
  // g24.bmp - 24-bit color (BGR)
  test_scanline_reader<bgr8_image_t, bmp_tag>(
      string(bmp_in + "g24.bmp").c_str());

  // g32def.bmp - 24-bit color (8 bits wasted), biCompression=BI_RGB (no
  // bitfields, defaults to BGRx)
  test_scanline_reader<bgra8_image_t, bmp_tag>(
...
#ifdef BOOST_GIL_IO_TEST_ALLOW_WRITING_IMAGES
    write_view(bmp_out + "rgb_partial.bmp", view(img), tag_t());
#endif // BOOST_GIL_IO_TEST_ALLOW_WRITING_IMAGES
  }
}
