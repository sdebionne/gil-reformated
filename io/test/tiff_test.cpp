...
BOOST_AUTO_TEST_CASE(read_image_info_test) {
  {
    ...
    backend_t backend = read_image_info(tiff_filename, tag_t());
...

  {
    ...
    backend_t backend = read_image_info(file, tag_t());
...
BOOST_AUTO_TEST_CASE(read_and_convert_image_test_2) {
  ...
  BOOST_CHECK(equal_pixels(
      const_view(img), color_converted_view<gray8_pixel_t>(const_view(img2))));
}
...
BOOST_AUTO_TEST_CASE(subimage_test) {
  run_subimage_test<rgba8_image_t, tag_t>(tiff_filename, point_t(0, 0),
                                          point_t(50, 50));

  run_subimage_test<rgba8_image_t, tag_t>(tiff_filename, point_t(50, 50),
                                          point_t(50, 50));
}
...
BOOST_AUTO_TEST_CASE(dynamic_image_test) {
  ...
  typedef mpl::vector<gray8_image_t, gray16_image_t, rgba8_image_t,
                      gray1_image_t>
      my_img_types;

  any_image<my_img_types> runtime_image;

  read_image(tiff_filename.c_str(), runtime_image, tag_t());

#ifdef BOOST_GIL_IO_TEST_ALLOW_WRITING_IMAGES
  write_view(tiff_out + "dynamic_image_test.tif", view(runtime_image), tag_t());
#endif // BOOST_GIL_IO_TEST_ALLOW_WRITING_IMAGES
}
