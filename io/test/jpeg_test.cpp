...
BOOST_AUTO_TEST_CASE(read_image_info_test) {

  {
    ...
    backend_t backend = read_image_info(jpeg_filename, tag_t());
...

  {
    ...
    typedef get_reader_backend<std::ifstream, tag_t>::type backend_t;

    backend_t backend = read_image_info(in, tag_t());
...

  {
    ...
    typedef get_reader_backend<FILE *, tag_t>::type backend_t;

    backend_t backend = boost::gil::read_image_info(file, tag_t());
  ...
  {
    typedef get_reader_backend<path, tag_t>::type backend_t;

    backend_t backend =
        boost::gil::read_image_info(path(jpeg_filename), tag_t());
...
BOOST_AUTO_TEST_CASE(read_image_test) {
...

  {
    ...
    image_read_settings<jpeg_tag> settings(point_t(0, 0), point_t(10, 10),
...
BOOST_AUTO_TEST_CASE(write_view_test) {
  {
    ...
    write_view(filename,
               create_mandel_view(320, 240, rgb8_pixel_t(0, 0, 255),
...
BOOST_AUTO_TEST_CASE(subimage_test) {
  run_subimage_test<rgb8_image_t, tag_t>(jpeg_filename, point_t(0, 0),
                                         point_t(50, 50));

  run_subimage_test<rgb8_image_t, tag_t>(jpeg_filename, point_t(43, 24),
                                         point_t(50, 50));
}
...
BOOST_AUTO_TEST_CASE(dynamic_image_test) {
  ...
  read_image(jpeg_filename.c_str(), runtime_image, jpeg_tag());

#ifdef BOOST_GIL_IO_TEST_ALLOW_WRITING_IMAGES
  write_view(jpeg_out + "old_dynamic_image_test.jpg", view(runtime_image),
             jpeg_tag());
#endif // BOOST_GIL_IO_TEST_ALLOW_WRITING_IMAGES
}
